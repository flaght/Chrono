...
class SQLiteMemoryStorage:
    ...
    def _init_db(self):
        ...
        # 🌟 2. 核心平滑热升级：检测已有表的列名，自动补齐统计字段，保护已有数据！
        cursor.execute("PRAGMA table_info(case_memories)")
        ...
        for col_name, col_type in needed_columns.items():
            if col_name not in existing_cols:
                cursor.execute(f"ALTER TABLE case_memories ADD COLUMN {col_name} {col_type}")
    ...
    def load_memory_by_case_id(self, case_id: str) -> Optional[dict]:
        cursor = self.conn.cursor()
        cursor.execute("""
        ...
        return {
            ...
            "stats": {
                ...
                "acceptance_rate": round(acc_cnt / ret_cnt, 4) if ret_cnt > 0 else 0.0
            },
    ...
    def load_atomic_memory(self, matched_uuid: str,
        ...
        if types == MODALITY_PREDICT:  #predict
            cursor.execute(
                """SELECT case_id FROM case_memories WHERE predict_uuid = ?""",
                (matched_uuid, ))
        elif types == MODALITY_REGIME:  #regmie
            cursor.execute(
                """SELECT case_id FROM case_memories WHERE regime_uuid = ?""",
                (matched_uuid, ))
        elif types == MODALITY_TEXT:  #textual
            cursor.execute(
                """SELECT case_id FROM case_memories WHERE textual_uuid = ?""",
                (matched_uuid, ))
        ...
        return self.load_memory_by_case_id(row[0])
...
class MultimodalBrain:
    ...
    def update_feedback(self, symbol, predict_uuids, regime_uuids,
        ...
        if predict_uuids and hasattr(self.predict_brain, "update_access_count_with_feed_back"):
            self.predict_brain.update_access_count_with_feed_back(symbol=symbol, uuids=predict_uuids, feedback=reward)
        if regime_uuids and hasattr(self.regime_brain, "update_access_count_with_feed_back"):
            self.regime_brain.update_access_count_with_feed_back(symbol=symbol, uuids=regime_uuids, feedback=reward)
        if textual_uuids and hasattr(self.textual_barain, "update_access_count_with_feed_back"):
            self.textual_barain.update_access_count_with_feed_back(symbol=symbol, uuids=textual_uuids, feedback=reward)

    def filter_unvalidated_memory(self, symbol: str, min_access: int = 1):
        from lib.bra001 import filter_unvalidated_memory as bra_filter
        res = {}
        if hasattr(self, "predict_brain"):
            res['predict'] = bra_filter(self.predict_brain, symbol=symbol, min_access=min_access)
        if hasattr(self, "regime_brain"):
            res['regime'] = bra_filter(self.regime_brain, symbol=symbol, min_access=min_access)
        if hasattr(self, "textual_barain"):
            res['textual'] = bra_filter(self.textual_barain, symbol=symbol, min_access=min_access)
        return res
