...
class Portfolio(object):
    ...
    def on_turnover(self, turnover):
        ...
        if turnover.offset == Offset.OPEN:
            ...
            if turnover.direction == Direction.LONG:  # 买入平空
                if turnover.tradehd in self.short_position:
                    position = self.short_position.pop(turnover.tradehd)
                    realized_pnl_on_trade = (
                        position.price2 -
                        turnover.price1) * position.volume * self.multiplier
                else:
                    # 如果找不到持仓，打印警告信息，这有助于调试
                    print(
                        f"Warning: Short position with ID {turnover.tradehd} not found for closing."
                    )
            else:  # 卖出平多
                if turnover.tradehd in self.long_position:
                    position = self.long_position.pop(turnover.tradehd)
                    realized_pnl_on_trade = (
                        turnover.price1 -
                        position.price2) * position.volume * self.multiplier
                else:
                    print(
                        f"Warning: Long position with ID {turnover.tradehd} not found for closing."
                    )
    ...
    def update_unrealized_pnl(self, last_price):
        ...
        for trade in self.long_position.values():
            self.unrealized_pnl += (
                last_price - trade.price2) * trade.volume * self.multiplier
        # 空头浮盈
        for trade in self.short_position.values():
            self.unrealized_pnl += (
                trade.price2 - last_price) * trade.volume * self.multiplier
    ...
    def mark_to_market(self, date, settle_price):
        ...
        settle_pnl = 0.0
        for trade in self.long_position.values():
            settle_pnl += (settle_price -
                           trade.price2) * trade.volume * self.multiplier
            trade.price2 = settle_price  # 更新成本为结算价

        for trade in self.short_position.values():
            settle_pnl += (trade.price2 -
                           settle_price) * trade.volume * self.multiplier
            trade.price2 = settle_price  # 更新成本为结算价
