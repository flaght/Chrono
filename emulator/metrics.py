...
class Metrics(object):
    ...
    def __init__(self, daily_results, initial_capital):
        ...
        self.final_net_value = self.daily_df['net_value'].iloc[
            -1] if not self.daily_df.empty else initial_capital
        self.total_commission = self.daily_df['commission'].iloc[
            -1] if not self.daily_df.empty else 0
    ...
    def calculate_metrics(self):
        ...
        net_value_with_initial = pd.concat(
            [initial_row['net_value'], df['net_value']])
        ...
        drawdown_series = pd.concat(
            [pd.Series([self.initial_capital]), df['net_value']])
        ...
        sharpe_ratio = (df['daily_return'].mean() /
                        df['daily_return'].std()) * np.sqrt(
                            252) if df['daily_return'].std() != 0 else 0
        ...
        try:
            ...
            if not self.daily_df.empty:  # 增加一个检查，确保 DataFrame 不为空
                plot_series = pd.concat([
                    pd.Series({
                        self.daily_df.index[0] - pd.Timedelta(days=1):
                        self.initial_capital
                    }), self.daily_df['net_value']
                ])
                plot_series.plot(title='Portfolio Net Value Over Time',
                                 grid=True,
                                 marker='o',
                                 linestyle='-')
