...
class FactorEvaluate(object):
    ...
    def _init_factor(self):
        print('init factor')
        self.factor_data = self.factor_data.set_index(
            'trade_time').sort_index()[[self.factor_name, self.ret_name]]
        '''
    ...
    def _scale(self):
        ...
        if self.scale_method == 'raw':
            # 直接使用原始值，不进行任何缩放，假设为已经处理好的因子值，离散值为[-1,0,1], 连续值为[-1，1]
            self.factor_df['f_scaled'] = x
        #roll_min_max:
        ...
        elif self.scale_method == 'roll_min_max':
    ...
    def cal_pnl(self):
        ...
        sharpe = self.factor_data['net_ret'].mean(
        ) / self.factor_data['net_ret'].std()
        ...
        profit_ratio = (
            self.factor_data.loc[self.factor_data['net_ret'] > 0,
                                 'net_ret'].sum() /
            np.abs(self.factor_data.loc[self.factor_data['net_ret'] < 0,
                                        'net_ret']).sum()
        )  #  盈亏比。正收益的绝对值之和除以负收益的绝对值之和。衡量盈利时的平均盈利幅度与亏损时的平均亏损幅度之比。
        return {
            'total_ret': total_ret,
            'avg_ret': avg_ret,
            'max_dd': max_dd,
