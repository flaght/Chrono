...
from demos.triangle_spread.local_input import load_sector_research
from demos.triangle_spread.spread_strategy import TriangleSpreadConfig, TriangleSpreadStrategy
...
def _selected_products(basic: pd.DataFrame, products: tuple[str, ...]) -> dict[str, str]:
    ...
    if len(products) != 3 or len(set(products)) != len(products) or any(
        not item.isalpha() or item != item.upper() for item in products
    ):
        raise ValueError("必须指定三个不重复的品种字母代码")
    result = {}
