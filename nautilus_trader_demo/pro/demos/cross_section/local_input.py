    ...
    *, bars_dir: str | Path, contract_struct_path: str | Path,
    ...
    for path in root.rglob("*.feather"):
        ...
        if match and match.group(1).rstrip("0123456789").upper() in signal_products:
            label = match.group(2)
            files.append((path, match.group(1).lower(), date.fromisoformat(
                f"{label[:4]}-{label[4:6]}-{label[6:]}",
            )))
    if not files:
    ...
    for path, symbol, trading_day in sorted(files):
        ...
        time_key = "datetime" if "datetime" in frame else "timestamp"
        if time_key not in frame or "close" not in frame:
            raise ValueError(f"{path}缺少datetime/timestamp或close")
        if frame.empty:
            continue
