...
class CtpTdApiTransport:
    ...
    `td_api_base`仅供假API测试；生产使用时延迟导入vnpy_ctp.api.TdApi，
    该类是CTP原生TraderApi的Python薄绑定，不使用vn.py交易Gateway。
    ...
    def connect(self, on_order, on_trade, on_disconnect) -> CtpTraderSession:
        ...
        if base is None:
            from vnpy_ctp.api import TdApi  # 延迟导入，避免无CTP库时影响回测
            base = TdApi
        owner = self
        ...
        self._api = api
        try:
            api.createFtdcTraderApi(self.flow_path)
    ...
    def _response(self, kind: str, data: Any, error: Any, reqid: int, last: bool) -> None:
        with self._cv:
            ...
            if error_id:
                pending.error = f"CTP {kind}回报错误: {error_id}"
