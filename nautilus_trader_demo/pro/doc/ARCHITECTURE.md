...
```mermaid
flowchart LR
    MD[CTP MdApi / DolphinDB] --> GW[行情适配层]
    HIST[CSV / Parquet / DolphinDB 历史] --> HS[HistoryService]
    HS --> GW
...
| HistoryService | 按统一请求加载 Bar 和外部数据历史，拼接文件、DolphinDB 与当日缓存 | 策略信号、实时事件分发 |
...
| 模式 | 市场数据 | 时钟 | 执行端 | 权威持仓 |
|---|---|---|---|---|
| 回测 | 本地 CSV/Parquet/Catalog、DolphinDB 历史查询 | 历史事件时钟 | NT/BB 本地撮合器 | 回测 Portfolio |
| SimNow | CTP MdApi 或实时 DolphinDB | 实时时钟 | BB → CTP TraderApi | SimNow 查询结果 |
| 实盘 | CTP MdApi 或实时 DolphinDB | 实时时钟 | BB → CTP TraderApi | 经纪商/交易所查询结果 |
...
```mermaid
flowchart LR
    SRC[CSV / Parquet / DolphinDB / Catalog] --> LOAD[Historical DataAPI]
...
flowchart TD
    ...
    CUT --> LOAD[加载文件、DolphinDB 历史与当日缓存]
...
合并主键使用 `dataset_id + key + ts_event + revision`。同一记录来自多个来源时，默认优先级为“实时/当日缓存 > DolphinDB 历史 > 本地历史文件”，具体优先级必须配置并记录审计信息。Bar 还要检查完整窗口、交易日、夜盘和是否已经收盘。
...
回测由 `LocalHistoricalProvider` 提供历史并按 `ts_init` 回放；实盘由组合 Provider 拼接文件、DolphinDB 和缓存，再接入 DolphinDB/CTP 实时流。策略和 `HistoryRequest` 不随运行模式变化。
...
- 实盘启动通过 DolphinDB Provider 加载历史，运行中通过 `subscribe` 接收相同的 `ExternalDataRecord`；
...
DolphinDB 可以同时承担历史查询、实时行情和外部数据服务，但两类流的语义不同：
...
sequenceDiagram
    ...
    R->>D: 加载 Instrument、Tick/Bar、参考数据
    D->>E: 标准事件流
    loop 每个事件时间
        E->>E: 推进历史时钟
        E->>M: 更新盘口并撮合已有订单
...
IPyStrategy
  ...
  │    └─ ExternalDataHub：本地或 DolphinDB 外部数据 Provider
  └─ TradingPort
...
| DolphinDB Stream | DolphinDB | Bomber/CTP |
| CTP | DolphinDB | vn.py/CTP |
...
4. 实现 `DolphinDBLiveDataClient` 与 CTP/Bomber Data Client；
