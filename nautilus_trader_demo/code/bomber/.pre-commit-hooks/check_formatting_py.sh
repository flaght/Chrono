...
rg_exit=0
output=$(rg -n -B 1 --no-heading '^\s*if\s' bomber tests examples docs python build.py --type py 2> /dev/null) || rg_exit=$?
...
rg_exit=0
output=$(rg -n -B 1 --no-heading '^\s*match\s' bomber tests examples docs python build.py --type py 2> /dev/null) || rg_exit=$?
if [ $rg_exit -gt 1 ]; then
  echo "ERROR: ripgrep failed with exit code $rg_exit"
  exit 1
fi
...
rg_exit=0
output=$(rg -n -B 1 --no-heading '^\s*(async\s+)?for\s' bomber tests examples docs python build.py --type py 2> /dev/null) || rg_exit=$?
if [ $rg_exit -gt 1 ]; then
  echo "ERROR: ripgrep failed with exit code $rg_exit"
  exit 1
fi
...
rg_exit=0
output=$(rg -n -B 1 --no-heading '^\s*while\s' bomber tests examples docs python build.py --type py 2> /dev/null) || rg_exit=$?
if [ $rg_exit -gt 1 ]; then
  echo "ERROR: ripgrep failed with exit code $rg_exit"
  exit 1
fi
