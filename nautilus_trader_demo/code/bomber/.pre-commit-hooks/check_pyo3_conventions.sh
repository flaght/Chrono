...
while IFS=: read -r file line_num match; do
  ...
  case "$module_path" in
    bomber.adapters.* | bomber.core.bomber_pyo3.*)
      continue
      ;;
  esac
  ...
  echo "  Use: bomber.adapters.<adapter_name> for stub metadata"
  echo "  Runtime PyO3 paths may use bomber.core.bomber_pyo3.<adapter_name>"
  echo
  ADAPTER_VIOLATIONS=$((ADAPTER_VIOLATIONS + 1))
done < <(rg -n '(module|stub_module)\s*=\s*"bomber\.[^"]+"' crates/adapters --type rust 2> /dev/null || true)

if [ $ADAPTER_VIOLATIONS -gt 0 ]; then
  echo -e "${RED}Found $ADAPTER_VIOLATIONS adapter module path violation(s)${NC}"
  echo
  echo "Convention:"
  echo "  - Public adapter stub paths use bomber.adapters.<adapter_name>"
  echo "  - Runtime PyO3 paths use bomber.core.bomber_pyo3.<adapter_name>"
  exit 1
fi
