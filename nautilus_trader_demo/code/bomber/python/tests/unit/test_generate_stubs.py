...
def test_collect_rust_class_fixups_reads_custom_data_stub_module(tmp_path):
    ...
    rust_file.write_text(
        """
#[custom_data(pyo3, no_arrow, stub_module = "bomber.adapters.hyperliquid")]
...
#[cfg_attr(
    feature = "arrow",
    custom_data(pyo3, stub_module = "bomber.adapters.hyperliquid")
)]
#[cfg_attr(
    not(feature = "arrow"),
    custom_data(pyo3, no_arrow, stub_module = "bomber.adapters.hyperliquid")
...
STUB_ROOT = WORKSPACE_ROOT / "python" / "bomber"
...
def _module_name_from_stub_path(relative_package: Path) -> str:
    ...
    return f"bomber.{'.'.join(relative_package.parts)}"
