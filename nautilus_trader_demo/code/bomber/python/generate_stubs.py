...
# -------------------------------------------------------------------------------------------------
"""
Custom build script for bomber v2 with automatic stub generation.
...
EXTRA_REEXPORTS: dict[str, tuple[str, ...]] = {
    "bomber/analysis/__init__.pyi": (
        "from bomber.analysis.config import GridLayout as GridLayout",
        "from bomber.analysis.config import TearsheetBarsWithFillsChart as TearsheetBarsWithFillsChart",
        ...
        "from bomber.analysis.config import TearsheetMonthlyReturnsChart as TearsheetMonthlyReturnsChart",
        "from bomber.analysis.config import TearsheetRollingSharpeChart as TearsheetRollingSharpeChart",
        ...
        "from bomber.analysis.config import TearsheetYearlyReturnsChart as TearsheetYearlyReturnsChart",
        ...
        "from bomber.analysis.tearsheet import create_monthly_returns_heatmap as create_monthly_returns_heatmap",
        "from bomber.analysis.tearsheet import create_returns_distribution as create_returns_distribution",
        ...
        "from bomber.analysis.tearsheet import create_tearsheet_from_stats as create_tearsheet_from_stats",
...
def add_missing_model_imports(content: str) -> str:
    ...
    """
    model_import = "from bomber import model"
    ...
    for name in sorted(MODEL_EXPORTS):
        if name in defined_names:
            continue
        if f"from bomber.model import {name}" in content:
            continue
    ...
    injected_imports = "\n".join(
        f"from bomber.model import {name}" for name in missing_imports
    )
...
def normalize_stub_content(content: str) -> str:
    ...
    # e.g. bomber.model.X -> model.X
    if "from bomber import model" in content:
        content = re.sub(r"\bbomber\.model\.", "model.", content)
    if "from bomber import infrastructure" in content:
        content = re.sub(r"\bbomber\.infrastructure\.", "infrastructure.", content)
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Bomber v2 build script")
    parser.add_argument(
        "action",
        nargs="?",
        default="stubs",
    ...
    print(f"Starting bomber v2 {args.action}...")
