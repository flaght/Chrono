...
This directory contains the `bomber` v2 Python package.
...
- This directory will remain when v1 is removed (the top-level `bomber/` goes away).
...
```bash
git clone https://github.com/nautechsystems/bomber.git
cd bomber/python
uv run maturin develop --extras dev,test
```
...
outside the Bomber source checkout so the repository's `exclude-newer` uv policy does not
filter out newly published v2 wheels. The installed package imports as `bomber`.
