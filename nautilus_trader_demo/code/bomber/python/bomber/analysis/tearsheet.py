...
from bomber.core import NAUTILUS_VERSION
...
def _require_pandas():
    try:
        ...
        raise ImportError(
            ...
            "`pip install bomber[visualization]`",
        ) from e
...
def _write_figure(fig: go.Figure, output_path: str) -> None:
    # Static image export uses Kaleido (install via `bomber[visualization]`).
...
def create_tearsheet(
    ...
    title: str = "Bomber Backtest Results",
    currency=None,
    config=None,
    ...
    title : str, default "Bomber Backtest Results"
    ...
    if not PLOTLY_AVAILABLE:
        msg = (
            ...
            "Install it with: pip install bomber[visualization]"
        )
    ...
    if title == "Bomber Backtest Results":
        ...
        title = f"<b>Bomber</b> v{NAUTILUS_VERSION} - Backtest Results"
...
def _create_tearsheet_figure(
    ...
    fig.update_layout(
        title_text=config.title if config.title != "Bomber Backtest Results" else title,
        title_font_size=20,
        template=theme_config["template"],
        height=config.height,
        showlegend=benchmark_returns is not None,
