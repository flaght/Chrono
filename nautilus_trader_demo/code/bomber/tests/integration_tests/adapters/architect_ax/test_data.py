...
from bomber.adapters.architect_ax.data import AxDataClient
...
def data_client_builder(
    ...
    def builder(monkeypatch, *, config_kwargs: dict | None = None):
        ...
        monkeypatch.setattr(
            "bomber.adapters.architect_ax.data.bomber_pyo3.AxMdWebSocketClient.without_auth",
...
async def test_subscribe_order_book_deltas_l2(data_client_builder, monkeypatch):
    ...
    try:
        ...
        # Assert
        expected_id = bomber_pyo3.InstrumentId.from_str("GBPUSD-PERP.AX")
        ws_client.subscribe_book_deltas.assert_awaited_once_with(
            expected_id,
            bomber_pyo3.AxMarketDataLevel.LEVEL2,
        )
    finally:
        await client._disconnect()
...
async def test_subscribe_order_book_deltas_l3(data_client_builder, monkeypatch):
    ...
    try:
        ...
        ws_client.subscribe_book_deltas.assert_awaited_once_with(
            expected_id,
            bomber_pyo3.AxMarketDataLevel.LEVEL3,
        )
    finally:
        await client._disconnect()
...
async def test_subscribe_bars(data_client_builder, monkeypatch):
    ...
    try:
        ...
        # Assert
        expected_bar_type = bomber_pyo3.BarType.from_str(
