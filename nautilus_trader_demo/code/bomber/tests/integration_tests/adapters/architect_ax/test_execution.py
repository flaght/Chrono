...
from bomber.adapters.architect_ax.execution import AxExecutionClient
...
def exec_client_builder(
    ...
    def builder(monkeypatch, *, config_kwargs: dict | None = None):
        ...
        monkeypatch.setattr(
            "bomber.adapters.architect_ax.execution.bomber_pyo3.AxOrdersWebSocketClient",
            lambda *args, **kwargs: next(ws_iter),
        )

        monkeypatch.setattr(
            "bomber.adapters.architect_ax.execution.AxExecutionClient._await_account_registered",
            AsyncMock(),
        )
        ...
        config = AxExecClientConfig(
            ...
            environment=bomber_pyo3.AxEnvironment.SANDBOX,
            **(config_kwargs or {}),
        )
...
async def test_generate_order_status_reports(exec_client_builder, monkeypatch):
    ...
    monkeypatch.setattr(
        "bomber.adapters.architect_ax.execution.OrderStatusReport.from_pyo3",
...
async def test_generate_fill_reports(exec_client_builder, monkeypatch):
    ...
    expected_report = MagicMock()
    monkeypatch.setattr(
        "bomber.adapters.architect_ax.execution.FillReport.from_pyo3",
        lambda obj: expected_report,
    )
...
async def test_generate_position_status_reports(exec_client_builder, monkeypatch):
    ...
    expected_report = MagicMock()
    monkeypatch.setattr(
        "bomber.adapters.architect_ax.execution.PositionStatusReport.from_pyo3",
        lambda obj: expected_report,
    )
