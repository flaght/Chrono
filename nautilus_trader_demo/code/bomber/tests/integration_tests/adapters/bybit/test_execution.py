...
from bomber.adapters.bybit.execution import _is_confirmed_submit_rejection_error
from bomber.adapters.bybit.execution import _parse_bybit_tp_sl_params
...
from bomber.model.orders import OrderList
from bomber.model.orders import StopMarketOrder
from bomber.test_kit.functions import eventually
...
def exec_client_builder(
    ...
    def builder(monkeypatch, *, config_kwargs: dict | None = None, clock=None):
        ...
        monkeypatch.setattr(
            "bomber.adapters.bybit.execution.bomber_pyo3.BybitWebSocketClient.new_private",
            lambda *args, **kwargs: next(ws_iter),
        )
        monkeypatch.setattr(
            "bomber.adapters.bybit.execution.bomber_pyo3.BybitWebSocketClient.new_trade",
            lambda *args, **kwargs: next(ws_iter),
        )

        # Skip account registration wait in tests
        monkeypatch.setattr(
            "bomber.adapters.bybit.execution.BybitExecutionClient._await_account_registered",
            AsyncMock(),
        )
...
async def test_generate_order_status_reports_converts_results(exec_client_builder, monkeypatch):
    ...
    monkeypatch.setattr(
        "bomber.adapters.bybit.execution.OrderStatusReport.from_pyo3",
...
async def test_generate_fill_reports_converts_results(exec_client_builder, monkeypatch):
    ...
    expected_report = MagicMock()
    monkeypatch.setattr(
        "bomber.adapters.bybit.execution.FillReport.from_pyo3",
        lambda obj: expected_report,
    )
...
async def test_generate_position_status_reports_converts_results(exec_client_builder, monkeypatch):
    ...
    expected_report = MagicMock()
    monkeypatch.setattr(
        "bomber.adapters.bybit.execution.PositionStatusReport.from_pyo3",
        lambda obj: expected_report,
    )
...
async def test_submit_market_order(exec_client_builder, monkeypatch, instrument):
    ...
    try:
        ...
        assert isinstance(kwargs["order_side"], bomber_pyo3.OrderSide)
        assert isinstance(kwargs["order_type"], bomber_pyo3.OrderType)
        assert isinstance(kwargs["time_in_force"], bomber_pyo3.TimeInForce)
    finally:
        await client._disconnect()
...
async def test_submit_limit_order_with_take_profit_stop_loss(
    ...
    try:
        ...
        assert call_kwargs["take_profit"] == bomber_pyo3.Price.from_str("55000.00")
        assert call_kwargs["stop_loss"] == bomber_pyo3.Price.from_str("47000.00")
        ws_trade_client.batch_place_orders.assert_awaited_once()
    finally:
        await client._disconnect()
...
@pytest.mark.parametrize(
    ...
    [
        (OrderSide.BUY, False, bomber_pyo3.BybitPositionIdx.BUY_HEDGE),
        (OrderSide.SELL, False, bomber_pyo3.BybitPositionIdx.SELL_HEDGE),
        (OrderSide.SELL, True, bomber_pyo3.BybitPositionIdx.BUY_HEDGE),
        (OrderSide.BUY, True, bomber_pyo3.BybitPositionIdx.SELL_HEDGE),
    ],
)
def test_resolve_position_idx_hedge_mode(
    ...
    client, *_ = exec_client_builder(
        ...
        config_kwargs={
            "position_mode": {
                instrument_id.symbol.value: bomber_pyo3.BybitPositionMode.BOTH_SIDES,
            },
        },
    )
...
def test_resolve_position_idx_one_way_mode(exec_client_builder, monkeypatch):
    ...
    client, *_ = exec_client_builder(
        ...
        config_kwargs={
            "position_mode": {
                instrument_id.symbol.value: bomber_pyo3.BybitPositionMode.MERGED_SINGLE,
            },
        },
    )
    ...
    assert (
        ...
        == bomber_pyo3.BybitPositionIdx.ONE_WAY
    )
...
def test_resolve_position_idx_manual_override_wins(exec_client_builder, monkeypatch):
    ...
    assert (
        ...
        == bomber_pyo3.BybitPositionIdx.SELL_HEDGE
    )
...
def test_resolve_position_idx_returns_none_when_symbol_not_in_map(
    ...
    client, *_ = exec_client_builder(
        ...
        config_kwargs={
            "position_mode": {
                "ETHUSDT-LINEAR": bomber_pyo3.BybitPositionMode.BOTH_SIDES,
            },
        },
    )
...
def test_resolve_position_idx_returns_none_for_non_derivative_products(
    ...
    client, *_ = exec_client_builder(
        ...
        config_kwargs={
            "position_mode": {
                symbol: bomber_pyo3.BybitPositionMode.MERGED_SINGLE,
            },
        },
    )
...
@pytest.mark.parametrize(
    ...
    [
        (None, None),
        (bomber_pyo3.BybitPositionIdx.ONE_WAY, None),
        (bomber_pyo3.BybitPositionIdx.BUY_HEDGE, "LTCUSDT-LINEAR.BYBIT-LONG"),
        (bomber_pyo3.BybitPositionIdx.SELL_HEDGE, "LTCUSDT-LINEAR.BYBIT-SHORT"),
    ],
)
def test_make_hedge_venue_position_id(position_idx, expected):
    instrument_id = bomber_pyo3.InstrumentId.from_str("LTCUSDT-LINEAR.BYBIT")

    result = bomber_pyo3.bybit_make_hedge_venue_position_id(instrument_id, position_idx)
...
def test_cache_order_position_id_caches_only_hedge_indexes(exec_client_builder, monkeypatch):
    ...
    client._cache_order_position_id(order, bomber_pyo3.BybitPositionIdx.BUY_HEDGE)

    assert client._order_position_ids[order.client_order_id] == PositionId(
        "LTCUSDT-LINEAR.BYBIT-LONG",
    )

    client._cache_order_position_id(order, bomber_pyo3.BybitPositionIdx.ONE_WAY)
...
def test_handle_fill_report_uses_cached_position_id_across_partial_fills(
    ...
    def fill_report(trade_id: str, last_qty: str) -> bomber_pyo3.FillReport:
        return bomber_pyo3.FillReport(
            account_id=bomber_pyo3.AccountId("BYBIT-UNIFIED"),
            instrument_id=bomber_pyo3.InstrumentId.from_str(instrument.id.value),
            venue_order_id=bomber_pyo3.VenueOrderId("BYBIT-ORDER-001"),
            trade_id=bomber_pyo3.TradeId(trade_id),
            order_side=bomber_pyo3.OrderSide.BUY,
            last_qty=bomber_pyo3.Quantity.from_str(last_qty),
            last_px=bomber_pyo3.Price.from_str("50000.00"),
            commission=bomber_pyo3.Money.from_str("0.01 USDT"),
            liquidity_side=bomber_pyo3.LiquiditySide.TAKER,
            ts_event=0,
            client_order_id=bomber_pyo3.ClientOrderId(order.client_order_id.value),
            report_id=bomber_pyo3.UUID4(),
            ts_init=0,
        )
...
async def test_submit_order_with_tp_sl_in_demo_mode_routes_to_http(
    ...
    client, ws_client, http_client, instrument_provider = exec_client_builder(
        monkeypatch,
        config_kwargs={"environment": bomber_pyo3.BybitEnvironment.DEMO},
    )
...
async def test_submit_order_live_ws_failure_waits_for_reconciliation(
    ...
    client, _, _, _ = exec_client_builder(
        ...
        config_kwargs={
            "position_mode": {
                instrument.id.symbol.value: bomber_pyo3.BybitPositionMode.BOTH_SIDES,
            },
        },
    )
...
async def test_handle_order_rejected_pyo3_conversion(
    ...
    pyo3_event = bomber_pyo3.OrderRejected(
        trader_id=bomber_pyo3.TraderId(TestIdStubs.trader_id().value),
        strategy_id=bomber_pyo3.StrategyId(TestIdStubs.strategy_id().value),
        instrument_id=bomber_pyo3.InstrumentId.from_str(instrument.id.value),
        client_order_id=bomber_pyo3.ClientOrderId("O-123456"),
        account_id=bomber_pyo3.AccountId(TestIdStubs.account_id().value),
        reason="InsufficientMargin",
        event_id=bomber_pyo3.UUID4(),
        ts_event=123456789,
        ts_init=123456789,
...
async def test_handle_order_cancel_rejected_pyo3_conversion(
    ...
    pyo3_event = bomber_pyo3.OrderCancelRejected(
        ...
        instrument_id=bomber_pyo3.InstrumentId.from_str(instrument.id.value),
        client_order_id=bomber_pyo3.ClientOrderId("O-123456"),
        venue_order_id=bomber_pyo3.VenueOrderId("BYBIT-12345"),
...
async def test_repay_spot_borrow_handles_api_errors_gracefully(
    ...
    bought_qty = bomber_pyo3.Quantity(50.0, 2)
...
async def test_repay_spot_borrow_skips_when_no_borrow(
    ...
    bought_qty = bomber_pyo3.Quantity(10.0, 2)
...
async def test_repay_spot_borrow_calls_repay_when_borrow_exists(
    ...
    http_client.repay_spot_borrow = AsyncMock()
    bought_qty = bomber_pyo3.Quantity(100.0, 2)
...
async def test_repay_spot_borrow_repays_partial_when_bought_less_than_borrowed(
    ...
    http_client.repay_spot_borrow = AsyncMock()
    bought_qty = bomber_pyo3.Quantity(150.0, 2)
...
async def test_repay_spot_borrow_repays_partial_single(
    ...
    try:
        ...
        fill_report = bomber_pyo3.FillReport(
            account_id=bomber_pyo3.AccountId("BYBIT-UNIFIED"),
            instrument_id=bomber_pyo3.InstrumentId.from_str(spot_instrument.id.value),
            venue_order_id=bomber_pyo3.VenueOrderId("BYBIT-PARTIAL-123"),
            trade_id=bomber_pyo3.TradeId("T-PARTIAL-001"),
            order_side=bomber_pyo3.OrderSide.BUY,
            last_qty=bomber_pyo3.Quantity.from_str("0.500000"),
            last_px=bomber_pyo3.Price.from_str("50000.00"),
            commission=bomber_pyo3.Money.from_str("0.025 USDT"),
            liquidity_side=bomber_pyo3.LiquiditySide.TAKER,
            ts_event=0,
            client_order_id=bomber_pyo3.ClientOrderId("O-PARTIAL-REPAY"),
            report_id=bomber_pyo3.UUID4(),
            ts_init=0,
        )
    ...
    try:
        ...
        for i, (order, qty) in enumerate(orders):
            fill_report = bomber_pyo3.FillReport(
                account_id=bomber_pyo3.AccountId("BYBIT-UNIFIED"),
                instrument_id=bomber_pyo3.InstrumentId.from_str(spot_instrument.id.value),
                venue_order_id=bomber_pyo3.VenueOrderId(f"BYBIT-MULTI-{i}"),
                trade_id=bomber_pyo3.TradeId(f"T-MULTI-{i:03d}"),
                order_side=bomber_pyo3.OrderSide.BUY,
                last_qty=bomber_pyo3.Quantity.from_str(qty),
                last_px=bomber_pyo3.Price.from_str("3000.00"),
                commission=bomber_pyo3.Money.from_str("0.025 USDT"),
                liquidity_side=bomber_pyo3.LiquiditySide.TAKER,
                ts_event=0,
                client_order_id=bomber_pyo3.ClientOrderId(order.client_order_id.value),
...
async def test_auto_repayment_skipped_during_blackout_window(
    ...
    http_client.repay_spot_borrow = AsyncMock()
    bought_qty = bomber_pyo3.Quantity(1.0, 2)
...
async def test_repay_accepts_decimal_type_from_fill_accumulation(
    ...
    bought_qty = bomber_pyo3.Quantity(0.08, 5)
    ...
    try:
        ...
        call_args = http_client.repay_spot_borrow.call_args
        assert call_args[0][0] == "ETH"
        assert isinstance(call_args[0][1], bomber_pyo3.Quantity)
    finally:
        await client._disconnect()
...
async def test_handle_fill_report_tracks_partial_fills_for_spot_buy(
    ...
    fill_report = bomber_pyo3.FillReport(
        account_id=bomber_pyo3.AccountId("BYBIT-UNIFIED"),
        instrument_id=bomber_pyo3.InstrumentId.from_str(spot_instrument.id.value),
        venue_order_id=bomber_pyo3.VenueOrderId("BYBIT-789"),
        trade_id=bomber_pyo3.TradeId("T-001"),
        order_side=bomber_pyo3.OrderSide.BUY,
        last_qty=bomber_pyo3.Quantity(0.050, 5),
        last_px=bomber_pyo3.Price(3000.00, 2),
        commission=bomber_pyo3.Money.from_str("0.01 USDT"),
        liquidity_side=bomber_pyo3.LiquiditySide.TAKER,
        ts_event=0,
        client_order_id=bomber_pyo3.ClientOrderId("O-123456"),
    ...
    try:
        ...
        fill_report2 = bomber_pyo3.FillReport(
            account_id=bomber_pyo3.AccountId("BYBIT-UNIFIED"),
            instrument_id=bomber_pyo3.InstrumentId.from_str(spot_instrument.id.value),
            venue_order_id=bomber_pyo3.VenueOrderId("BYBIT-789"),
            trade_id=bomber_pyo3.TradeId("T-002"),
            order_side=bomber_pyo3.OrderSide.BUY,
            last_qty=bomber_pyo3.Quantity(0.050, 5),
            last_px=bomber_pyo3.Price(3000.00, 2),
            commission=bomber_pyo3.Money.from_str("0.01 USDT"),
...
async def test_handle_fill_report_ignores_non_spot_orders(
    ...
    fill_report = bomber_pyo3.FillReport(
        ...
        trade_id=bomber_pyo3.TradeId("T-001"),
        order_side=bomber_pyo3.OrderSide.BUY,
        last_qty=bomber_pyo3.Quantity.from_str("0.100"),
        last_px=bomber_pyo3.Price.from_str("50000.00"),
        commission=bomber_pyo3.Money.from_str("0.01 USDT"),
        liquidity_side=bomber_pyo3.LiquiditySide.TAKER,
        ts_event=0,
...
async def test_query_account_borrow_action_success(
    ...
    command = QueryAccount(
        ...
        client_id=client.id,
        ts_init=0,
        params={"action": bomber_pyo3.BybitMarginAction.BORROW, "coin": "USDT", "amount": 1000},
    )
...
async def test_query_account_borrow_action_publishes_result(
    ...
    data_type = DataType(bomber_pyo3.BybitMarginBorrowResult)
    ...
    try:
        ...
        assert isinstance(result, bomber_pyo3.BybitMarginBorrowResult)
...
async def test_query_account_repay_action_publishes_result(
    ...
    data_type = DataType(bomber_pyo3.BybitMarginRepayResult)
    msgbus.subscribe(topic=f"data.{data_type.topic}", handler=handler)
    ...
    command = QueryAccount(
        ...
        client_id=client.id,
        ts_init=0,
        params={"action": bomber_pyo3.BybitMarginAction.REPAY, "coin": "USDT", "amount": 500},
    )
    ...
    try:
        ...
        assert len(received_data) == 1
        result = received_data[0]
        assert isinstance(result, bomber_pyo3.BybitMarginRepayResult)
        assert result.coin == "USDT"
...
async def test_query_account_get_borrow_amount_publishes_result(
    ...
    data_type = DataType(bomber_pyo3.BybitMarginStatusResult)
    msgbus.subscribe(topic=f"data.{data_type.topic}", handler=handler)
    ...
    command = QueryAccount(
        ...
        client_id=client.id,
        ts_init=0,
        params={"action": bomber_pyo3.BybitMarginAction.GET_BORROW_AMOUNT, "coin": "USDT"},
    )
    ...
    try:
        ...
        assert len(received_data) == 1
        result = received_data[0]
        assert isinstance(result, bomber_pyo3.BybitMarginStatusResult)
        assert result.coin == "USDT"
...
async def test_query_account_borrow_action_missing_params(
    ...
    command = QueryAccount(
        ...
        client_id=client.id,
        ts_init=0,
        params={"action": bomber_pyo3.BybitMarginAction.BORROW, "coin": "USDT"},
    )
...
async def test_query_account_repay_action_repay_all(
    ...
    command = QueryAccount(
        ...
        client_id=client.id,
        ts_init=0,
        params={"action": bomber_pyo3.BybitMarginAction.REPAY, "coin": "USDT"},
    )
