...
def patch_fetch_fee_schedules():
    ...
    """
    with patch(
        "bomber.adapters.polymarket.providers.fetch_fee_schedules",
...
async def test_gamma_markets_loads_all_sibling_tokens(mock_clob_client, live_clock):
    ...
    with patch("bomber.adapters.polymarket.providers.list_markets") as mock_list_markets:
...
async def test_load_all_async_uses_event_slug_builder_when_configured(
    ...
    with patch(
        "bomber.adapters.polymarket.providers.PolymarketDataLoader._fetch_event_by_slug",
    ) as mock_fetch_event:
...
async def test_event_slug_builder_takes_priority_over_gamma_markets(
    ...
    with (
        ...
        patch(
            "bomber.adapters.polymarket.providers.list_markets",
...
async def test_event_slug_builder_callable_raises_exception(
    ...
    with patch(
        "bomber.adapters.polymarket.providers.resolve_path",
    ) as mock_resolve:
