...
def patch_fetch_fee_schedules():
    ...
    """
    with patch(
        "bomber.adapters.polymarket.loaders.fetch_fee_schedules",
...
async def test_fetch_markets(test_instrument, markets_list_data):
    # Arrange
    mock_http_client = MagicMock(spec=bomber_pyo3.HttpClient)
    mock_response = Mock()
...
async def test_load_trades_sorts_across_pages_with_same_second_collisions(
    ...
    loader = PolymarketDataLoader(
        ...
        http_client=MagicMock(spec=bomber_pyo3.HttpClient),
    )
