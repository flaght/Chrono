...
from bomber.adapters.okx.execution import OKXExecutionClient
...
def exec_client_builder(
    ...
    def builder(monkeypatch, *, config_kwargs: dict | None = None):
        ...
        monkeypatch.setattr(
            "bomber.adapters.okx.execution.bomber_pyo3.OKXWebSocketClient.with_credentials",
...
async def test_connect_success(exec_client_builder, monkeypatch):
    ...
    try:
        ...
        private_ws.subscribe_orders.assert_awaited_once_with(bomber_pyo3.OKXInstrumentType.SPOT)
...
async def test_connect_events_skips_algo_channels(exec_client_builder, monkeypatch):
    ...
    client, private_ws, business_ws, http_client, _ = exec_client_builder(
        monkeypatch,
        config_kwargs={"instrument_types": (bomber_pyo3.OKXInstrumentType.EVENTS,)},
    )
    ...
    try:
        # Assert
        private_ws.subscribe_orders.assert_awaited_once_with(bomber_pyo3.OKXInstrumentType.EVENTS)
...
async def test_generate_order_status_reports_converts_results(exec_client_builder, monkeypatch):
    ...
    monkeypatch.setattr(
        "bomber.adapters.okx.execution.OrderStatusReport.from_pyo3",
...
async def test_generate_order_status_reports_load_spreads_uses_generic_request(
    ...
    assert len(calls) == 2
    assert calls[0].kwargs["instrument_type"] == bomber_pyo3.OKXInstrumentType.SPOT
...
async def test_generate_fill_reports_converts_results(exec_client_builder, monkeypatch):
    ...
    expected_report = MagicMock()
    monkeypatch.setattr(
        "bomber.adapters.okx.execution.FillReport.from_pyo3",
        lambda obj: expected_report,
    )
...
async def test_generate_position_status_reports_converts_results(exec_client_builder, monkeypatch):
    ...
    client, _, _, http_client, _ = exec_client_builder(
        monkeypatch,
        config_kwargs={"instrument_types": (bomber_pyo3.OKXInstrumentType.SWAP,)},
    )

    expected_report = MagicMock()
    monkeypatch.setattr(
        "bomber.adapters.okx.execution.PositionStatusReport.from_pyo3",
        lambda obj: expected_report,
    )
...
async def test_generate_position_status_reports_spot_margin_uses_margin_inst_type(
    ...
    client, _, _, http_client, _ = exec_client_builder(
        ...
        config_kwargs={
            ...
            "margin_mode": bomber_pyo3.OKXMarginMode.CROSS,
        },
    )
    ...
    assert call_args.kwargs["instrument_type"] == bomber_pyo3.OKXInstrumentType.MARGIN
    assert reports == [expected_report]
...
def test_amend_algo_order_stubs_preserve_positional_argument_order():
    ...
    stub_paths = (
        repo_root / "bomber/core/bomber_pyo3.pyi",
        repo_root / "python/bomber/adapters/okx/__init__.pyi",
    )
    expected_order = (
        "new_trigger_price",
        "new_limit_price",
...
async def test_attached_oco_status_report_accepts_both_logical_children(
    ...
    pyo3_report = bomber_pyo3.OrderStatusReport(
        account_id=bomber_pyo3.AccountId(client.account_id.value),
        instrument_id=bomber_pyo3.InstrumentId.from_str(instrument.id.value),
        venue_order_id=bomber_pyo3.VenueOrderId("algo-oco-1"),
        client_order_id=bomber_pyo3.ClientOrderId(sl_order.client_order_id.value),
        order_side=bomber_pyo3.OrderSide.SELL,
        order_type=bomber_pyo3.OrderType.MARKET_IF_TOUCHED,
        time_in_force=bomber_pyo3.TimeInForce.GTC,
        order_status=bomber_pyo3.OrderStatus.ACCEPTED,
        quantity=bomber_pyo3.Quantity.from_str(str(sl_order.quantity)),
        filled_qty=bomber_pyo3.Quantity.from_str("0"),
        trigger_price=bomber_pyo3.Price.from_str("41000.00"),
        trigger_type=bomber_pyo3.TriggerType.DEFAULT,
        reduce_only=True,
        ts_accepted=0,
        ts_last=0,
...
async def test_handle_close_fraction_algo_accept_report_uses_cached_order_quantity(
    ...
    pyo3_report = bomber_pyo3.OrderStatusReport(
        account_id=bomber_pyo3.AccountId(client.account_id.value),
        instrument_id=bomber_pyo3.InstrumentId.from_str(instrument.id.value),
        venue_order_id=bomber_pyo3.VenueOrderId("algo-close-frac-1"),
        client_order_id=bomber_pyo3.ClientOrderId(order.client_order_id.value),
        order_side=bomber_pyo3.OrderSide.SELL,
        order_type=bomber_pyo3.OrderType.STOP_MARKET,
        time_in_force=bomber_pyo3.TimeInForce.GTC,
        order_status=bomber_pyo3.OrderStatus.ACCEPTED,
        quantity=bomber_pyo3.Quantity.from_str("0"),
        filled_qty=bomber_pyo3.Quantity.from_str("0"),
        trigger_price=bomber_pyo3.Price.from_str("40000.00"),
        trigger_type=bomber_pyo3.TriggerType.DEFAULT,
        reduce_only=True,
        ts_accepted=0,
        ts_last=0,
