...
from bomber.adapters.okx.factories import get_cached_okx_http_client
...
# Test suites
# ----------------------------------------------------------------------
async def quick_tests(http_client, bar_type: bomber_pyo3.BarType, logger: Logger) -> None:
    logger.info("\n=== QUICK TESTS ===")
...
async def limit_tests(http_client, bar_type: bomber_pyo3.BarType, logger: Logger) -> None:
...
async def edge_case_tests(http_client, bar_type: bomber_pyo3.BarType, logger: Logger) -> None:
    logger.info("\n=== EDGE-CASE TESTS ===")
    ...
    # Wrong instrument
    wrong_type = bomber_pyo3.BarType.from_str("BTC-USD-SWAP.OKX-1-MINUTE-SETTLEMENT-EXTERNAL")
    try:
...
async def pagination_demo(http_client, bar_type: bomber_pyo3.BarType, logger: Logger) -> None:
...
async def main(args: argparse.Namespace) -> None:
    ...
    # Cache instruments
    inst_type = bomber_pyo3.OKXInstrumentType.SWAP
    ...
    # BarType
    bar_type = bomber_pyo3.BarType.from_str(args.bar_type)
