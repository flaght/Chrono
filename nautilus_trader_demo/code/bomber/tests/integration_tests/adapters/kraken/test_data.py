...
async def test_request_all_instrument_statuses_merges_spot_and_futures(
    ...
    futures_http_client = MagicMock(spec=bomber_pyo3.KrakenFuturesHttpClient)
...
async def test_request_instrument_receives_single_instrument(
    ...
    mock_pyo3_instrument.id = bomber_pyo3.InstrumentId.from_str(instrument.id.value)
    ...
    try:
        ...
        with (
            patch(
                "bomber.adapters.kraken.data.transform_instrument_from_pyo3",
                return_value=instrument,
            ),
            patch(
                "bomber.adapters.kraken.data.KRAKEN_INSTRUMENT_TYPES",
