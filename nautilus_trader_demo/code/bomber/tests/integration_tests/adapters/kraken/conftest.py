...
def mock_http_client():
    mock = MagicMock(spec=bomber_pyo3.KrakenSpotHttpClient)
...
def _create_ws_mock() -> MagicMock:
    mock = MagicMock(spec=bomber_pyo3.KrakenSpotWebSocketClient)
...
def _create_mock_account_state():
    ...
    """
    return bomber_pyo3.AccountState(
...
def _create_mock_account_state_futures():
    ...
    return bomber_pyo3.AccountState(
        account_id=bomber_pyo3.AccountId("KRAKEN-UNIFIED"),
        account_type=bomber_pyo3.AccountType.MARGIN,
        base_currency=None,
        balances=[
            bomber_pyo3.AccountBalance(
                total=bomber_pyo3.Money.from_str("100000 USD"),
                locked=bomber_pyo3.Money.from_str("0 USD"),
                free=bomber_pyo3.Money.from_str("100000 USD"),
            ),
        ],
        margins=[],
        is_reported=True,
...
def mock_http_client_futures():
    ...
    """
    mock = MagicMock(spec=bomber_pyo3.KrakenFuturesHttpClient)
    mock.api_key = "test_api_key"
    mock.api_secret = "test_api_secret"
    mock.api_key_masked = "test_api_****"
...
def _create_exec_ws_mock_futures() -> MagicMock:
    ...
    """
    mock = MagicMock(spec=bomber_pyo3.KrakenFuturesWebSocketClient)
