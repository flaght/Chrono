...
from bomber.adapters.databento import loaders as loaders_module
...
def test_loader_mbo_pyo3() -> None:
    ...
    # Assert
    assert len(data) == 2
    assert isinstance(data[0], bomber_pyo3.OrderBookDelta)
    assert isinstance(data[1], bomber_pyo3.OrderBookDelta)
    delta = data[0]
    assert delta.instrument_id == bomber_pyo3.InstrumentId.from_str("ESH1.GLBX")
    assert delta.action == bomber_pyo3.BookAction.DELETE
    assert delta.order.side == bomber_pyo3.OrderSide.SELL
    assert delta.order.price == bomber_pyo3.Price.from_str("3722.75")
    assert delta.order.size == bomber_pyo3.Quantity.from_int(1)
    assert delta.order.order_id == 647784973705
    assert delta.flags == 128
    assert delta.sequence == 1170352
    assert delta.ts_event == 1609160400000704060
...
def test_loader_mbp_1_pyo3() -> None:
    ...
    # Assert
    assert len(data) == 2
    assert isinstance(data[0], bomber_pyo3.QuoteTick)
    assert isinstance(data[1], bomber_pyo3.QuoteTick)
    quote = data[0]
    assert quote.instrument_id == bomber_pyo3.InstrumentId.from_str("ESH1.GLBX")
    assert quote.bid_price == bomber_pyo3.Price.from_str("3720.25")
    assert quote.ask_price == bomber_pyo3.Price.from_str("3720.50")
    assert quote.bid_size == bomber_pyo3.Quantity.from_int(24)
    assert quote.ask_size == bomber_pyo3.Quantity.from_int(11)
    assert quote.ts_event == 1609160400006136329
    assert quote.ts_init == 1609160400006136329
...
def test_loader_bbo_1s_pyo3() -> None:
    ...
    quote = data[0]
    assert quote.instrument_id == bomber_pyo3.InstrumentId.from_str("ESH1.GLBX")
    assert quote.bid_price == bomber_pyo3.Price.from_str("3702.25")
    assert quote.ask_price == bomber_pyo3.Price.from_str("3702.75")
    assert quote.bid_size == bomber_pyo3.Quantity.from_int(18)
    assert quote.ask_size == bomber_pyo3.Quantity.from_int(13)
    assert quote.ts_event == 1609113600000000000
    assert quote.ts_init == 1609113600000000000
...
def test_loader_mbp_10_pyo3() -> None:
    ...
    # Assert
    assert len(data) == 2
    assert isinstance(data[0], bomber_pyo3.OrderBookDepth10)
    assert isinstance(data[1], bomber_pyo3.OrderBookDepth10)
    depth = data[0]
    assert depth.instrument_id == bomber_pyo3.InstrumentId.from_str("ESH1.GLBX")
    assert len(depth.bids) == 10
    assert len(depth.asks) == 10
    assert depth.bids[0].price == bomber_pyo3.Price.from_str("3720.25")
    assert depth.bids[0].size == bomber_pyo3.Quantity.from_int(24)
    assert depth.asks[0].price == bomber_pyo3.Price.from_str("3720.50")
    assert depth.asks[0].size == bomber_pyo3.Quantity.from_int(10)
    assert depth.bid_counts == [15, 18, 23, 26, 35, 28, 35, 39, 32, 39]
    assert depth.ask_counts == [8, 24, 25, 17, 19, 33, 40, 38, 35, 26]
    depth = data[1]
    assert depth.instrument_id == bomber_pyo3.InstrumentId.from_str("ESH1.GLBX")
...
def test_loader_tbbo_quotes_pyo3() -> None:
    ...
    assert quote.bid_price == bomber_pyo3.Price.from_str("3720.25")
    assert quote.ask_price == bomber_pyo3.Price.from_str("3720.50")
    assert quote.bid_size == bomber_pyo3.Quantity.from_int(26)
    assert quote.ask_size == bomber_pyo3.Quantity.from_int(7)
    assert quote.ts_event == 1609160400099150057
    assert quote.ts_init == 1609160400099150057
...
def test_loader_trades_pyo3() -> None:
    ...
    # Assert
    assert len(data) == 2
    assert isinstance(data[0], bomber_pyo3.TradeTick)
    assert isinstance(data[1], bomber_pyo3.TradeTick)
    trade = data[0]
    assert trade.instrument_id == bomber_pyo3.InstrumentId.from_str("ESH1.GLBX")
    assert trade.price == bomber_pyo3.Price.from_str("3720.25")
    assert trade.size == bomber_pyo3.Quantity.from_int(5)
    assert trade.aggressor_side == bomber_pyo3.AggressorSide.SELLER
    assert trade.trade_id == bomber_pyo3.TradeId("1170380")
    assert trade.ts_event == 1609160400099150057
    assert trade.ts_init == 1609160400099150057
...
def test_loader_with_ohlcv_1m_pyo3(
    ...
    # Assert
    assert len(data) == 2
    assert isinstance(data[0], bomber_pyo3.Bar)
    assert isinstance(data[1], bomber_pyo3.Bar)
    bar = data[0]
    assert bar.bar_type == bomber_pyo3.BarType.from_str("ESH1.GLBX-1-MINUTE-LAST-EXTERNAL")
    assert bar.open == bomber_pyo3.Price.from_str("372025.00")
...
def test_load_status_pyo3() -> None:
    ...
    # Assert
    assert len(data) == 4
    assert isinstance(data[0], bomber_pyo3.InstrumentStatus)
    assert data[0].action == bomber_pyo3.MarketStatusAction.TRADING
    assert data[0].ts_event == 1609110000000000000
    assert data[0].ts_init == 1609113600000000000
    assert data[0].reason == "Scheduled"
    assert data[0].trading_event is None
...
def test_load_imbalance() -> None:
    ...
    # Assert
    assert len(data) == 2
    assert isinstance(data[0], bomber_pyo3.DatabentoImbalance)
    assert isinstance(data[1], bomber_pyo3.DatabentoImbalance)
...
def test_load_statistics() -> None:
    ...
    instrument_id = bomber_pyo3.InstrumentId.from_str("ESM4.GLBX")
    ...
    # Assert
    assert len(data) == 2
    assert isinstance(data[0], bomber_pyo3.DatabentoStatistics)
    assert isinstance(data[1], bomber_pyo3.DatabentoStatistics)
...
def test_loader_cmbp_1_pyo3() -> None:
    ...
    assert isinstance(data[1], bomber_pyo3.QuoteTick)
    quote = data[0]
    assert quote.instrument_id == bomber_pyo3.InstrumentId.from_str("ESM4.GLBX")
    assert quote.bid_price == bomber_pyo3.Price.from_str("3720.25")
    assert quote.ask_price == bomber_pyo3.Price.from_str("3720.50")
    assert quote.bid_size == bomber_pyo3.Quantity.from_int(24)
    assert quote.ask_size == bomber_pyo3.Quantity.from_int(11)
