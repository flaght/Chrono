...
from bomber.adapters.hyperliquid.execution import HyperliquidExecutionClient
from bomber.adapters.hyperliquid.execution import _is_transport_error
...
def exec_client_builder(
    ...
    def builder(
        ...
        *,
        ...
        monkeypatch.setattr(
            "bomber.adapters.hyperliquid.execution.bomber_pyo3.HyperliquidWebSocketClient",
            lambda *args, **kwargs: next(ws_iter),
        )

        # Skip account registration wait in tests
        monkeypatch.setattr(
            "bomber.adapters.hyperliquid.execution.HyperliquidExecutionClient._await_account_registered",
            AsyncMock(),
        )

        monkeypatch.setattr(
            "bomber.adapters.hyperliquid.execution.bomber_pyo3.hyperliquid_resolve_execution_account_address",
...
async def test_generate_order_status_reports_converts_results(
    ...
    monkeypatch.setattr(
        "bomber.adapters.hyperliquid.execution.OrderStatusReport.from_pyo3",
...
async def test_generate_fill_reports_converts_results(exec_client_builder, monkeypatch):
    ...
    expected_report.venue_order_id = None
    monkeypatch.setattr(
        "bomber.adapters.hyperliquid.execution.FillReport.from_pyo3",
        lambda obj: expected_report,
    )
...
async def test_generate_position_status_reports_converts_results(
    ...
    expected_report = MagicMock()
    monkeypatch.setattr(
        "bomber.adapters.hyperliquid.execution.PositionStatusReport.from_pyo3",
        lambda obj: expected_report,
    )
...
async def test_modify_order_after_partial_fill_sends_remaining_qty(
    ...
    try:
        ...
        assert sent_quantity == bomber_pyo3.Quantity.from_str("0.00060")
...
def _build_status_report_pyo3(
    ...
    return bomber_pyo3.OrderStatusReport(
        ...
        venue_order_id=bomber_pyo3.VenueOrderId(venue_order_id),
        client_order_id=bomber_pyo3.ClientOrderId(client_order_id.value),
        order_side=bomber_pyo3.OrderSide.BUY,
        order_type=bomber_pyo3.OrderType.LIMIT,
        time_in_force=bomber_pyo3.TimeInForce.GTC,
        order_status=order_status,
        quantity=bomber_pyo3.Quantity.from_str(quantity),
        filled_qty=bomber_pyo3.Quantity.from_str("0"),
        price=bomber_pyo3.Price.from_str(price) if price is not None else None,
        ts_accepted=0,
        ts_last=0,
...
async def test_modify_order_cancel_replace_emits_updated_not_canceled(
    ...
    accepted_report = _build_status_report_pyo3(
        ...
        new_voi.value,
        bomber_pyo3.OrderStatus.ACCEPTED,
        price="53893.0",
        quantity="0.00020",
    )
    canceled_report = _build_status_report_pyo3(
        ...
        order.client_order_id,
        old_voi.value,
        bomber_pyo3.OrderStatus.CANCELED,
        price="56730.0",
        quantity="0.00020",
    )
...
def _build_canceled_event_pyo3(
    ...
    return bomber_pyo3.OrderCanceled.from_dict(OrderCanceled.to_dict(event))
...
async def test_submit_order_list_converts_to_pyo3(
    ...
    try:
        ...
        assert isinstance(submitted[0], bomber_pyo3.MarketOrder)
        assert isinstance(submitted[1], bomber_pyo3.LimitOrder)
        assert isinstance(submitted[2], bomber_pyo3.StopMarketOrder)
    finally:
        await client._disconnect()
...
def _build_fill_report_pyo3(
    ...
    return bomber_pyo3.FillReport(
        ...
        last_px=bomber_pyo3.Price.from_str(last_px),
        commission=bomber_pyo3.Money.from_str("0.00 USD"),
...
def _build_order_accepted_pyo3(client, instrument, client_order_id, venue_order_id):
    return bomber_pyo3.OrderAccepted(
