...
import bomber.adapters.hyperliquid.data as hyperliquid_data_module
...
def data_client_builder(
    ...
    def builder(monkeypatch, *, config_kwargs: dict | None = None):
        ...
        monkeypatch.setattr(
            "bomber.adapters.hyperliquid.data.bomber_pyo3.HyperliquidWebSocketClient",
...
async def test_subscribe_order_book_deltas(data_client_builder, monkeypatch):
    ...
    try:
        ...
        # Assert
        expected_id = bomber_pyo3.InstrumentId.from_str("BTC-USD-PERP.HYPERLIQUID")
...
async def test_handle_msg_custom_data_all_mids_forwarded(data_client_builder, monkeypatch):
    ...
    monkeypatch.setattr(
        hyperliquid_data_module.bomber_pyo3,
        "CustomData",
        _FakePyo3CustomData,
    )
