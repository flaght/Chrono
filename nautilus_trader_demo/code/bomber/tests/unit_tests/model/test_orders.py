...
class TestOrders:
    ...
    def test_limit_order_to_own_book_order(self) -> None:
        ...
        # Assert
        assert isinstance(own_order, bomber_pyo3.OwnBookOrder)
        assert own_order.client_order_id == bomber_pyo3.ClientOrderId(
            "O-19700101-000000-000-001-1",
        )
        assert own_order.side == bomber_pyo3.OrderSide.BUY
        assert own_order.price == bomber_pyo3.Price.from_str("1.00000")
        assert own_order.size == bomber_pyo3.Quantity.from_int(100_000)
        assert own_order.time_in_force == bomber_pyo3.TimeInForce.GTC
