...
def test_to_dict():
    ...
    assert bomber_pyo3.Equity.from_dict(result) == _AAPL_EQUITY
    assert result == {
        "type": "Equity",
        "id": "AAPL.XNAS",
        "raw_symbol": "AAPL",
...
def test_pyo3_cython_conversion():
    ...
    equity_pyo3_back = bomber_pyo3.Equity.from_dict(equity_cython_dict)
...
def test_pyo3_cython_conversion_with_fees():
    # Arrange
    equity_pyo3 = bomber_pyo3.Equity(
        instrument_id=InstrumentId.from_str("TEST.XNAS"),
        raw_symbol=bomber_pyo3.Symbol("TEST"),
        isin=None,
        currency=bomber_pyo3.Currency.from_str("USD"),
        price_precision=2,
