...
_XBTUSD_FUTURE_INVERSE = bomber_pyo3.CryptoFuture(
...
_ETHUSD_FUTURE_QUANTO = bomber_pyo3.CryptoFuture(
...
_ETHUSD_FUTURE_USD_STABLE_SETTLED = bomber_pyo3.CryptoFuture(
...
def test_to_dict():
    ...
    assert bomber_pyo3.CryptoFuture.from_dict(result) == _BTCUSDT_FUTURE
    assert result == {
        "type": "CryptoFuture",
...
def test_pyo3_cython_conversion():
    ...
    crypto_future_pyo3_back = bomber_pyo3.CryptoFuture.from_dict(crypto_future_cython_dict)
...
def _crypto_future_with_quote_settlement(
    quote_currency_code: str,
    settlement_currency_code: str,
) -> bomber_pyo3.CryptoFuture:
    return bomber_pyo3.CryptoFuture(
        instrument_id=InstrumentId.from_str("ETHUSD_240329_STABLE.BITMEX"),
        raw_symbol=Symbol("ETHUSD_240329_STABLE"),
        underlying=Pyo3Currency.from_str("ETH"),
