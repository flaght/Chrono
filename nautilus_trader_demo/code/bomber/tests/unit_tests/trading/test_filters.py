...
from bomber.core.datetime import as_utc_index
from bomber.trading.filters import EconomicNewsEventFilter
...
class TestForexSessionFilter:
    @pytest.mark.parametrize(
        ("session", "expected"),
        [
            [bomber_pyo3.ForexSession.SYDNEY, "1970-01-01T10:00:00+10:00"],
            [bomber_pyo3.ForexSession.TOKYO, "1970-01-01T09:00:00+09:00"],
            [bomber_pyo3.ForexSession.LONDON, "1970-01-01T01:00:00+01:00"],
            [bomber_pyo3.ForexSession.NEW_YORK, "1969-12-31T19:00:00-05:00"],
        ],
    )
    def test_local_from_utc_given_various_sessions_returns_expected_datetime(
        ...
        # Arrange, Act
        result = bomber_pyo3.fx_local_from_utc(session, _UNIX_EPOCH)
    ...
    @pytest.mark.parametrize(
        ("session", "expected"),
        [
            [bomber_pyo3.ForexSession.SYDNEY, dt.datetime(1970, 1, 1, 21, 0, tzinfo=dt.UTC)],
            [bomber_pyo3.ForexSession.TOKYO, dt.datetime(1970, 1, 1, 0, 0, tzinfo=dt.UTC)],
            [bomber_pyo3.ForexSession.LONDON, dt.datetime(1970, 1, 1, 7, 0, tzinfo=dt.UTC)],
            [bomber_pyo3.ForexSession.NEW_YORK, dt.datetime(1970, 1, 1, 13, 0, tzinfo=dt.UTC)],
        ],
    )
    def test_next_start_given_various_sessions_returns_expected_datetime(self, session, expected):
        # Arrange, Act
        result = bomber_pyo3.fx_next_start(session, _UNIX_EPOCH)
    ...
    def test_next_start_on_weekend_returns_expected_datetime_monday(self):
        ...
        result = bomber_pyo3.fx_next_start(bomber_pyo3.ForexSession.TOKYO, time_now)
    ...
    @pytest.mark.parametrize(
        ("session", "expected"),
        [
            [bomber_pyo3.ForexSession.SYDNEY, dt.datetime(1969, 12, 31, 21, 0, tzinfo=dt.UTC)],
            [bomber_pyo3.ForexSession.TOKYO, dt.datetime(1970, 1, 1, 0, 0, tzinfo=dt.UTC)],
            [bomber_pyo3.ForexSession.LONDON, dt.datetime(1969, 12, 31, 7, 0, tzinfo=dt.UTC)],
            [bomber_pyo3.ForexSession.NEW_YORK, dt.datetime(1969, 12, 31, 13, 0, tzinfo=dt.UTC)],
        ],
    )
    def test_prev_start_given_various_sessions_returns_expected_datetime(self, session, expected):
        # Arrange, Act
        result = bomber_pyo3.fx_prev_start(session, _UNIX_EPOCH)
    ...
    @pytest.mark.parametrize(
        ("session", "expected"),
        [
            [bomber_pyo3.ForexSession.SYDNEY, dt.datetime(1970, 1, 1, 6, 0, tzinfo=dt.UTC)],
            [bomber_pyo3.ForexSession.TOKYO, dt.datetime(1970, 1, 1, 9, 0, tzinfo=dt.UTC)],
            [bomber_pyo3.ForexSession.LONDON, dt.datetime(1970, 1, 1, 15, 0, tzinfo=dt.UTC)],
            [bomber_pyo3.ForexSession.NEW_YORK, dt.datetime(1970, 1, 1, 22, 0, tzinfo=dt.UTC)],
        ],
    )
    def test_next_end_given_various_sessions_returns_expected_datetime(self, session, expected):
        # Arrange, Act
        result = bomber_pyo3.fx_next_end(session, _UNIX_EPOCH)
    ...
    @pytest.mark.parametrize(
        ("session", "expected"),
        [
            [bomber_pyo3.ForexSession.SYDNEY, dt.datetime(1969, 12, 31, 6, 0, tzinfo=dt.UTC)],
            [bomber_pyo3.ForexSession.TOKYO, dt.datetime(1969, 12, 31, 9, 0, tzinfo=dt.UTC)],
            [bomber_pyo3.ForexSession.LONDON, dt.datetime(1969, 12, 31, 15, 0, tzinfo=dt.UTC)],
            [
                bomber_pyo3.ForexSession.NEW_YORK,
    ...
    def test_prev_end_given_various_sessions_returns_expected_datetime(self, session, expected):
        # Arrange, Act
        result = bomber_pyo3.fx_prev_end(session, _UNIX_EPOCH)
