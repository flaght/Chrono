...
# -------------------------------------------------------------------------------------------------
"""
Tests to verify that bomber_pyo3.pyi type stubs match the actual runtime bindings.

This file validates the default legacy ``bomber_pyo3`` extension surface. It does not validate
...
STUB_FILE = PACKAGE_ROOT / "bomber" / "core" / "bomber_pyo3.pyi"
...
def get_module_classes() -> set[str]:
    """
    Get all class names available directly from bomber_pyo3.
    """
    return {
        name
        for name in dir(bomber_pyo3)
        if not name.startswith("_") and isinstance(getattr(bomber_pyo3, name), type)
    }
...
class TestEnumVariants:
    ...
    def test_enum_variants_exist(self, class_name: str, info: dict) -> None:
        ...
        """
        cls = getattr(bomber_pyo3, class_name)
...
class TestStubClassesExist:
    ...
    def test_stub_classes_in_module(self) -> None:
        """
        Verify all stub classes are available in bomber_pyo3.
        """
        missing = [name for name in STUB_CLASSES if not hasattr(bomber_pyo3, name)]
