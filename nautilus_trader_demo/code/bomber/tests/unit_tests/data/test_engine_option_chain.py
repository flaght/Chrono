...
from bomber.data.messages import SubscribeOptionChain
from bomber.data.messages import SubscribeOptionGreeks
from bomber.data.messages import UnsubscribeOptionChain
...
class TestOptionChainEngine:
    ...
    def setup(self):
        ...
        self.series_id = bomber_pyo3.OptionSeriesId(
            "OPRA",
            "AAPL",
            "USD",
            EXPIRY_NS,
    ...
    def _subscribe_and_bootstrap(self, series_id, strike_range=_UNSET, snapshot_interval_ms=None):
        ...
        if strike_range is self._UNSET:
            strike_range = bomber_pyo3.StrikeRange.atm_relative(100, 100)
    ...
    def test_subscribe_option_chain_fixed_strike_range_skips_bootstrap(self):
        # Arrange
        strike_range = bomber_pyo3.StrikeRange.fixed(
            [
                bomber_pyo3.Price.from_str("150.00"),
                bomber_pyo3.Price.from_str("155.00"),
            ],
        )
        command = SubscribeOptionChain(
            series_id=self.series_id,
    ...
    def test_subscribe_option_chain_atm_relative_creates_pending_request(self):
        # Arrange
        strike_range = bomber_pyo3.StrikeRange.atm_relative(2, 2)
        command = SubscribeOptionChain(
            series_id=self.series_id,
            strike_range=strike_range,
            snapshot_interval_ms=None,
    ...
    def test_option_chain_does_not_mix_underlyings_same_expiry(self):
        # Arrange
        msft_series = bomber_pyo3.OptionSeriesId(
            "OPRA",
            "MSFT",
            "USD",
            EXPIRY_NS,
...
class TestStrikeRangeKind:
    def test_fixed_kind(self):
        strike_range = bomber_pyo3.StrikeRange.fixed(
            [bomber_pyo3.Price.from_str("100.00")],
        )
    ...
    def test_atm_percent_kind(self):
        strike_range = bomber_pyo3.StrikeRange.atm_percent(0.05)
