...
RUST_INCLUDES = ["bomber/core/includes"]
...
def _build_extensions() -> list[Extension]:
    ...
    return [
        ...
        for pyx in itertools.chain(Path("bomber").rglob("*.pyx"))
    ]
...
def _copy_rust_dylibs_to_project() -> None:
    ...
    src = Path(CARGO_TARGET_DIR) / f"{RUST_LIB_PFX}bomber_pyo3.{RUST_DYLIB_EXT}"
    dst = Path("bomber/core") / f"bomber_pyo3{ext_suffix}"
...
def _strip_unneeded_symbols() -> None:
    try:
        ...
        for so in itertools.chain(Path("bomber").rglob("*.so")):
...
def _ensure_local_editable_pth() -> None:
    ...
    # gap where a bare `make build` leaves the venv unable to resolve `bomber`
    ...
    pth_file = site_packages / "bomber-local.pth"
