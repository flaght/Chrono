...
from bomber.core.message import Document
...
from bomber.model.enums import contingency_type_to_str
from bomber.model.enums import liquidity_side_to_str
from bomber.model.enums import order_side_to_str
from bomber.model.enums import order_status_to_str
from bomber.model.enums import order_type_to_str
from bomber.model.enums import position_side_to_str
from bomber.model.enums import time_in_force_to_str
from bomber.model.enums import trailing_offset_type_to_str
from bomber.model.enums import trigger_type_to_str
from bomber.model.functions import contingency_type_from_pyo3
from bomber.model.functions import contingency_type_to_pyo3
from bomber.model.functions import liquidity_side_from_pyo3
from bomber.model.functions import order_side_from_pyo3
from bomber.model.functions import order_side_to_pyo3
from bomber.model.functions import order_status_from_pyo3
from bomber.model.functions import order_status_to_pyo3
from bomber.model.functions import order_type_from_pyo3
from bomber.model.functions import order_type_to_pyo3
from bomber.model.functions import position_side_from_pyo3
from bomber.model.functions import time_in_force_from_pyo3
...
class OrderStatusReport(ExecutionReport):
    ...
    def to_pyo3(self) -> bomber_pyo3.OrderStatusReport:
        return bomber_pyo3.OrderStatusReport(
            account_id=bomber_pyo3.AccountId(self.account_id.value),
            instrument_id=bomber_pyo3.InstrumentId.from_str(self.instrument_id.value),
            venue_order_id=bomber_pyo3.VenueOrderId(self.venue_order_id.value),
            ...
            quantity=bomber_pyo3.Quantity.from_str(str(self.quantity)),
            filled_qty=bomber_pyo3.Quantity.from_str(str(self.filled_qty)),
            ts_accepted=self.ts_accepted,
            ts_last=self.ts_last,
            ts_init=self.ts_init,
            client_order_id=(
                bomber_pyo3.ClientOrderId(self.client_order_id.value)
                if self.client_order_id
                else None
            ),
            order_list_id=(
                bomber_pyo3.OrderListId(self.order_list_id.value) if self.order_list_id else None
            ),
            venue_position_id=(
                bomber_pyo3.PositionId(self.venue_position_id.value)
                if self.venue_position_id
                else None
            ),
            linked_order_ids=(
                [bomber_pyo3.ClientOrderId(oid.value) for oid in self.linked_order_ids]
                if self.linked_order_ids
                else None
            ),
            parent_order_id=(
                bomber_pyo3.ClientOrderId(self.parent_order_id.value)
            ...
            price=bomber_pyo3.Price.from_str(str(self.price)) if self.price else None,
            trigger_price=(
                bomber_pyo3.Price.from_str(str(self.trigger_price))
            ...
            avg_px=self.avg_px,
            display_qty=(
                bomber_pyo3.Quantity.from_str(str(self.display_qty)) if self.display_qty else None
            ),
            post_only=self.post_only,
            reduce_only=self.reduce_only,
            cancel_reason=self.cancel_reason,
            ts_triggered=self.ts_triggered,
            report_id=bomber_pyo3.UUID4.from_str(self.id.value),
        )

    @staticmethod
    def from_pyo3(pyo3_report: bomber_pyo3.OrderStatusReport) -> OrderStatusReport:
...
class FillReport(ExecutionReport):
    ...
    def to_pyo3(self) -> bomber_pyo3.FillReport:
        return bomber_pyo3.FillReport(
            account_id=bomber_pyo3.AccountId(self.account_id.value),
            instrument_id=bomber_pyo3.InstrumentId.from_str(self.instrument_id.value),
            venue_order_id=bomber_pyo3.VenueOrderId(self.venue_order_id.value),
            trade_id=bomber_pyo3.TradeId(self.trade_id.value),
            order_side=order_side_to_pyo3(self.order_side),
            last_qty=bomber_pyo3.Quantity.from_str(str(self.last_qty)),
            last_px=bomber_pyo3.Price.from_str(str(self.last_px)),
            commission=bomber_pyo3.Money.from_str(str(self.commission)),
            liquidity_side=getattr(bomber_pyo3.LiquiditySide, self.liquidity_side.name),
            ts_event=self.ts_event,
            ts_init=self.ts_init,
            client_order_id=(
                bomber_pyo3.ClientOrderId(self.client_order_id.value)
    ...
    @staticmethod
    def from_pyo3(pyo3_report: bomber_pyo3.FillReport) -> FillReport:
        return FillReport(
            account_id=AccountId(pyo3_report.account_id.value),
            instrument_id=InstrumentId.from_str(pyo3_report.instrument_id.value),
            venue_order_id=VenueOrderId(pyo3_report.venue_order_id.value),
...
class PositionStatusReport(ExecutionReport):
    ...
    def to_pyo3(self) -> bomber_pyo3.PositionStatusReport:
        return bomber_pyo3.PositionStatusReport(
            account_id=bomber_pyo3.AccountId(self.account_id.value),
            instrument_id=bomber_pyo3.InstrumentId.from_str(self.instrument_id.value),
            position_side=getattr(bomber_pyo3.PositionSide, self.position_side.name),
            quantity=bomber_pyo3.Quantity.from_str(str(self.quantity)),
            ts_last=self.ts_last,
            ts_init=self.ts_init,
            report_id=bomber_pyo3.UUID4.from_str(self.id.value),
    ...
    @staticmethod
    def from_pyo3(pyo3_report: bomber_pyo3.PositionStatusReport) -> PositionStatusReport:
        return PositionStatusReport(
            account_id=AccountId(pyo3_report.account_id.value),
            instrument_id=InstrumentId.from_str(pyo3_report.instrument_id.value),
...
class ExecutionMassStatus(Document):
    ...
    def to_pyo3(self) -> bomber_pyo3.ExecutionMassStatus:
        pyo3_mass_status = bomber_pyo3.ExecutionMassStatus(
            client_id=bomber_pyo3.ClientId(self.client_id.value),
            account_id=bomber_pyo3.AccountId(self.account_id.value),
            venue=bomber_pyo3.Venue(self._get_venue_value()),
            ts_init=self.ts_init,
            report_id=bomber_pyo3.UUID4.from_str(self.id.value),
        )
