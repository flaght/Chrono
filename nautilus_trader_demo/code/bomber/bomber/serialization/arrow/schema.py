...
def infer_dtype(dtype_str: str) -> pa.DataType:
    if dtype_str.startswith("FixedSizeBinary"):
        return pa.binary(bomber_pyo3.PRECISION_BYTES)
    else:
...
NAUTILUS_ARROW_SCHEMA = {
    OrderBookDelta: pa.schema(
        [
            ...
            for k, v in bomber_pyo3.OrderBookDelta.get_fields().items()
        ],
    ),
    OrderBookDepth10: pa.schema(
        [
            pa.field(k, infer_dtype(v), False)
            for k, v in bomber_pyo3.OrderBookDepth10.get_fields().items()
        ],
    ),
    QuoteTick: pa.schema(
        [
            pa.field(k, infer_dtype(v), False)
            for k, v in bomber_pyo3.QuoteTick.get_fields().items()
        ],
    ),
    TradeTick: pa.schema(
        [
            pa.field(k, infer_dtype(v), False)
            for k, v in bomber_pyo3.TradeTick.get_fields().items()
        ],
    ),
    Bar: pa.schema(
        [pa.field(k, infer_dtype(v), False) for k, v in bomber_pyo3.Bar.get_fields().items()],
    ),
    MarkPriceUpdate: pa.schema(
        [
            pa.field(k, infer_dtype(v), False)
            for k, v in bomber_pyo3.MarkPriceUpdate.get_fields().items()
        ],
    ),
    IndexPriceUpdate: pa.schema(
        [
            pa.field(k, infer_dtype(v), False)
            for k, v in bomber_pyo3.IndexPriceUpdate.get_fields().items()
        ],
    ),
    ...
    #     [
    #         pa.field(k, infer_dtype(v), False)
    #         for k, v in bomber_pyo3.InstrumentClose.get_fields().items()
    #     ],
    # ),
