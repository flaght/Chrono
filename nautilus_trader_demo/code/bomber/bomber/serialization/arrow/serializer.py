...
from bomber.common.messages import ComponentStateChanged
from bomber.common.messages import ShutdownSystem
from bomber.common.messages import TradingStateChanged
from bomber.core import bomber_pyo3
from bomber.core.correctness import PyCondition
from bomber.core.data import Data
from bomber.core.message import Event
...
from bomber.model.events import PositionEvent
from bomber.model.instruments import Instrument
from bomber.persistence.wranglers_v2 import BarDataWranglerV2
from bomber.persistence.wranglers_v2 import OrderBookDeltaDataWranglerV2
from bomber.persistence.wranglers_v2 import OrderBookDepth10DataWranglerV2
from bomber.persistence.wranglers_v2 import QuoteTickDataWranglerV2
from bomber.persistence.wranglers_v2 import TradeTickDataWranglerV2
from bomber.serialization.arrow.implementations import account_state
from bomber.serialization.arrow.implementations import component_commands
from bomber.serialization.arrow.implementations import component_events
from bomber.serialization.arrow.implementations import funding_rate_update
from bomber.serialization.arrow.implementations import instruments
from bomber.serialization.arrow.implementations import order_events
from bomber.serialization.arrow.implementations import position_events
from bomber.serialization.arrow.schema import NAUTILUS_ARROW_SCHEMA
...
class ArrowSerializer:
    ...
    def rust_defined_to_record_batch(  # noqa: C901 (too complex)
        ...
        match data_cls:
            case bomber_pyo3.OrderBookDelta:
                batch_bytes = bomber_pyo3.book_deltas_to_arrow_record_batch_bytes(
                    data,
                )
            case bomber_pyo3.OrderBookDepth10:
                batch_bytes = bomber_pyo3.book_depth10_to_arrow_record_batch_bytes(
                    data,
                )
            case bomber_pyo3.QuoteTick:
                batch_bytes = bomber_pyo3.quotes_to_arrow_record_batch_bytes(data)
            case bomber_pyo3.TradeTick:
                batch_bytes = bomber_pyo3.trades_to_arrow_record_batch_bytes(data)
            case bomber_pyo3.Bar:
                batch_bytes = bomber_pyo3.bars_to_arrow_record_batch_bytes(data)
            case bomber_pyo3.OptionGreeks:
                batch_bytes = bomber_pyo3.option_greeks_to_arrow_record_batch_bytes(data)
            case _:
                if data_cls in (OrderBookDelta, OrderBookDeltas):
                    ...
                    batch_bytes = bomber_pyo3.quotes_to_arrow_record_batch_bytes(
                        pyo3_quotes,
                    )
                elif data_cls == TradeTick:
                    pyo3_trades = TradeTick.to_pyo3_list(data)
                    batch_bytes = bomber_pyo3.trades_to_arrow_record_batch_bytes(
                        pyo3_trades,
                    )
                elif data_cls == Bar:
                    pyo3_bars = Bar.to_pyo3_list(data)
                    batch_bytes = bomber_pyo3.bars_to_arrow_record_batch_bytes(pyo3_bars)
                elif data_cls == MarkPriceUpdate:
                    pyo3_mark_prices = MarkPriceUpdate.to_pyo3_list(data)
                    batch_bytes = bomber_pyo3.mark_prices_to_arrow_record_batch_bytes(
                        pyo3_mark_prices,
                    )
                elif data_cls == IndexPriceUpdate:
                    pyo3_index_prices = IndexPriceUpdate.to_pyo3_list(data)
                    batch_bytes = bomber_pyo3.index_prices_to_arrow_record_batch_bytes(
                        pyo3_index_prices,
                    )
                elif data_cls == InstrumentClose:
                    pyo3_instrument_closes = InstrumentClose.to_pyo3_list(data)
                    batch_bytes = bomber_pyo3.instrument_closes_to_arrow_record_batch_bytes(
                        pyo3_instrument_closes,
                    )
                elif data_cls == OptionGreeks:
                    pyo3_option_greeks = [item.to_pyo3() for item in data]
                    batch_bytes = bomber_pyo3.option_greeks_to_arrow_record_batch_bytes(
                        pyo3_option_greeks,
                    )
                elif data_cls == OrderBookDepth10:
                    data = [
                        bomber_pyo3.OrderBookDepth10.from_dict(OrderBookDepth10.to_dict(item))
                        for item in data
                    ]
                    batch_bytes = bomber_pyo3.book_depth10_to_arrow_record_batch_bytes(
                        data,
    ...
    def serialize(
        ...
        if delegate is None:
            ...
            raise TypeError(
                ...
                f"serialization method via `bomber.serialization.arrow.serializer.register_arrow()`",
            )
    ...
    def deserialize(data_cls: type, batch: pa.RecordBatch | pa.Table) -> list[Data | Event]:
        ...
        if delegate is None:
            ...
            raise TypeError(
                ...
                f"deserialization method via `bomber.serialization.arrow.serializer.register_arrow()`",
            )
...
def _instrument_status_encoder(data: list) -> pa.RecordBatch:
    if not isinstance(data, list):
        data = [data]
    batch_bytes = bomber_pyo3.instrument_status_to_arrow_record_batch_bytes(data)
    reader = pa.ipc.open_stream(BytesIO(batch_bytes))
...
def _instrument_status_decoder(table) -> list:
    ...
    return bomber_pyo3.instrument_status_from_arrow_record_batch_bytes(ipc_bytes)
...
def _option_greeks_decoder(table) -> list:
    ...
    writer.close()
    ipc_bytes = sink.getvalue().to_pybytes()
    return bomber_pyo3.option_greeks_from_arrow_record_batch_bytes(ipc_bytes)
