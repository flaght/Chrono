...
from bomber.model.currencies import register_currency
...
def create_inferred_order_filled_event(
    ...
    pyo3_trade_id = bomber_pyo3.create_inferred_reconciliation_trade_id(
        bomber_pyo3.AccountId(report.account_id.value),
        bomber_pyo3.InstrumentId.from_str(report.instrument_id.value),
        bomber_pyo3.ClientOrderId(order.client_order_id.value),
        bomber_pyo3.VenueOrderId(report.venue_order_id.value) if report.venue_order_id else None,
        bomber_pyo3.OrderSide(order.side.name),
        bomber_pyo3.OrderType(order.order_type.name),
        bomber_pyo3.Quantity.from_str(str(report.filled_qty)),
        bomber_pyo3.Quantity.from_str(str(last_qty)),
        bomber_pyo3.Price.from_str(str(last_px)),
        bomber_pyo3.PositionId(position_id.value),
        report.ts_last,
    )
...
def calculate_reconciliation_price(
    ...
    """
    result = bomber_pyo3.calculate_reconciliation_price(
...
def adjust_fills_for_partial_window(
    ...
    for instrument, pyo3_instrument in zip(instruments, pyo3_instruments, strict=False):
        ...
        pyo3_orders, pyo3_fills = bomber_pyo3.process_mass_status_for_reconciliation(
            pyo3_mass_status,
            pyo3_instrument,
        )
