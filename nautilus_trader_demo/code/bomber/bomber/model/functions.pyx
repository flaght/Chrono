...
from bomber.core.rust.model cimport account_type_from_cstr
from bomber.core.rust.model cimport account_type_to_cstr
from bomber.core.rust.model cimport aggregation_source_from_cstr
from bomber.core.rust.model cimport aggregation_source_to_cstr
from bomber.core.rust.model cimport aggressor_side_from_cstr
from bomber.core.rust.model cimport aggressor_side_to_cstr
from bomber.core.rust.model cimport asset_class_from_cstr
from bomber.core.rust.model cimport asset_class_to_cstr
from bomber.core.rust.model cimport bar_aggregation_from_cstr
from bomber.core.rust.model cimport bar_aggregation_to_cstr
from bomber.core.rust.model cimport book_action_from_cstr
from bomber.core.rust.model cimport book_action_to_cstr
from bomber.core.rust.model cimport book_type_from_cstr
from bomber.core.rust.model cimport book_type_to_cstr
from bomber.core.rust.model cimport contingency_type_from_cstr
from bomber.core.rust.model cimport contingency_type_to_cstr
from bomber.core.rust.model cimport currency_type_from_cstr
from bomber.core.rust.model cimport currency_type_to_cstr
from bomber.core.rust.model cimport instrument_class_from_cstr
from bomber.core.rust.model cimport instrument_class_to_cstr
from bomber.core.rust.model cimport instrument_close_type_from_cstr
from bomber.core.rust.model cimport instrument_close_type_to_cstr
from bomber.core.rust.model cimport liquidity_side_from_cstr
from bomber.core.rust.model cimport liquidity_side_to_cstr
from bomber.core.rust.model cimport market_status_action_from_cstr
from bomber.core.rust.model cimport market_status_action_to_cstr
from bomber.core.rust.model cimport market_status_from_cstr
from bomber.core.rust.model cimport market_status_to_cstr
from bomber.core.rust.model cimport oms_type_from_cstr
from bomber.core.rust.model cimport oms_type_to_cstr
from bomber.core.rust.model cimport option_kind_from_cstr
from bomber.core.rust.model cimport option_kind_to_cstr
from bomber.core.rust.model cimport order_side_from_cstr
from bomber.core.rust.model cimport order_side_to_cstr
from bomber.core.rust.model cimport order_status_from_cstr
from bomber.core.rust.model cimport order_status_to_cstr
from bomber.core.rust.model cimport order_type_from_cstr
from bomber.core.rust.model cimport order_type_to_cstr
from bomber.core.rust.model cimport oto_trigger_mode_from_cstr
from bomber.core.rust.model cimport oto_trigger_mode_to_cstr
from bomber.core.rust.model cimport position_adjustment_type_from_cstr
from bomber.core.rust.model cimport position_adjustment_type_to_cstr
from bomber.core.rust.model cimport position_side_from_cstr
from bomber.core.rust.model cimport position_side_to_cstr
from bomber.core.rust.model cimport price_type_from_cstr
from bomber.core.rust.model cimport price_type_to_cstr
from bomber.core.rust.model cimport record_flag_from_cstr
from bomber.core.rust.model cimport record_flag_to_cstr
from bomber.core.rust.model cimport time_in_force_from_cstr
from bomber.core.rust.model cimport time_in_force_to_cstr
from bomber.core.rust.model cimport trading_state_from_cstr
from bomber.core.rust.model cimport trading_state_to_cstr
from bomber.core.rust.model cimport trailing_offset_type_from_cstr
from bomber.core.rust.model cimport trailing_offset_type_to_cstr
from bomber.core.rust.model cimport trigger_type_from_cstr
from bomber.core.rust.model cimport trigger_type_to_cstr
...
cpdef account_type_to_pyo3(AccountType value):
    if value == AccountType.CASH:
        return bomber_pyo3.AccountType.CASH
    if value == AccountType.MARGIN:
        return bomber_pyo3.AccountType.MARGIN
    if value == AccountType.BETTING:
        return bomber_pyo3.AccountType.BETTING
...
cpdef order_side_to_pyo3(OrderSide value):
    if value == OrderSide.BUY:
        return bomber_pyo3.OrderSide.BUY
    if value == OrderSide.SELL:
        return bomber_pyo3.OrderSide.SELL
    if value == OrderSide.NO_ORDER_SIDE:
        return bomber_pyo3.OrderSide.NO_ORDER_SIDE
...
cpdef order_type_to_pyo3(OrderType value):
    if value == OrderType.MARKET:
        return bomber_pyo3.OrderType.MARKET
    if value == OrderType.LIMIT:
        return bomber_pyo3.OrderType.LIMIT
    if value == OrderType.STOP_MARKET:
        return bomber_pyo3.OrderType.STOP_MARKET
    if value == OrderType.STOP_LIMIT:
        return bomber_pyo3.OrderType.STOP_LIMIT
    if value == OrderType.MARKET_TO_LIMIT:
        return bomber_pyo3.OrderType.MARKET_TO_LIMIT
    if value == OrderType.MARKET_IF_TOUCHED:
        return bomber_pyo3.OrderType.MARKET_IF_TOUCHED
    if value == OrderType.LIMIT_IF_TOUCHED:
        return bomber_pyo3.OrderType.LIMIT_IF_TOUCHED
    if value == OrderType.TRAILING_STOP_MARKET:
        return bomber_pyo3.OrderType.TRAILING_STOP_MARKET
    if value == OrderType.TRAILING_STOP_LIMIT:
        return bomber_pyo3.OrderType.TRAILING_STOP_LIMIT
...
cpdef order_status_to_pyo3(OrderStatus value):
    if value == OrderStatus.INITIALIZED:
        return bomber_pyo3.OrderStatus.INITIALIZED
    if value == OrderStatus.DENIED:
        return bomber_pyo3.OrderStatus.DENIED
    if value == OrderStatus.EMULATED:
        return bomber_pyo3.OrderStatus.EMULATED
    if value == OrderStatus.RELEASED:
        return bomber_pyo3.OrderStatus.RELEASED
    if value == OrderStatus.SUBMITTED:
        return bomber_pyo3.OrderStatus.SUBMITTED
    if value == OrderStatus.ACCEPTED:
        return bomber_pyo3.OrderStatus.ACCEPTED
    if value == OrderStatus.REJECTED:
        return bomber_pyo3.OrderStatus.REJECTED
    if value == OrderStatus.CANCELED:
        return bomber_pyo3.OrderStatus.CANCELED
    if value == OrderStatus.EXPIRED:
        return bomber_pyo3.OrderStatus.EXPIRED
    if value == OrderStatus.TRIGGERED:
        return bomber_pyo3.OrderStatus.TRIGGERED
    if value == OrderStatus.PENDING_UPDATE:
        return bomber_pyo3.OrderStatus.PENDING_UPDATE
    if value == OrderStatus.PENDING_CANCEL:
        return bomber_pyo3.OrderStatus.PENDING_CANCEL
    if value == OrderStatus.PARTIALLY_FILLED:
        return bomber_pyo3.OrderStatus.PARTIALLY_FILLED
    if value == OrderStatus.FILLED:
        return bomber_pyo3.OrderStatus.FILLED
...
cpdef time_in_force_to_pyo3(TimeInForce value):
    if value == TimeInForce.GTC:
        return bomber_pyo3.TimeInForce.GTC
    if value == TimeInForce.IOC:
        return bomber_pyo3.TimeInForce.IOC
    if value == TimeInForce.FOK:
        return bomber_pyo3.TimeInForce.FOK
    if value == TimeInForce.GTD:
        return bomber_pyo3.TimeInForce.GTD
    if value == TimeInForce.DAY:
        return bomber_pyo3.TimeInForce.DAY
    if value == TimeInForce.AT_THE_OPEN:
        return bomber_pyo3.TimeInForce.AT_THE_OPEN
    if value == TimeInForce.AT_THE_CLOSE:
        return bomber_pyo3.TimeInForce.AT_THE_CLOSE
...
cpdef trigger_type_to_pyo3(TriggerType value):
    if value == TriggerType.NO_TRIGGER:
        return bomber_pyo3.TriggerType.NO_TRIGGER
    if value == TriggerType.DEFAULT:
        return bomber_pyo3.TriggerType.DEFAULT
    if value == TriggerType.LAST_PRICE:
        return bomber_pyo3.TriggerType.LAST_PRICE
    if value == TriggerType.MARK_PRICE:
        return bomber_pyo3.TriggerType.MARK_PRICE
    if value == TriggerType.INDEX_PRICE:
        return bomber_pyo3.TriggerType.INDEX_PRICE
    if value == TriggerType.BID_ASK:
        return bomber_pyo3.TriggerType.BID_ASK
    if value == TriggerType.DOUBLE_LAST:
        return bomber_pyo3.TriggerType.DOUBLE_LAST
    if value == TriggerType.DOUBLE_BID_ASK:
        return bomber_pyo3.TriggerType.DOUBLE_BID_ASK
...
cpdef trailing_offset_type_to_pyo3(TrailingOffsetType value):
    if value == TrailingOffsetType.NO_TRAILING_OFFSET:
        return bomber_pyo3.TrailingOffsetType.NO_TRAILING_OFFSET
    if value == TrailingOffsetType.PRICE:
        return bomber_pyo3.TrailingOffsetType.PRICE
    if value == TrailingOffsetType.BASIS_POINTS:
        return bomber_pyo3.TrailingOffsetType.BASIS_POINTS
    if value == TrailingOffsetType.TICKS:
        return bomber_pyo3.TrailingOffsetType.TICKS
    if value == TrailingOffsetType.PRICE_TIER:
        return bomber_pyo3.TrailingOffsetType.PRICE_TIER
...
cpdef contingency_type_to_pyo3(ContingencyType value):
    if value == ContingencyType.NO_CONTINGENCY:
        return bomber_pyo3.ContingencyType.NO_CONTINGENCY
    if value == ContingencyType.OCO:
        return bomber_pyo3.ContingencyType.OCO
    if value == ContingencyType.OTO:
        return bomber_pyo3.ContingencyType.OTO
    if value == ContingencyType.OUO:
        return bomber_pyo3.ContingencyType.OUO

    raise ValueError(f"Unsupported `ContingencyType`, was '{contingency_type_to_str(value)}'")


cpdef OrderSide order_side_from_pyo3(value: bomber_pyo3.OrderSide):
    if value == bomber_pyo3.OrderSide.BUY:
        return OrderSide.BUY
    if value == bomber_pyo3.OrderSide.SELL:
        return OrderSide.SELL
    if value == bomber_pyo3.OrderSide.NO_ORDER_SIDE:
        return OrderSide.NO_ORDER_SIDE

    raise ValueError(f"Unsupported `OrderSide`, was '{value}'")


cpdef OrderType order_type_from_pyo3(value: bomber_pyo3.OrderType):
    if value == bomber_pyo3.OrderType.MARKET:
        return OrderType.MARKET
    if value == bomber_pyo3.OrderType.LIMIT:
        return OrderType.LIMIT
    if value == bomber_pyo3.OrderType.STOP_MARKET:
        return OrderType.STOP_MARKET
    if value == bomber_pyo3.OrderType.STOP_LIMIT:
        return OrderType.STOP_LIMIT
    if value == bomber_pyo3.OrderType.MARKET_TO_LIMIT:
        return OrderType.MARKET_TO_LIMIT
    if value == bomber_pyo3.OrderType.MARKET_IF_TOUCHED:
        return OrderType.MARKET_IF_TOUCHED
    if value == bomber_pyo3.OrderType.LIMIT_IF_TOUCHED:
        return OrderType.LIMIT_IF_TOUCHED
    if value == bomber_pyo3.OrderType.TRAILING_STOP_MARKET:
        return OrderType.TRAILING_STOP_MARKET
    if value == bomber_pyo3.OrderType.TRAILING_STOP_LIMIT:
        return OrderType.TRAILING_STOP_LIMIT

    raise ValueError(f"Unsupported `OrderType`, was '{value}'")


cpdef OrderStatus order_status_from_pyo3(value: bomber_pyo3.OrderStatus):
    if value == bomber_pyo3.OrderStatus.INITIALIZED:
        return OrderStatus.INITIALIZED
    if value == bomber_pyo3.OrderStatus.DENIED:
        return OrderStatus.DENIED
    if value == bomber_pyo3.OrderStatus.EMULATED:
        return OrderStatus.EMULATED
    if value == bomber_pyo3.OrderStatus.RELEASED:
        return OrderStatus.RELEASED
    if value == bomber_pyo3.OrderStatus.SUBMITTED:
        return OrderStatus.SUBMITTED
    if value == bomber_pyo3.OrderStatus.ACCEPTED:
        return OrderStatus.ACCEPTED
    if value == bomber_pyo3.OrderStatus.REJECTED:
        return OrderStatus.REJECTED
    if value == bomber_pyo3.OrderStatus.CANCELED:
        return OrderStatus.CANCELED
    if value == bomber_pyo3.OrderStatus.EXPIRED:
        return OrderStatus.EXPIRED
    if value == bomber_pyo3.OrderStatus.TRIGGERED:
        return OrderStatus.TRIGGERED
    if value == bomber_pyo3.OrderStatus.PENDING_UPDATE:
        return OrderStatus.PENDING_UPDATE
    if value == bomber_pyo3.OrderStatus.PENDING_CANCEL:
        return OrderStatus.PENDING_CANCEL
    if value == bomber_pyo3.OrderStatus.PARTIALLY_FILLED:
        return OrderStatus.PARTIALLY_FILLED
    if value == bomber_pyo3.OrderStatus.FILLED:
        return OrderStatus.FILLED

    raise ValueError(f"Unsupported `OrderStatus`, was '{value}'")


cpdef TimeInForce time_in_force_from_pyo3(value: bomber_pyo3.TimeInForce):
    if value == bomber_pyo3.TimeInForce.GTC:
        return TimeInForce.GTC
    if value == bomber_pyo3.TimeInForce.IOC:
        return TimeInForce.IOC
    if value == bomber_pyo3.TimeInForce.FOK:
        return TimeInForce.FOK
    if value == bomber_pyo3.TimeInForce.GTD:
        return TimeInForce.GTD
    if value == bomber_pyo3.TimeInForce.DAY:
        return TimeInForce.DAY
    if value == bomber_pyo3.TimeInForce.AT_THE_OPEN:
        return TimeInForce.AT_THE_OPEN
    if value == bomber_pyo3.TimeInForce.AT_THE_CLOSE:
        return TimeInForce.AT_THE_CLOSE

    raise ValueError(f"Unsupported `TimeInForce`, was '{value}'")


cpdef LiquiditySide liquidity_side_from_pyo3(value: bomber_pyo3.LiquiditySide):
    if value == bomber_pyo3.LiquiditySide.NO_LIQUIDITY_SIDE:
        return LiquiditySide.NO_LIQUIDITY_SIDE
    if value == bomber_pyo3.LiquiditySide.MAKER:
        return LiquiditySide.MAKER
    if value == bomber_pyo3.LiquiditySide.TAKER:
        return LiquiditySide.TAKER

    raise ValueError(f"Unsupported `LiquiditySide`, was '{value}'")


cpdef ContingencyType contingency_type_from_pyo3(value: bomber_pyo3.ContingencyType):
    if value == bomber_pyo3.ContingencyType.NO_CONTINGENCY:
        return ContingencyType.NO_CONTINGENCY
    if value == bomber_pyo3.ContingencyType.OCO:
        return ContingencyType.OCO
    if value == bomber_pyo3.ContingencyType.OTO:
        return ContingencyType.OTO
    if value == bomber_pyo3.ContingencyType.OUO:
        return ContingencyType.OUO

    raise ValueError(f"Unsupported `ContingencyType`, was '{value}'")


cpdef PositionSide position_side_from_pyo3(value: bomber_pyo3.PositionSide):
    if value == bomber_pyo3.PositionSide.FLAT:
        return PositionSide.FLAT
    if value == bomber_pyo3.PositionSide.LONG:
        return PositionSide.LONG
    if value == bomber_pyo3.PositionSide.SHORT:
