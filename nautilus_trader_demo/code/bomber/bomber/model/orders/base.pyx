...
from bomber.core.rust.model cimport FIXED_SCALAR
...
from bomber.model.functions cimport order_side_to_pyo3
from bomber.model.functions cimport order_side_to_str
from bomber.model.functions cimport order_status_to_pyo3
from bomber.model.functions cimport order_type_to_pyo3
from bomber.model.functions cimport order_type_to_str
from bomber.model.functions cimport position_side_to_str
from bomber.model.functions cimport time_in_force_to_pyo3
...
cdef class Order:
    ...
    def to_own_book_order(self) -> bomber_pyo3.OwnBookOrder:
        """
        Returns an own/user order representation of this order.

        Returns
        -------
        bomber_pyo3.OwnBookOrder

        """
        if not self.has_price_c():
            raise TypeError(f"Cannot initialize {self.type_string_c()} order as `bomber_pyo3.OwnBookOrder`, no price")

        cdef Price price = self.price
        return bomber_pyo3.OwnBookOrder(
            trader_id=bomber_pyo3.TraderId(self.trader_id.value),
            client_order_id=bomber_pyo3.ClientOrderId(self.client_order_id.value),
            venue_order_id=bomber_pyo3.VenueOrderId(self.venue_order_id.value) if self.venue_order_id else None,
            side=order_side_to_pyo3(self.side),
            price=bomber_pyo3.Price(price.as_f64_c(), price._mem.precision),
            size=bomber_pyo3.Quantity(self.leaves_qty.as_f64_c(), self.leaves_qty._mem.precision),
