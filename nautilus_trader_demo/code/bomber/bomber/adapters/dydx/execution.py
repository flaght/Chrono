...
from bomber.model.orders import MarketIfTouchedOrder
...
class DydxExecutionClient(LiveExecutionClient):
    ...
    def __init__(
        ...
        grpc_urls = config.base_url_grpc or bomber_pyo3.get_dydx_grpc_urls(self._network)  # type: ignore[attr-defined]

        # Initialize gRPC and order submitter (created on connect)
        self._grpc_client: bomber_pyo3.DydxGrpcClient | None = None  # type: ignore[name-defined]
        self._order_submitter: bomber_pyo3.DydxOrderSubmitter | None = None  # type: ignore[name-defined]
        self._grpc_urls = grpc_urls

        # Bidirectional client order ID encoder (set from WS client in _connect)
        self._encoder: bomber_pyo3.DydxClientOrderIdEncoder | None = None  # type: ignore[name-defined]
        ...
        self._ws_client: bomber_pyo3.DydxWebSocketClient | None = None  # type: ignore[name-defined]
        self._ws_url = ws_url

        # Account tracking
        self._wallet_address: str | None = None
        self._pyo3_account_id: bomber_pyo3.AccountId | None = None

    @property
    def pyo3_account_id(self) -> bomber_pyo3.AccountId:
        """
        Return the PyO3 account ID, caching it if not already created.
        """
        if self._pyo3_account_id is None:
            self._pyo3_account_id = bomber_pyo3.AccountId(self.account_id.value)
    ...
    async def _connect(self) -> None:
        ...
        temp_wallet = bomber_pyo3.DydxWallet.from_private_key(private_key)  # type: ignore[attr-defined]
        ...
        if isinstance(self._grpc_urls, str):
            ...
            self._grpc_client = await bomber_pyo3.DydxGrpcClient.connect_with_fallback(  # type: ignore[attr-defined]
                [self._grpc_urls],
            )
        else:
        ...
        self._order_submitter = bomber_pyo3.DydxOrderSubmitter(  # type: ignore[attr-defined]
        ...
        self._ws_client = bomber_pyo3.DydxWebSocketClient.new_private(  # type: ignore[attr-defined]
            url=self._ws_url,
            private_key=private_key,
            authenticator_ids=self._config.authenticator_ids or [],
            account_id=bomber_pyo3.AccountId(account_id.value),
            heartbeat=20,
        ...
        await self._ws_client.connect(
            ...
            trader_id=bomber_pyo3.TraderId(str(self.trader_id)),
        )
    ...
    def _handle_msg(self, raw: object) -> None:
        try:
            ...
            elif isinstance(raw, bomber_pyo3.AccountState):
                self._handle_account_state(raw)
            elif isinstance(raw, bomber_pyo3.OrderAccepted):
                event = OrderAccepted.from_dict(raw.to_dict())
                self._send_order_event(event)
            elif isinstance(raw, bomber_pyo3.OrderFilled):
                event = OrderFilled.from_dict(raw.to_dict())
                self._send_order_event(event)
            elif isinstance(raw, bomber_pyo3.OrderCanceled):
                event = OrderCanceled.from_dict(raw.to_dict())
                self._send_order_event(event)
            elif isinstance(raw, bomber_pyo3.OrderStatusReport):
                report = OrderStatusReport.from_pyo3(raw)
                self._cleanup_order_context(report)
                self._send_order_status_report(report)
            elif isinstance(raw, bomber_pyo3.FillReport):
                report = FillReport.from_pyo3(raw)
                self._send_fill_report(report)
            elif isinstance(raw, bomber_pyo3.PositionStatusReport):
    ...
    async def _submit_order(self, command: SubmitOrder) -> None:
        ...
        self._ws_client.register_order_identity(
            client_order_id=bomber_pyo3.ClientOrderId(str(order.client_order_id)),
            instrument_id=bomber_pyo3.InstrumentId.from_str(str(order.instrument_id)),
            strategy_id=bomber_pyo3.StrategyId(str(order.strategy_id)),
