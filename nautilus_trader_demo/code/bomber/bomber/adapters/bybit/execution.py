...
from bomber.core.bomber_pyo3 import BybitAccountType
...
from bomber.live.enqueue import ThrottledEnqueuer
...
class BybitExecutionClient(LiveExecutionClient):
    ...
    client : bomber_pyo3.BybitHttpClient
    ...
    def __init__(
        ...
        self._is_demo = environment == bomber_pyo3.BybitEnvironment.DEMO
        ...
        self._ws_private_client = bomber_pyo3.BybitWebSocketClient.new_private(
        ...
        self._ws_trade_client: bomber_pyo3.BybitWebSocketClient = (
            bomber_pyo3.BybitWebSocketClient.new_trade(
                environment=environment,
                api_key=config.api_key,
                api_secret=config.api_secret,
    ...
    async def _apply_leverage_settings(self) -> None:
        ...
        for symbol_str, leverage in self._futures_leverages.items():
            try:
                product_type = bomber_pyo3.bybit_product_type_from_symbol(symbol_str)
    ...
    async def generate_order_status_report(
        ...
        try:
            ...
            product_type = bomber_pyo3.bybit_product_type_from_symbol(
    ...
    async def set_leverage(
        ...
        """
        try:
            raw_symbol = bomber_pyo3.bybit_extract_raw_symbol(symbol)
            product_type = bomber_pyo3.bybit_product_type_from_symbol(symbol)
    ...
    def _resolve_position_idx(
        ...
        product_type = bomber_pyo3.bybit_product_type_from_symbol(instrument_id.symbol.value)
        ...
        return bomber_pyo3.bybit_resolve_position_idx(
            position_mode=mode,
    ...
    def _cache_order_position_id(
        ...
        pyo3_venue_position_id = bomber_pyo3.bybit_make_hedge_venue_position_id(
            pyo3_instrument_id,
            position_idx,
        )
    ...
    async def _handle_borrow_action(self, command: QueryAccount, params: dict[str, Any]) -> None:
        ...
        try:
            pyo3_amount = bomber_pyo3.Quantity.from_str(str(amount))
        ...
        response = bomber_pyo3.BybitMarginBorrowResult(
            coin=coin,
            amount=str(amount),
            success=success,
            message=message,
    ...
    async def _handle_repay_action(self, command: QueryAccount, params: dict[str, Any]) -> None:
        ...
        try:
            pyo3_amount = bomber_pyo3.Quantity.from_str(str(amount)) if amount else None
        ...
        response = bomber_pyo3.BybitMarginRepayResult(
            coin=coin,
    ...
    async def _handle_get_borrow_amount_action(
        ...
        try:
            ...
            response = bomber_pyo3.BybitMarginStatusResult(
                coin=coin,
    ...
    async def _submit_order(self, command: SubmitOrder) -> None:
        ...
        pyo3_price = (
            bomber_pyo3.Price.from_str(str(order.price))
        ...
        if order.has_trigger_price:
            pyo3_trigger_price = bomber_pyo3.Price.from_str(str(order.trigger_price))
        ...
        try:
            if self._is_demo:
                ...
                pyo3_take_profit = (
                    bomber_pyo3.Price.from_str(tp_sl["take_profit"])
                    if tp_sl.get("take_profit")
                    else None
                )
                pyo3_stop_loss = (
                    bomber_pyo3.Price.from_str(tp_sl["stop_loss"])
    ...
    async def _submit_order_list_ws(
        ...
        for order in orders:
            ...
            pyo3_stop_loss = (
                bomber_pyo3.Price.from_str(tp_sl["stop_loss"]) if tp_sl.get("stop_loss") else None
            )
            position_idx = self._resolve_position_idx(
                order.instrument_id,
                order.side,
        ...
        if order_params:
            ...
            pyo3_strategy_id = bomber_pyo3.StrategyId(command.strategy_id.value)
    ...
    async def _modify_order(self, command: ModifyOrder) -> None:
        ...
        pyo3_strategy_id = bomber_pyo3.StrategyId(order.strategy_id.value)
        pyo3_instrument_id = bomber_pyo3.InstrumentId.from_str(command.instrument_id.value)
        pyo3_client_order_id = bomber_pyo3.ClientOrderId(command.client_order_id.value)
        pyo3_venue_order_id = (
            bomber_pyo3.VenueOrderId(command.venue_order_id.value)
            if command.venue_order_id
            else None
    ...
    async def _batch_cancel_orders(self, command: BatchCancelOrders) -> None:  # noqa: C901
        ...
        if self._is_demo:
            ...
            for cancel in command.cancels:
                ...
                pyo3_client_order_id = (
                    bomber_pyo3.ClientOrderId(cancel.client_order_id.value)
        ...
        for cancel in command.cancels:
            pyo3_instrument_id = bomber_pyo3.InstrumentId.from_str(cancel.instrument_id.value)
            pyo3_client_order_id = (
                bomber_pyo3.ClientOrderId(cancel.client_order_id.value)
                if cancel.client_order_id
                else None
    ...
    def _handle_msg(self, msg: Any) -> None:
        if isinstance(msg, bomber_pyo3.BybitWebSocketError):
            self._log.warning(f"WebSocket error: {msg}")
        elif isinstance(msg, bomber_pyo3.AccountState):
    ...
    def _handle_position_status_report_pyo3(self, msg: bomber_pyo3.PositionStatusReport) -> None:
    ...
    async def _repay_spot_borrow_if_needed(self, coin: str, bought_qty: Quantity) -> None:
        ...
        try:
            ...
            repay_qty = bomber_pyo3.Quantity.from_decimal_dp(repay_amount, bought_qty.precision)
...
def _validate_price_string(key: str, val: str) -> str:
    s = str(val)
    try:
        p = bomber_pyo3.Price.from_str(s)
    except ValueError:
        raise ValueError(
...
def _build_native_tp_sl_params(tp_sl: dict) -> bomber_pyo3.BybitNativeTpSlParams | None:
    ...
    if not kwargs:
        return None
    return bomber_pyo3.BybitNativeTpSlParams(**kwargs)
