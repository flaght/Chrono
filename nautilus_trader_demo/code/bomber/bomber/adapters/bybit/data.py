...
from bomber.model.data import IndexPriceUpdate
...
class BybitDataClient(LiveMarketDataClient):
    ...
    def __init__(
        ...
        self._product_types = (
            ...
            if config.product_types
                ...
                bomber_pyo3.BybitProductType.OPTION,
            ]
        )
        ...
        self._ws_clients: dict[
            bomber_pyo3.BybitProductType,
            bomber_pyo3.BybitWebSocketClient,
        ] = {}
        ...
        for product_type in self._product_types:
            ws_client = bomber_pyo3.BybitWebSocketClient.new_public(
        ...
        self._depths: dict[bomber_pyo3.InstrumentId, int] = {}
        self._quote_depths: dict[bomber_pyo3.InstrumentId, int] = {}

        # Reference counting for ticker channel
        self._ticker_subscriptions: dict[bomber_pyo3.InstrumentId, set[str]] = {}
    ...
    def _get_ws_client_for_instrument(
        self,
        instrument_id: bomber_pyo3.InstrumentId,
    ) -> bomber_pyo3.BybitWebSocketClient:
    ...
    async def _subscribe_quote_ticks(self, command: SubscribeQuoteTicks) -> None:
        ...
        if product_type == bomber_pyo3.BybitProductType.SPOT:
            depth = 1
    ...
    async def _subscribe_option_greeks(self, command: SubscribeOptionGreeks) -> None:
        ...
        if product_type != bomber_pyo3.BybitProductType.OPTION:
            self._log.warning(
    ...
    async def _request_order_book_snapshot(self, request: RequestOrderBookSnapshot) -> None:
        ...
        if limit is not None:
            match product_type:
                case bomber_pyo3.BybitProductType.SPOT:
                    max_limit = 200
                case bomber_pyo3.BybitProductType.OPTION:
                    max_limit = 25
                case bomber_pyo3.BybitProductType.LINEAR | bomber_pyo3.BybitProductType.INVERSE:
                    max_limit = 500
    ...
    def _handle_msg(self, msg: Any) -> None:
        try:
            ...
            if isinstance(msg, bomber_pyo3.MarkPriceUpdate):
                data = MarkPriceUpdate.from_pyo3(msg)
                self._handle_data(data)
                return

            if isinstance(msg, bomber_pyo3.IndexPriceUpdate):
                data = IndexPriceUpdate.from_pyo3(msg)
                self._handle_data(data)
                return

            if isinstance(msg, bomber_pyo3.FundingRateUpdate):
                data = FundingRateUpdate.from_pyo3(msg)
                self._handle_data(data)
                return

            if isinstance(msg, bomber_pyo3.InstrumentStatus):
                data = InstrumentStatus.from_pyo3(msg)
                self._handle_data(data)
                return

            if isinstance(msg, bomber_pyo3.OptionGreeks):
