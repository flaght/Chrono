...
from bomber.adapters.hyperliquid.constants import HYPERLIQUID_POST_ONLY_WOULD_MATCH
...
class HyperliquidExecutionClient(LiveExecutionClient):
    ...
    client : bomber_pyo3.HyperliquidHttpClient
    ...
    def __init__(
        ...
        environment = config.environment or bomber_pyo3.HyperliquidEnvironment.MAINNET
        ...
        self._ws_client = bomber_pyo3.HyperliquidWebSocketClient(
        ...
        self._processed_trade_ids: bomber_pyo3.FifoCache = bomber_pyo3.FifoCache()
        self._accepted_orders: bomber_pyo3.FifoCache = bomber_pyo3.FifoCache()
        self._terminal_orders: bomber_pyo3.FifoCache = bomber_pyo3.FifoCache()
        ...
        self._pending_fills: dict[str, list[bomber_pyo3.FillReport]] = {}
        ...
        if self._account_address is None:
            try:
                self._account_address = bomber_pyo3.hyperliquid_resolve_execution_account_address(
    ...
    def _sync_cloid_cache(self) -> None:
        ...
        for order in orders:
            ...
            cloid = bomber_pyo3.hyperliquid_cloid_from_client_order_id(pyo3_client_order_id)
    ...
    def _cleanup_cloid_mapping(self, client_order_id: ClientOrderId) -> None:
        ...
        try:
            pyo3_client_order_id = bomber_pyo3.ClientOrderId(client_order_id.value)
            cloid = bomber_pyo3.hyperliquid_cloid_from_client_order_id(pyo3_client_order_id)
    ...
    async def _calculate_market_order_price(self, order: Any) -> Any:
        ...
        return bomber_pyo3.Price.from_decimal(price)
        ...
        try:
            ...
            if order.has_price:
                if self._config.normalize_prices:
                    ...
                    pyo3_price = bomber_pyo3.Price.from_decimal(rounded)
                else:
                    pyo3_price = bomber_pyo3.Price.from_str(str(order.price))
            ...
            if order.has_trigger_price:
                if self._config.normalize_prices:
                    ...
                    pyo3_trigger_price = bomber_pyo3.Price.from_decimal(rounded)
                else:
    ...
    def _process_submit_reports(
        self,
        pyo3_reports: list[bomber_pyo3.OrderStatusReport] | None,
    ) -> None:
    ...
    async def _modify_order(self, command: ModifyOrder) -> None:  # noqa: C901 (sequence of guard clauses)
        ...
        try:
            ...
            pyo3_order_side = order_side_to_pyo3(order.side)
            pyo3_order_type = order_type_to_pyo3(order.order_type)
            pyo3_price = bomber_pyo3.Price.from_str(str(price))
            pyo3_quantity = bomber_pyo3.Quantity.from_str(str(quantity))
            pyo3_time_in_force = time_in_force_to_pyo3(order.time_in_force)
            ...
            if trigger_price is not None:
                pyo3_trigger_price = bomber_pyo3.Price.from_str(str(trigger_price))
    ...
    async def _cancel_all_orders(self, command: CancelAllOrders) -> None:
        ...
        cancel_requests = [
            (
                bomber_pyo3.InstrumentId.from_str(order.instrument_id.value),
                bomber_pyo3.ClientOrderId(order.client_order_id.value),
                bomber_pyo3.VenueOrderId(order.venue_order_id.value)
                if order.venue_order_id
                else None,
            )
        ...
        for cancel_cmd in command.cancels:
            ...
            entries.append(
                (
                    order,
                    (
                        bomber_pyo3.InstrumentId.from_str(cancel_cmd.instrument_id.value),
                        bomber_pyo3.ClientOrderId(cancel_cmd.client_order_id.value),
                        bomber_pyo3.VenueOrderId(order.venue_order_id.value)
                        if order.venue_order_id
                        else None,
                    ),
    ...
    def _handle_fill_report_pyo3(  # noqa: C901 (complexity unavoidable)
        self,
        pyo3_report: bomber_pyo3.FillReport,
    ) -> None:
        report = FillReport.from_pyo3(pyo3_report)
    ...
    def _drain_fill_buffer(
        self,
        buffer: dict[str, list[bomber_pyo3.FillReport]],
        key: str,
    ) -> None:
