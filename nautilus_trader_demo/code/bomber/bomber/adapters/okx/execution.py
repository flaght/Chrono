...
from bomber.adapters.okx.config import OKXExecClientConfig
from bomber.adapters.okx.constants import OKX_VENUE
from bomber.adapters.okx.providers import OKXInstrumentProvider
from bomber.adapters.okx.types import OKXAttachedOcoBinding
from bomber.adapters.okx.types import OkxInstrument
...
from bomber.common.secure import mask_api_key
...
from bomber.core.bomber_pyo3 import OKXEnvironment
from bomber.core.bomber_pyo3 import OKXInstrumentType
from bomber.core.bomber_pyo3 import OKXMarginMode
from bomber.core.bomber_pyo3 import OKXRegion
from bomber.core.bomber_pyo3 import OKXTradeMode
...
from bomber.execution.messages import QueryAccount
...
class OKXExecutionClient(LiveExecutionClient):
    ...
    client : bomber_pyo3.OKXHttpClient
        The OKX HTTP client.
    msgbus : MessageBus
    ...
    def __init__(
        ...
        # WebSocket API
        _private_url = config.base_url_ws or bomber_pyo3.get_okx_ws_url_private(
            self._environment,
            self._region,
        )
        self._ws_client = bomber_pyo3.OKXWebSocketClient.with_credentials(
            url=_private_url,
        ...
        self._ws_business_client = bomber_pyo3.OKXWebSocketClient.with_credentials(
            url=bomber_pyo3.derive_okx_ws_url(_private_url, "business"),
            api_key=config.api_key,
            api_secret=config.api_secret,
            api_passphrase=config.api_passphrase,
            account_id=self.pyo3_account_id,
    ...
    async def generate_order_status_reports(
        ...
        pyo3_reports: list[bomber_pyo3.OrderStatusReport] = []
    ...
    async def _resolve_algo_fallback(
        ...
        command: GenerateOrderStatusReport,
        pyo3_instrument_id: bomber_pyo3.InstrumentId,
    ) -> OrderStatusReport | None:
    ...
    async def _fetch_algo_order_status_report(
        ...
        try:
            pyo3_client_order_id = bomber_pyo3.ClientOrderId(
    ...
    async def generate_fill_reports(
        ...
        pyo3_reports: list[bomber_pyo3.FillReport] = []
    ...
    async def generate_position_status_reports(  # noqa: C901 (too complex)
        ...
        pyo3_reports: list[bomber_pyo3.PositionStatusReport] = []
        reports: list[PositionStatusReport] = []
    ...
    async def _place_order_http(
        ...
        pyo3_trader_id = bomber_pyo3.TraderId.from_str(order.trader_id.value)
        pyo3_strategy_id = bomber_pyo3.StrategyId.from_str(order.strategy_id.value)
    ...
    async def _submit_algo_order_http(self, command: SubmitOrder) -> None:
        ...
        if order.order_type == OrderType.TRAILING_STOP_MARKET:
            ...
            if order.activation_price is not None:
                pyo3_activation_price = bomber_pyo3.Price.from_str(
    ...
    def _categorize_orders_for_batch_cancel(self, cancels):
        ...
        for cancel in cancels:
            ...
            pyo3_inst_id = bomber_pyo3.InstrumentId.from_str(cancel.instrument_id.value)
            resolved_client_order_id = self._exchange_client_order_id(cancel.client_order_id)
            pyo3_client_order_id = (
                bomber_pyo3.ClientOrderId(resolved_client_order_id.value)
                if resolved_client_order_id is not None
                else None
            )
            pyo3_venue_order_id = (
                bomber_pyo3.VenueOrderId(cancel.venue_order_id.value)
    ...
    async def _batch_cancel_algo_orders(
        ...
        for client_order_id, inst_id, algo_id in algo_orders:
            ...
            pyo3_pair = (bomber_pyo3.InstrumentId.from_str(inst_id.value), algo_id)
    ...
    async def _modify_algo_order_http(self, command: ModifyOrder, order: Order) -> None:
        ...
        new_limit_price = (
            bomber_pyo3.Price.from_str(str(command.price))
    ...
    async def _cancel_order_http(self, command: CancelOrder, order: Order) -> None:
        try:
            ...
            response = await self._http_client.cancel_order(
                instrument_id=bomber_pyo3.InstrumentId.from_str(command.instrument_id.value),
                client_order_id=(
                    bomber_pyo3.ClientOrderId(client_order_id.value)
                    if client_order_id is not None
                    else None
                ),
                venue_order_id=(
                    bomber_pyo3.VenueOrderId(venue_order_id.value)
    ...
    def _hydrate_zero_quantity_algo_report(
        ...
        return bomber_pyo3.OrderStatusReport(
            ...
            quantity=bomber_pyo3.Quantity.from_str(str(order.quantity)),
    ...
    def _handle_msg(self, msg: Any) -> None:  # noqa: C901 (too complex)
        if isinstance(msg, bomber_pyo3.OKXWebSocketError):
        ...
        try:
            ...
            elif isinstance(msg, bomber_pyo3.OrderTriggered):
    ...
    def _handle_account_state(self, pyo3_account_state: bomber_pyo3.AccountState) -> None:
        account_state = AccountState.from_dict(pyo3_account_state.to_dict())
        self.generate_account_state(
            balances=account_state.balances,
            margins=account_state.margins,
    ...
    def _handle_order_accepted_pyo3(self, pyo3_event: bomber_pyo3.OrderAccepted) -> None:
        event = OrderAccepted.from_dict(pyo3_event.to_dict())
        self._send_order_event(event)

    def _handle_order_canceled_pyo3(self, pyo3_event: bomber_pyo3.OrderCanceled) -> None:
        event = OrderCanceled.from_dict(pyo3_event.to_dict())
        self._send_order_event(event)
        self._clear_order_state(event.client_order_id)

    def _handle_order_expired_pyo3(self, pyo3_event: bomber_pyo3.OrderExpired) -> None:
        event = OrderExpired.from_dict(pyo3_event.to_dict())
        self._send_order_event(event)
        self._clear_order_state(event.client_order_id)

    def _handle_order_rejected_pyo3(self, pyo3_event: bomber_pyo3.OrderRejected) -> None:
        event = OrderRejected.from_dict(pyo3_event.to_dict())
        self._send_order_event(event)
        self._clear_order_state(event.client_order_id)

    def _handle_order_triggered_pyo3(self, pyo3_event: bomber_pyo3.OrderTriggered) -> None:
        event = OrderTriggered.from_dict(pyo3_event.to_dict())
        self._send_order_event(event)

    def _handle_order_updated_pyo3(self, pyo3_event: bomber_pyo3.OrderUpdated) -> None:
        event = OrderUpdated.from_dict(pyo3_event.to_dict())
        self._send_order_event(event)

    def _handle_order_cancel_rejected_pyo3(
        self,
        pyo3_event: bomber_pyo3.OrderCancelRejected,
    ) -> None:
    ...
    def _handle_order_modify_rejected_pyo3(
        self,
        pyo3_event: bomber_pyo3.OrderModifyRejected,
    ) -> None:
