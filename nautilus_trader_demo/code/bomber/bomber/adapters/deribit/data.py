...
from bomber.adapters.deribit.constants import DERIBIT_DATA_SESSION_NAME
from bomber.adapters.deribit.constants import DERIBIT_VENUE
from bomber.adapters.deribit.constants import DERIBIT_WS_HEARTBEAT_SECS
...
from bomber.data.messages import SubscribeData
...
from bomber.data.messages import UnsubscribeData
...
_PYO3DeribitVolatilityIndex: Any = getattr(bomber_pyo3, "DeribitVolatilityIndex", None)
...
class DeribitDataClient(LiveMarketDataClient):
    ...
    client : bomber_pyo3.DeribitHttpClient
    ...
    def __init__(
        ...
        # WebSocket API
        ws_url = config.base_url_ws or bomber_pyo3.get_deribit_ws_url(environment)
        self._ws_client = bomber_pyo3.DeribitWebSocketClient(
            url=ws_url,
    ...
    async def _fetch_instruments_for_currency(
        self,
        currency: bomber_pyo3.DeribitCurrency,
        product_type: bomber_pyo3.DeribitProductType | None = None,
    ) -> list[Instrument]:
        try:
    ...
    async def _request_instruments(self, request: RequestInstruments) -> None:
        ...
        if product_types:
            ...
            instruments = await self._fetch_instruments_for_currency(
                DeribitCurrency.ANY,
                bomber_pyo3.DeribitProductType.FUTURE,
            )
            all_instruments.extend(instruments)
    ...
    def _handle_msg(self, msg: Any) -> None:
        try:
            ...
            elif isinstance(msg, bomber_pyo3.CustomData):
                if _PYO3DeribitVolatilityIndex is None:
                    self._log.warning(
                        "DeribitVolatilityIndex type is not available in bomber_pyo3",
                    )
                    return
