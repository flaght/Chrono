...
from bomber.adapters.deribit.constants import DERIBIT_EXECUTION_SESSION_NAME
...
from bomber.core.bomber_pyo3 import DeribitEnvironment
...
class DeribitExecutionClient(LiveExecutionClient):
    ...
    http_client : bomber_pyo3.DeribitHttpClient
    ...
    def __init__(
        ...
        self._ws_client = bomber_pyo3.DeribitWebSocketClient.with_credentials(
        ...
        if command.order_side != OrderSide.NO_ORDER_SIDE:
            ...
            for order in open_orders:
                ...
                try:
                    pyo3_trader_id = bomber_pyo3.TraderId.from_str(order.trader_id.value)
                    pyo3_strategy_id = bomber_pyo3.StrategyId.from_str(order.strategy_id.value)
                    pyo3_order_instrument_id = bomber_pyo3.InstrumentId.from_str(
                        order.instrument_id.value,
                    )
                    pyo3_client_order_id = bomber_pyo3.ClientOrderId.from_str(
    ...
    def _handle_order_rejected(self, pyo3_event: bomber_pyo3.OrderRejected) -> None:
        event = OrderRejected.from_dict(pyo3_event.to_dict())
        self._send_order_event(event)

    def _handle_order_accepted(self, pyo3_event: bomber_pyo3.OrderAccepted) -> None:
    ...
    def _handle_order_canceled(self, pyo3_event: bomber_pyo3.OrderCanceled) -> None:
        event = OrderCanceled.from_dict(pyo3_event.to_dict())
        self._send_order_event(event)

    def _handle_order_expired(self, pyo3_event: bomber_pyo3.OrderExpired) -> None:
        event = OrderExpired.from_dict(pyo3_event.to_dict())
        self._send_order_event(event)

    def _handle_order_updated(self, pyo3_event: bomber_pyo3.OrderUpdated) -> None:
        event = OrderUpdated.from_dict(pyo3_event.to_dict())
        self._send_order_event(event)

    def _handle_order_cancel_rejected(self, pyo3_event: bomber_pyo3.OrderCancelRejected) -> None:
        event = OrderCancelRejected.from_dict(pyo3_event.to_dict())
        self._send_order_event(event)

    def _handle_order_modify_rejected(self, pyo3_event: bomber_pyo3.OrderModifyRejected) -> None:
