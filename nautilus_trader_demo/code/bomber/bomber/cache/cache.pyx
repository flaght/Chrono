...
from bomber.core.rust.model import PriceType as PriceType_py
...
from bomber.cache.facade cimport CacheDatabaseFacade
...
from bomber.model.book cimport should_handle_own_book_order
from bomber.model.data cimport Bar
from bomber.model.data cimport BarAggregation
from bomber.model.data cimport BarSpecification
...
from bomber.model.functions cimport order_status_to_pyo3
...
cdef class Cache(CacheFacade):
    ...
    def __init__(
        ...
        self._own_order_books: dict[InstrumentId, bomber_pyo3.OwnOrderBook] = {}
        ...
        self._index_orders_open_pyo3: set[bomber_pyo3.ClientOrderId] = set()
...
# -- COMMANDS -------------------------------------------------------------------------------------
    ...
    cpdef void purge_closed_orders(
        ...
        cdef uint64_t buffer_ns = bomber_pyo3.secs_to_nanos(buffer_secs)
        ...
        for client_order_id, order in self._orders.items():
            ...
            if order.is_open_c():
                ...
                if self._own_order_books:
                    self._index_orders_open_pyo3.add(bomber_pyo3.ClientOrderId(client_order_id.value))
    ...
    cpdef void add_own_order_book(self, own_order_book):
        ...
        Parameters
        ----------
        own_order_book : bomber_pyo3.OwnOrderBook
    ...
    cpdef void update_order(self, Order order):
        ...
        if order.is_open_c():
            ...
            if self._own_order_books:
                self._index_orders_open_pyo3.add(bomber_pyo3.ClientOrderId(order.client_order_id.value))
            ...
            if self._own_order_books:
                self._index_orders_open_pyo3.discard(bomber_pyo3.ClientOrderId(order.client_order_id.value))
    ...
    cpdef void update_own_order_book(self, Order order):
        ...
        if own_book is None:
            ...
            own_book = bomber_pyo3.OwnOrderBook(pyo3_instrument_id)
...
# -- DATA QUERIES ---------------------------------------------------------------------------------
    ...
    cpdef object own_order_book(self, InstrumentId instrument_id):
        ...
        Returns
        -------
        bomber_pyo3.OwnOrderBook or ``None``
    ...
    cpdef get_xrate(
        ...
        try:
            ...
            xrate = bomber_pyo3.get_exchange_rate(
                from_currency=from_currency.code,
                to_currency=to_currency.code,
                price_type=bomber_pyo3.PriceType.from_int(price_type),
...
# -- STRATEGY QUERIES -----------------------------------------------------------------------------
    ...
    cpdef void force_remove_from_own_order_book(self, ClientOrderId client_order_id):
        ...
        if self._own_order_books:
            self._index_orders_open_pyo3.discard(bomber_pyo3.ClientOrderId(client_order_id.value))
    ...
    cpdef void audit_own_order_books(self):
        ...
        for client_order_id in self._index_orders_open:
            if self._own_order_books:
                valid_order_ids.add(bomber_pyo3.ClientOrderId(client_order_id.value))
...
cdef inline dict[Decimal, list[Order]] process_own_order_map(
    dict[Decimal, list[bomber_pyo3.OwnBookOrder]] own_order_map,
