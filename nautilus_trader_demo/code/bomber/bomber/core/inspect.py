...
def is_nautilus_class(cls: type) -> bool:
    ...
    """
    if cls.__module__.startswith("bomber.core.bomber_pyo3.model"):
        return True
    if cls.__module__.startswith("bomber.model.greeks"):
        return False
    if cls.__module__.startswith("bomber.model"):
        return True
    if cls.__module__.startswith("bomber.common"):
        # Custom user signals return False, everything else returns True
        return not cls.__name__.startswith("Signal")
    if cls.__module__.startswith("bomber.test_kit"):
        return False
    return bool(any(base.__module__.startswith("bomber.model") for base in cls.__bases__))
