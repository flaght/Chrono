...
- Changed `bomber_pyo3.get_exchange_rate` to return `decimal.Decimal` instead of `float`
...
- Improved `make build` to leave the venv able to import `bomber` from any cwd via a local editable `.pth`
...
- Added `DydxNetwork` re-export on the `bomber.adapters.dydx` package
...
- Renamed `bomber.adapters.dydx_v4` module to `bomber.adapters.dydx` and standardized class names to `Dydx` prefix (e.g. `DydxDataClientConfig`, `DydxLiveDataClientFactory`)
...
### Breaking Changes
- Removed `bomber.analysis.statistics` subpackage - all statistics are now implemented in Rust and must be imported from `bomber.analysis` (e.g., `from bomber.analysis import WinRate`)
...
- Changed indicator imports from nested modules to flat structure (e.g., `from bomber.indicators.atr import AverageTrueRange` becomes `from bomber.indicators import AverageTrueRange`)
...
- Moved `Indicator` base class from `bomber.indicators.base.indicator` to `bomber.indicators.base`
...
### Breaking Changes
- Removed [talib](https://github.com/nautechsystems/bomber/tree/develop/bomber/indicators/ta_lib) subpackage (see deprecations for v1.211.0)
...
See the [RFC](https://github.com/nautechsystems/bomber/issues/2084) for more details.
...
- Changed meaning of `ws_connection_delay_secs` for [PolymarketDataClientConfig](https://github.com/nautechsystems/bomber/blob/develop/bomber/adapters/polymarket/config.py) to be **non-initial** delay (#2271), thanks @ryantam626
...
### Deprecations
- The [talib](https://github.com/nautechsystems/bomber/tree/develop/bomber/indicators/ta_lib) subpackage for indicators is deprecated and will be removed in a future version, see [RFC](https://github.com/nautechsystems/bomber/issues/2206)
...
### Breaking Changes
- Moved `BinanceOrderBookDeltaDataLoader` from `bomber.persistence.loaders` to `bomber.adapters.binance.loaders`
...
- Moved `TEST_DATA_DIR` constant from `tests` to `bomber` package (#2020), thanks @faysou
...
- Added `bomber.common.events` module with re-exports for `TimeEvent` and other system events
...
### Breaking Changes
- Reorganized configuration objects (separated into a `config` module per subpackage, with re-exports from `bomber.config`)
...
- Defined public API for data, can now import directly from `bomber.model.data` (denest namespace)
- Defined public API for events, can now import directly from `bomber.model.events` (denest namespace)
...
- Defined public API for instruments, can now import directly from `bomber.model.instruments` (denest namespace)
- Defined public API for orders, can now import directly from `bomber.model.orders` (denest namespace)
- Defined public API for order book, can now import directly from `bomber.model.orderbook` (denest namespace)
...
- All configuration objects can now be imported directly from `bomber.config`
...
# Bomber 1.128.0 Beta - Release Notes
...
strategies, opening up further possibilities for extending Bomber with
...
- Added time unit conversion functions as found in `bomber.core.datetime`
...
# Bomber 1.107.1 Beta - Release Notes
