...
Security is a priority for the Bomber project, and we value the work of
...
- Bomber open-source software and official repositories.
...
**Preferred method:** [GitHub Security Advisories](https://github.com/nautechsystems/bomber/security/advisories/new)
...
We only support the latest version of Bomber. If you are using an older
...
- The Core team assesses severity and exposure within the Bomber context.
...
Users who build Bomber from source or extend it with additional
dependencies are responsible for auditing their own dependency trees. The
controls described in this policy apply to official Bomber releases and
...
IDENTITY='^https://github\.com/nautechsystems/bomber/\.github/workflows/build\.yml@refs/heads/(master|nightly)$'

# `gh attestation verify` takes one subject per call, so loop over wheels
for whl in bomber-*.whl; do
  gh attestation verify "$whl" \
    --repo nautechsystems/bomber \
    --cert-identity-regex "$IDENTITY" \
    --cert-oidc-issuer "$ISSUER"
done

gh attestation verify bomber-*.tar.gz \
  --repo nautechsystems/bomber \
  --cert-identity-regex "$IDENTITY" \
  --cert-oidc-issuer "$ISSUER"
```
...
DIGEST=$(crane digest ghcr.io/nautechsystems/bomber:latest)
IMAGE=ghcr.io/nautechsystems/bomber@${DIGEST}
ISSUER=https://token.actions.githubusercontent.com
IDENTITY='^https://github\.com/nautechsystems/bomber/\.github/workflows/docker\.yml@refs/heads/(master|nightly)$'
```
