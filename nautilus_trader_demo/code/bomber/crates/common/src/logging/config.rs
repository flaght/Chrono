...
#[cfg_attr(
    feature = "python",
    pyo3::pyclass(module = "bomber.core.bomber_pyo3.common", from_py_object)
)]
#[cfg_attr(
    feature = "python",
    pyo3_stub_gen::derive::gen_stub_pyclass(module = "bomber.common")
)]
...
mod tests {
    ...
    fn test_from_spec_module_path_with_underscores() {
        let config =
            LoggerConfig::from_spec("bomber::adapters::interactive_brokers=Debug")
                .unwrap();
        assert_eq!(
            config.module_level[&Ustr::from("bomber::adapters::interactive_brokers")],
            LevelFilter::Debug
        );
    }
