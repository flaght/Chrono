...
`nautilus-interactive-brokers` is the Rust adapter that connects Bomber
to Interactive Brokers TWS and IB Gateway.

The crate wraps the `ibapi` client and feeds Bomber's live data,
execution, historical data, and instrument loading infrastructure. Optional
PyO3 bindings expose the same implementation through `bomber`.
...
- `python`: PyO3 bindings exposed as `bomber_pyo3.interactive_brokers` when the `python` feature is enabled.
...
- `extension-module`: Builds the crate as a Python extension module. This is
  the feature used by the `bomber` package and includes `python` and
  `gateway`.
...
Bomber. This is a user-managed setting in TWS / IB Gateway, not
something the adapter converts automatically at runtime.

## Status

This crate is the Rust implementation of Bomber's Interactive Brokers
