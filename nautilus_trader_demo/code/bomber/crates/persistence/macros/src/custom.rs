...
//! - `stub_module = "bomber.<module>"`: Generate pyo3-stub-gen metadata for the
...
mod tests {
    ...
    fn parse_options_accepts_no_arrow_stub_module_with_pyo3() {
        let options =
            parse_options(&quote! { pyo3, no_arrow, stub_module = "bomber.persistence" })
        ...
        assert_eq!(
            ...
            Some("bomber.persistence"),
        );
    }

    #[rstest]
    fn parse_options_rejects_stub_module_without_pyo3() {
        let err = parse_options_error(&quote! { stub_module = "bomber.persistence" });
    ...
    fn expand_emits_stub_attributes_before_pyo3_attributes() {
        let attr = quote! { pyo3, no_arrow, stub_module = "bomber.test" };
        let item = quote! {
            pub struct TestData {
                pub value: f64,
                pub ts_event: nautilus_core::UnixNanos,
    ...
    fn expand_emits_referenced_bound_for_classmethod_receivers() {
        let attr = quote! { pyo3, stub_module = "bomber.test" };
        let item = quote! {
            pub struct TestData {
                pub value: f64,
                pub ts_event: nautilus_core::UnixNanos,
