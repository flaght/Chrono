...
write_release_source() {
  ...
  printf 'wheel artifact\n' > "${release_source}/bomber-1.2.3-py3-none-any.whl"
  printf 'sdist artifact\n' > "${release_source}/bomber-1.2.3.tar.gz"
  cat > "${release_source}/dist-manifest.json" << 'JSON'
{
  ...
  "artifacts": [
    {
      "name": "bomber-1.2.3-py3-none-any.whl",
      "sha256": "wheel-sha256",
      "size": 15
    }
  ]
  ...
  cat > "$output" << 'JSON'
{
  ...
  "assets": [
    ...
    {"name": "bomber-1.2.3-py3-none-any.whl", "state": "uploaded", "size": 15},
    {"name": "bomber-1.2.3-py3-none-any.whl.sha256", "state": "uploaded", "size": 96},
    {"name": "bomber-1.2.3-py3-none-any.whl.sigstore", "state": "uploaded", "size": 96},
    {"name": "bomber-1.2.3-py3-none-any.whl.intoto.jsonl", "state": "uploaded", "size": 96}
  ]
}
JSON
}
...
run_verify_assets() {
  ...
  (
    ...
    env \
      ...
      GITHUB_REPOSITORY=nautechsystems/bomber \
...
run_verify_failure "$missing_sigstore_json" "${work_dir}/missing-sigstore.out" \
  "missing asset: bomber-1.2.3-py3-none-any.whl.sigstore"
...
assert_file_exists "${generated_assets}/bomber-1.2.3-py3-none-any.whl.sha256" \
  "generate-only should write the wheel checksum."
assert_file_exists "${generated_assets}/bomber-1.2.3.tar.gz.sha256" \
...
assert_contains "${work_dir}/upload.log" "bomber-1.2.3-py3-none-any.whl.sha256" \
  "publish-existing should upload per-wheel checksums."
assert_contains "${work_dir}/upload.log" "bomber-1.2.3.tar.gz.sha256" \
