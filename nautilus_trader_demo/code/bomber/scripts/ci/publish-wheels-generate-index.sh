...
bucket_path="s3://${CLOUDFLARE_R2_BUCKET_NAME}/${CLOUDFLARE_R2_PREFIX:-simple/bomber}/"
...
if aws s3 ls "${bucket_path}${index_file}" --endpoint-url="${CLOUDFLARE_R2_URL}" > /dev/null 2>&1; then
  ...
  echo '<html><head><title>Bomber Packages</title></head>' >> "$index_file"
  echo '<body><h1>Packages for bomber</h1></body></html>' >> "$index_file"
fi
...
echo '<html><head><title>Bomber Packages</title></head>' >> "${index_file}.new"
echo '<body><h1>Packages for bomber</h1>' >> "${index_file}.new"
...
for file in dist/bomber-*.whl; do
  if [[ -f "$file" ]]; then
...
existing_files=$(aws s3 ls "${bucket_path}" --endpoint-url="${CLOUDFLARE_R2_URL}" | grep 'bomber-.*\.whl$' | awk '{print $4}')
