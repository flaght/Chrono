...
python_artifact="${work_dir}/bomber-1.2.3.tar.gz"
...
JSON
    ...
    trusted)
      cat > "$output" <<JSON
{"versions":[{"num":"1.2.3","checksum":"${CRATE_SHA256}","trustpub_data":{"provider":"github","repository":"nautechsystems/bomber","sha":"abc123"},"published_by":null}]}
JSON
      ;;
    *)
...
case "$url" in
  https://pypi.org/pypi/bomber/json)
    cat > "$output" <<JSON
{"releases":{"1.2.3":[{"filename":"bomber-1.2.3.tar.gz","digests":{"sha256":"${PYTHON_SHA256}"},"url":"https://example.invalid/bomber-1.2.3.tar.gz"}]}}
JSON
    ;;
  https://example.invalid/bomber-1.2.3.tar.gz)
    cp "$PYTHON_ARTIFACT_FILE" "$output"
    ;;
  https://pypi.org/integrity/bomber/1.2.3/bomber-1.2.3.tar.gz/provenance*)
...
JSON
    ...
    cat > "$output" <<'JSON'
{"attestation_bundles":[{"publisher":{"kind":"GitHub","repository":"nautechsystems/bomber","workflow":"build.yml","environment":"release"}}]}
JSON
    ;;
...
assert_current_release_crates_manifest() {
  ...
  if ! jq -e '
    ...
    and .crates[0].trusted_publishing.repository == "nautechsystems/bomber"
