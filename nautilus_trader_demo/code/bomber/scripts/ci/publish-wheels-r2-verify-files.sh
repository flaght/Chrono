...
ok=false
for i in {1..5}; do
  if aws s3 ls "s3://${CLOUDFLARE_R2_BUCKET_NAME}/${CLOUDFLARE_R2_PREFIX:-simple/bomber}/" \
...
ok_index=false
for i in {1..5}; do
  if aws s3 ls "s3://${CLOUDFLARE_R2_BUCKET_NAME}/${CLOUDFLARE_R2_PREFIX:-simple/bomber}/index.html" \
    --endpoint-url="${CLOUDFLARE_R2_URL}" --cli-connect-timeout 10 --cli-read-timeout 60; then
    ok_index=true
    break
  else
