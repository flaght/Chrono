...
files=$(aws s3 ls "s3://${CLOUDFLARE_R2_BUCKET_NAME}/${CLOUDFLARE_R2_PREFIX:-simple/bomber}/" \
  ...
  for platform_tag in $platform_tags; do
    ...
    for file in $matching_files; do
      if [[ "$file" != "$latest" ]]; then
        ...
        success=false
        for i in {1..5}; do
          if aws s3 rm "s3://${CLOUDFLARE_R2_BUCKET_NAME}/${CLOUDFLARE_R2_PREFIX:-simple/bomber}/$file" \
