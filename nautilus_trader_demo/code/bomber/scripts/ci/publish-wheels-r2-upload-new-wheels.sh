...
echo "dist/ contents:"
ls -la dist/
find dist/ -type f -name "bomber-*.whl" -ls

# Verify wheels exist
if ! find dist/ -type f -name "bomber-*.whl" -print -quit | grep -q .; then
...
for file in dist/bomber-*.whl; do
  ...
  success=false
  for i in {1..5}; do
    aws s3 cp "$file" "s3://${CLOUDFLARE_R2_BUCKET_NAME}/${CLOUDFLARE_R2_PREFIX:-simple/bomber}/" \
