...
Bomber uses modular *adapters* to connect to trading venues and data providers, translating raw APIs into a unified interface and normalized domain model.
...
The primary goal of Bomber is to provide a unified trading system for
...
- The full range of an exchange's functionality (where applicable to Bomber) should *eventually* be supported.
...
- Actions unsupported by an exchange or Bomber will be logged as a warning or error when invoked.

## API unification

All integrations must conform to Bomber’s system API, requiring normalization and standardization:
