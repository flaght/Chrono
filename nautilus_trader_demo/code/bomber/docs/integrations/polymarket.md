...
Bomber supports multiple Polymarket signature types for order signing, which gives
flexibility for different wallet configurations while Bomber handles signing and order
preparation.

## Installation

To install Bomber with Polymarket support:

```bash
uv pip install "bomber[polymarket]"
```
...
You can find live example scripts [here](https://github.com/nautechsystems/bomber/tree/develop/examples/live/polymarket/).
...
Bomber represents Polymarket outcome tokens as `BinaryOption` instruments.
...
|---------------------|-------------------------------------------------------------------------------|---------------------------------------------------------------|-------|
| Public package path | `bomber.adapters.polymarket`                                         | `bomber.adapters.polymarket`                         | Rust is the consolidation target. |
...
To interact with Polymarket via Bomber, you'll need a **Polygon**-compatible wallet (such as MetaMask).
...
Bomber defaults to signature type 0 (EOA) but can be configured to use any of the supported signature types via the `signature_type` configuration parameter.
...
You can do this by running the provided script located at `bomber/adapters/polymarket/scripts/set_allowances.py`.
...
```bash
python bomber/adapters/polymarket/scripts/set_allowances.py
```
...
2. Run the script using:
   ...
   ```bash
   python bomber/adapters/polymarket/scripts/create_api_key.py
   ```
...
When setting up Bomber to work with Polymarket, it’s crucial to properly configure the necessary parameters, particularly the private key.
...
Polymarket calls the `POST /order` field `orderType`. In Bomber, this maps to
...
:::note
Polymarket uses `FAK` (Fill-And-Kill) for the semantics Bomber calls
...
Bomber records the initial trade details in the `info` field of the `OrderFilled` event,
...
`bomber.backtest.models.FeeModel` subclass) which applies the taker
...
```python
from bomber.adapters.polymarket.fee_model import PolymarketFeeModel
...
Bomber automatically manages WebSocket connections to handle this limitation:
...
That is a Bomber default, not Polymarket's current published limit.
...
Class: `PolymarketDataClientConfig` in `bomber.adapters.polymarket.config`.
...
Class: `PolymarketExecClientConfig` in `bomber.adapters.polymarket.config`.
...
```python
from bomber.adapters.polymarket import PolymarketInstrumentProviderConfig
from bomber.adapters.polymarket import PolymarketUpDownEventSlugConfig
...
python bomber/adapters/polymarket/scripts/active_markets.py

# List BTC and ETH UpDown markets specifically
python bomber/adapters/polymarket/scripts/list_updown_markets.py
```
...
from bomber.adapters.polymarket import PolymarketDataLoader
...
from bomber.adapters.polymarket import POLYMARKET_VENUE
from bomber.adapters.polymarket import PolymarketDataLoader
...
# Create Bomber InstrumentId from Polymarket identifiers
