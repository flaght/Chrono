...
To install Bomber with Interactive Brokers (and Docker) support:

```bash
uv pip install "bomber[ib,docker]"
```
...
:::note
Because IB does not provide wheels for `ibapi`, Bomber [repackages](https://pypi.org/project/nautilus-ibapi/) it for release on PyPI.
:::

## Examples

You can find live example scripts [here](https://github.com/nautechsystems/bomber/tree/develop/examples/live/interactive_brokers/).
...
:::warning
Configure TWS or IB Gateway to return market data timestamps in UTC before connecting Bomber. This setting must be enabled by the user in TWS/IB Gateway, as Bomber is designed to work with UTC timestamps.
:::
...
```python
from bomber.adapters.interactive_brokers.config import InteractiveBrokersDataClientConfig
from bomber.adapters.interactive_brokers.config import InteractiveBrokersExecClientConfig
...
```python
from bomber.adapters.interactive_brokers.config import DockerizedIBGatewayConfig
from bomber.adapters.interactive_brokers.gateway import DockerizedIBGateway
...
In Interactive Brokers, a Bomber `Instrument` corresponds to an IB [Contract](https://ibkrcampus.com/ibkr-api-page/trader-workstation-api/#contracts). The adapter handles two types of contract representations:
...
- Cannot be directly converted to a Bomber `Instrument`
...
- Can be converted to a Bomber `Instrument`
...
```python
from bomber.adapters.interactive_brokers.config import InteractiveBrokersInstrumentProviderConfig
from bomber.adapters.interactive_brokers.config import SymbologyMethod
...
```python
from bomber.adapters.interactive_brokers.common import IBContract
...
Interactive Brokers supports option spreads through BAG contracts, which combine multiple option legs into a single tradeable instrument. Bomber provides support for creating, loading, and trading option spreads.
...
```python
from bomber.model.identifiers import InstrumentId, new_generic_spread_id
...
```python
from bomber.adapters.interactive_brokers.historical.client import HistoricInteractiveBrokersClient
...
from bomber.adapters.interactive_brokers.common import IBContract
from bomber.adapters.interactive_brokers.historical.client import HistoricInteractiveBrokersClient
from bomber.persistence.catalog import ParquetDataCatalog
...
```python
from bomber.adapters.interactive_brokers.config import IBMarketDataTypeEnum
from bomber.adapters.interactive_brokers.config import InteractiveBrokersDataClientConfig
...
```python
from bomber.adapters.interactive_brokers.config import InteractiveBrokersExecClientConfig
from bomber.config import RoutingConfig
...
```python
from bomber.adapters.interactive_brokers.common import IBOrderTags
...
```python
import os
from bomber.adapters.interactive_brokers.common import IB
from bomber.adapters.interactive_brokers.common import IB_VENUE
from bomber.adapters.interactive_brokers.config import InteractiveBrokersDataClientConfig
from bomber.adapters.interactive_brokers.config import InteractiveBrokersExecClientConfig
from bomber.adapters.interactive_brokers.config import InteractiveBrokersInstrumentProviderConfig
from bomber.adapters.interactive_brokers.config import IBMarketDataTypeEnum
from bomber.adapters.interactive_brokers.config import SymbologyMethod
from bomber.adapters.interactive_brokers.factories import InteractiveBrokersLiveDataClientFactory
from bomber.adapters.interactive_brokers.factories import InteractiveBrokersLiveExecClientFactory
from bomber.config import LiveDataEngineConfig
...
Bomber supports using multiple Interactive Brokers execution clients simultaneously, each connected to a different IB account. This is useful when you need to trade with multiple accounts, such as:
...
```python
from bomber.adapters.interactive_brokers.config import (
...
from bomber.live.config import TradingNodeConfig, RoutingConfig, LoggingConfig
from bomber.model.identifiers import AccountId, Venue, ClientId
...
```python
from bomber.model.identifiers import AccountId, ClientId
...
You can find additional examples here: <https://github.com/nautechsystems/bomber/tree/develop/examples/live/interactive_brokers>
...
- **Bomber Examples**: [GitHub Examples](https://github.com/nautechsystems/bomber/tree/develop/examples/live/interactive_brokers)
...
[contributing guide](https://github.com/nautechsystems/bomber/blob/develop/CONTRIBUTING.md).
:::
