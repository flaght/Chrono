...
Bomber provides an adapter for integrating with the Betfair REST API and
Exchange Streaming API.

## Installation

Install Bomber with Betfair support:

```bash
uv pip install "bomber[betfair]"
```
...
You can find live example scripts [here](https://github.com/nautechsystems/bomber/tree/develop/examples/live/betfair/).
...
Bomber currently ships a stable Python Betfair adapter and an in-progress Rust parity path.
...
```python
from bomber.adapters.betfair.constants import BETFAIR_CLIENT_ID
...
```python
from bomber.adapters.betfair.parsing.core import parse_betfair_rcm_file
...
```python
from bomber.adapters.betfair import BETFAIR
from bomber.adapters.betfair import BetfairLiveDataClientFactory
from bomber.adapters.betfair import BetfairLiveExecClientFactory
