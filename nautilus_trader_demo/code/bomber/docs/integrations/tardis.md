...
Bomber integrates with the Tardis API, Tardis Machine WebSocket server, and Tardis CSV
...
We recommend also referring to the Tardis documentation in conjunction with this Bomber integration guide.
...
Bomber does not support exchange-native Tardis market data formats in this adapter.

The following normalized Tardis Machine formats are supported by Bomber. See the official
...
The Tardis integration ensures compatibility with Bomber's crypto exchange adapters
by consistently normalizing symbols. Typically, Bomber uses the native exchange naming
...
The following environment variables are used by Tardis and Bomber.

- `TM_API_KEY`: API key for the Tardis Machine.
- `TARDIS_API_KEY`: API key for Bomber Tardis clients.
- `TARDIS_MACHINE_WS_URL` (optional): WebSocket URL for the `TardisMachineClient`.
- `TARDIS_BASE_URL` (optional): Base URL for the `TardisHttpClient` in Bomber.
...
async def run():
    ...
    await bomber_pyo3.run_tardis_machine_replay(str(config_filepath.resolve()))
...
from bomber.adapters.tardis import TardisCSVDataLoader
...
async def run():
    http_client = bomber_pyo3.TardisHttpClient()
...
The `TardisDataClient` integrates Tardis Machine with a running Bomber system.
