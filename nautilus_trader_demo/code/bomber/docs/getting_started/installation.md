...
Bomber is officially supported for Python 3.12-3.14 on the following 64-bit platforms:
...
:::note
Bomber may work on other platforms, but only those listed above are regularly used by developers and tested in CI.
:::
...
To install the latest [bomber](https://pypi.org/project/bomber/) binary wheel (or sdist package) from PyPI:

```bash
uv pip install bomber
```
...
```bash
uv pip install "bomber[docker,ib]"
```
...
```bash
uv pip install bomber --index-url=https://packages.nautechsystems.io/simple
```
...
```bash
uv pip install bomber --pre --index-url=https://packages.nautechsystems.io/simple
```

To install a specific development wheel (e.g., `1.221.0a20250912` for September 12, 2025):

```bash
uv pip install bomber==1.221.0a20250912 --index-url=https://packages.nautechsystems.io/simple
```
...
```bash
uv pip install --pre --index-url=https://packages.nautechsystems.io/v2/simple/ bomber
```

The `--pre` flag is required because these wheels are pre-release builds. The installed import name
is still `bomber`.

Run this command outside a Bomber source checkout. The repository root uses an
...
```bash
curl -s https://packages.nautechsystems.io/simple/bomber/index.html | grep -oP '(?<=<a href=")[^"]+(?=")' | awk -F'#' '{print $1}' | sort
```
...
For step-by-step verification commands, see [Verifying releases](https://github.com/nautechsystems/bomber/blob/develop/SECURITY.md#verifying-releases) in `SECURITY.md`.
...
Use it from a Bomber source checkout, when a v2 development wheel is not available for your
...
[python-v2-readme]: https://github.com/nautechsystems/bomber/blob/develop/python/README.md

## From GitHub release

To install a binary wheel from GitHub, first navigate to the [latest release](https://github.com/nautechsystems/bomber/releases/latest).
...
Use Bomber only if you are prepared to adapt to these changes.

## Redis

Using [Redis](https://redis.io) with Bomber is **optional** and only required if configured as the backend for a cache database or [message bus](../concepts/message_bus.md).
...
- Expose Redis on the default port 6379, making it accessible to Bomber on your machine.
