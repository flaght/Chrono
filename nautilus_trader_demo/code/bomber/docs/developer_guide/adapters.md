...
This developer guide provides specifications for how to build an integration adapter for the Bomber platform.
...
Bomber adapters follow a layered architecture pattern with:
...
### Python layer (`bomber/adapters/your_adapter`)
...
- Mock the PyO3 boundary (`bomber_pyo3` shims, stubbed Rust clients) so tests stay fast while verifying that configuration, factory wiring, and error handling match the exported Rust API.
...
from bomber.live.data_client import LiveDataClient
from bomber.model import DataType
...
```python
from bomber.config import LiveDataClientConfig
from bomber.config import LiveExecClientConfig
