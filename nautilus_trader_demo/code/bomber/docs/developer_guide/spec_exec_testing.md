...
(`bomber.test_kit.strategies.tester_exec`) and Rust
...
Legacy examples still use `bomber.live.node.TradingNode`, but new Rust-backed
PyO3 adapters should prefer `bomber.live.LiveNode`. Use `LiveNode.builder(...)`
when you need to register adapter client factories before the node is built.

```python
from bomber.common import Environment
from bomber.live import LiveExecEngineConfig, LiveNode, LiveRiskEngineConfig
