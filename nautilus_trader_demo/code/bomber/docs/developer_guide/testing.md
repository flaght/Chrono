...
`bomber.live.LiveNode`. `bomber.live.node.TradingNode` remains the
...
- Use `@pytest.mark.parametrize` to cover multiple inputs without duplicating
  test bodies.
- Import model types from `bomber.model`, not from
  `bomber.core.bomber_pyo3`.
...
### Step 0: Compile `bomber` with debug symbols

   ```bash
   cd bomber && make build-debug-pyo3
   ```

### Step 1: Set up debugging configuration

```python
from bomber.test_kit.debug_helpers import setup_debugging
...
By default `setup_debugging()` expects the `.vscode` folder one level above the `bomber` root directory.
...
| Backtest client        | `bomber/backtest/data_client.pyx`  | Backtest client overrides base subscribe/unsubscribe.      |
...
5. **Backtest client** (`bomber/backtest/data_client.pyx`): Override
