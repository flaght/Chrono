...
The `nautilus-model` build script regenerates `bomber/core/includes/model.h` and
`bomber/core/rust/model.pxd` when the `ffi` feature is enabled. Those files encode
...
```bash
git diff -- bomber/core/includes/model.h bomber/core/rust/model.pxd
```
...
#[cfg_attr(
    feature = "python",
    pyo3::pyclass(module = "bomber.model")
)]
#[cfg_attr(
    feature = "python",
    pyo3_stub_gen::derive::gen_stub_pyclass(module = "bomber.model")
)]
...
#[cfg_attr(
    feature = "python",
    pyo3_stub_gen::derive::gen_stub_pyclass_enum(module = "bomber.model")
)]
...
#[pyo3_stub_gen::derive::gen_stub_pyfunction(module = "bomber.serialization")]
...
**Module parameter:** set `module = "bomber.<package>"` to match the Python
package where the type is imported. For example, model types use
`bomber.model` and serialization functions use
`bomber.serialization`.
...
Python bindings are provided via [PyO3](https://pyo3.rs), allowing users to import Bomber crates directly in Python without a Rust toolchain.
