...
Bomber uses increasingly more [Rust](https://www.rust-lang.org), so Rust should be installed on your system as well
...
:::info
Bomber *must* compile and run on **Linux, macOS, and Windows**. Please keep portability in
...
```bash
git clone --branch develop https://github.com/nautechsystems/bomber
cd bomber
...
Bomber pins every development tool so that all contributors and CI run identical versions.
...
The Nautilus CLI is a command-line interface tool for interacting with the Bomber ecosystem.
...
After installing the Bomber CLI, you may need to ensure the Cargo binary takes precedence by either:
