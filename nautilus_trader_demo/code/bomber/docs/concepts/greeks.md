...
The legacy Cython `GreeksCalculator` class in `bomber/model/greeks.pyx` computes
Black-Scholes Greeks from cached market data. A PyO3 calculator is also exposed from
`bomber.common.GreeksCalculator` for the v2 runtime surface.
Both calculators use the cache and clock and are accessible from actors or strategies.

```python
from bomber.model.greeks import GreeksCalculator  # legacy Cython

# v2 PyO3: from bomber.common import GreeksCalculator
...
```python
from bomber.model.greeks_data import YieldCurveData
import numpy as np
