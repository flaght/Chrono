...
Bomber exposes the same instrument model to Rust and Python users. Rust
examples use `nautilus_model`; Python examples use `bomber.model.instruments`.
...
Bomber groups instruments by the market structure they represent:
...
Python users normally work with instrument classes from `bomber.model`:
...
instrument. Bomber enforces the resulting price and size grids strictly because
