...
series_id = bomber_pyo3.OptionSeriesId(...)  # identifies the series (venue, underlying, expiry)

# Subscribe to 5 strikes above and below ATM, snapshot every 1000ms
strike_range = bomber_pyo3.StrikeRange.atm_relative(strikes_above=5, strikes_below=5)
...
|---------------|-----------------------------------------------------|------------------------------------------------|
| `Fixed`       | Subscribe to an explicit set of strikes.            | `bomber_pyo3.StrikeRange.fixed([...])`       |
| `AtmRelative` | N strikes above and N below the current ATM strike. | `bomber_pyo3.StrikeRange.atm_relative(5, 5)` |
| `AtmPercent`  | All strikes within a percentage band around ATM.    | `bomber_pyo3.StrikeRange.atm_percent(0.10)`  |
| `Delta`       | Strikes whose call or put delta is near a target.   | `bomber_pyo3.StrikeRange.delta(0.25, 0.05)`  |
