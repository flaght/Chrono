...
This guide covers the architectural principles and structure of Bomber:
...
The major architectural techniques and design patterns employed by Bomber are:
...
Bomber is incrementally adopting a high-assurance mindset: critical code
...
Bomber draws inspiration from [crash-only design](https://en.wikipedia.org/wiki/Crash-only_software)
...
Bomber prioritizes data integrity over availability for trading operations. The system employs
...
By crashing immediately on invalid data, Bomber aims to provide:
...
and helps Bomber maintain strong data correctness for production trading.
...
The Bomber codebase is actually both a framework for composing trading
...
![Architecture](https://github.com/nautechsystems/bomber/blob/develop/assets/architecture-overview.png?raw=true "architecture")
...
An environment context in Bomber defines the type of data and trading venue you work with.
...
The bulk of the production code resides in the `bomber` directory, which contains a collection of Python/Cython subpackages and modules.
...
```mermaid
flowchart TB
    subgraph trader["bomber<br/>Python / Cython"]
    end
...
The documentation aims to cover all possible exceptions that Bomber code
...
- [Overview](overview.md) - High-level introduction to Bomber.
