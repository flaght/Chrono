...
Bomber offers two API levels for backtesting:
...
Bomber is primarily designed and optimized for order book data, which provides
...
Bomber treats historical order book and trade data as **immutable** during backtesting. What happened in the market is preserved exactly as recorded. Fills never modify the underlying book state.
...
```python
from bomber.backtest.engine import BacktestEngine
from bomber.model.enums import OmsType, AccountType
from bomber.model import Money, Currency
...
venue_config = BacktestVenueConfig(
    ...
    fill_model=ImportableFillModelConfig(
        fill_model_path="bomber.backtest.models:FillModel",
...
venue_config = BacktestVenueConfig(
    ...
    starting_balances=["100_000 USD"],
    fill_model=ImportableFillModelConfig(
        fill_model_path="bomber.backtest.models:ThreeTierFillModel",
    ),
)
```
...
from bomber.model.book import OrderBook, BookOrder
...
```python
from bomber.adapters.binance import BINANCE_VENUE
from bomber.backtest.engine import BacktestEngine
