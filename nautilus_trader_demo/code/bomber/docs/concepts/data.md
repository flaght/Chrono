...
Bomber operates primarily on granular order book data for the highest realism
...
Data aggregation in Bomber transforms granular market data into structured bars or candles for several reasons:
...
Bomber implements three distinct data aggregation methods:
...
Bomber defines a unique *bar type* (`BarType` class) based on the following components:
...
For bar-to-bar aggregation, the target bar type is always `INTERNAL` (since you're doing the aggregation within Bomber),
...
Bomber provides two distinct operations for working with bars:
...
Bomber supports data loading and conversion for three main use cases:
...
Data wranglers are implemented per specific Nautilus data type, and can be found in the `bomber.persistence.wranglers` module.
...
Bomber uses fixed-point arithmetic for `Price` and `Quantity` types for precise financial calculations without floating-point errors. Understanding how raw values work is essential when creating data or working with catalogs.
...
from bomber.adapters.binance.loaders import BinanceOrderBookDeltaDataLoader
from bomber.persistence.wranglers import OrderBookDeltaDataWrangler
...
The Bomber data catalog is built on a dual-backend architecture that combines the performance of Rust with the flexibility of Python:
...
```python
from bomber.model import QuoteTick, TradeTick
...
```python
from bomber.config import BacktestDataConfig
from bomber.model import QuoteTick, InstrumentId
...
```python
from bomber.config import BacktestRunConfig, BacktestVenueConfig
...
```python
from bomber.persistence.config import StreamingConfig, RotationMode
import pandas as pd
...
The Bomber data catalog provides market data management:
...
Bomber defines an internal data format specified in the `nautilus_model` crate.
...
be sure to check out commit [e284162](https://github.com/nautechsystems/bomber/commit/e284162cf27a3222115aeb5d10d599c8cf09cf50)
...
import msgspec
from bomber.core import Data
from bomber.core.datetime import unix_nanos_to_iso8601
from bomber.model import DataType
from bomber.serialization.base import register_serializable_type
from bomber.serialization.arrow.serializer import register_arrow
import pyarrow as pa

from bomber.model import InstrumentId
from bomber.core.datetime import dt_to_unix_nanos, unix_nanos_to_dt, format_iso8601
...
```python
from bomber.model.custom import customdataclass
...
To use custom data with the Rust-backed catalog (`ParquetDataCatalog` from `bomber_pyo3`), use the
...
```python
from bomber.core.bomber_pyo3 import ParquetDataCatalog
from bomber.core.bomber_pyo3.model import CustomData
from bomber.core.bomber_pyo3.model import DataType
from bomber.core.bomber_pyo3.model import register_custom_data_class
from bomber.model.custom import customdataclass_pyo3
...
See `bomber.model.custom.customdataclass_pyo3` for details.
