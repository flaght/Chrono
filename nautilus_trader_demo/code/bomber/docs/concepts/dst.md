...
**Deterministic simulation testing (DST)** runs Bomber under a seed-controlled runtime so that
timing-sensitive execution behavior is bitwise reproducible from a single integer. This guide
explains what DST is, how Bomber supports it, what guarantees the support provides, and
where those guarantees stop.

The goal is a published contract that external users and auditors can verify: the determinism
Bomber claims is backed by source-level evidence and enforced at commit time by a pre-
...
Bomber's DST support has two halves:
...
- **Seed-reproducible execution** for the in-scope portion of the Bomber runtime.
...
- **Matching engine** (`crates/execution/src/matching_engine/engine.rs`): nine fields
  ...
  [#3914](https://github.com/nautechsystems/bomber/issues/3914).
...
under `bomber/` are excluded from the contract as a policy, not as a weakness. Any
