...
def predict(variant):
    ...
    pdb.set_trace()
    prediction_model = create_prediction_model(features=test_datasets.features,
                                               window=variant['window'])
    volatility_model = create_volatility_model(features=test_datasets.features,
                                               window=variant['window'],
                                               seq_cycle=variant['seq_cycle'])
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    pdb.set_trace()
    prediction_model = prediction_model.to(device=device)
    prediction_model_dict = torch.load("runs/models/best_prediction_3.pth",
                                       map_location=device)
    new_state_prediction_dict = OrderedDict()
    for k, v in prediction_model_dict.items():
        name = k[:]
        new_state_prediction_dict[name] = v
    pdb.set_trace()
    prediction_model.load_state_dict(new_state_prediction_dict)

    volatility_model = volatility_model.to(device=device)
    volatility_model_dict = torch.load("runs/models/best_volatility_3.pth",
                                       map_location=device)

    new_state_volatility_dict = OrderedDict()
    for k, v in volatility_model_dict.items():
        pdb.set_trace()
        name = k[:]
        new_state_volatility_dict[name] = v
    volatility_model.load_state_dict(new_state_volatility_dict)
    print('-->')
