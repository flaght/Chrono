...
base_path = '/workspace/data/dev/kd/evolution/nn/yunkin'
method = 'aicso1'
categories = 'o2o'
horizon = 1
seq_cycle = 3
...
# 使用示例
train_dataset = Dataset10.generate(train_data,
                                   seq_cycle=seq_cycle,
                                   features=features,
                                   window=window,
                                   target=['nxt1_ret'],
...
train_loader = DataLoader(
    ...
    shuffle=False,
    collate_fn=Dataset10.collate_fn)

for batch in train_loader:
    for time_batch in batch:
        print(f"\nTime: {time_batch['time']}")
        if time_batch['time'] == '2024-01-09 10:31:00':
            pdb.set_trace()
        print(f"Values shape: {time_batch['values'].shape}")  # [N, 3, 7]
        print(f"Target shape: {time_batch['target'].shape}")  # [1]
