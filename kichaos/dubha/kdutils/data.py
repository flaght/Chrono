...
def fetch_dfactors(base_path):
   ...
   # columns = ['sw1', 'dummy120_fst', 'dummy120_fst_close','hs300']
    ...
    for ff in res:
        ...
        ### 先算覆盖率
        '''
        disp = booster.create_state(
            (factor_data.copy() * dummy120_fst * dummy120_fst_close).values,
            (dummy120_fst * dummy120_fst_close).values)
        if disp[-2] > 0.15:
            print('nan:{0}'.format(disp[-2]))
            filter1.append(ff)
            continue
        '''
        ...
        #    (hs300 == 1) | (zz500 == 1) |
        #    (zz1000 == 1)] * dummy120_fst_close * dummy120_fst
        #factor_data = factor_data[(zz3800 == 1)] * dummy120_fst_close * dummy120_fst
        factor_data = factor_data * dummy120_fst_close * dummy120_fst
