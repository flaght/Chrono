...
from envs.trader.cn_ashares import Long003TraderEnv as LongTraderEnv
...
def create_env(id, initial_amount, data, liaison_id, liaison_window, envoy_id,
               envoy_window, pawn_id, pawn_window, liaison_transformer_class,
               pawn_transformer_class, envoy_transformer_class, buy_cost_pct,
               sell_cost_pct, step_len, features, targets, mode, variant):
    ### 原始因子特征
    ...
    env = LongTraderEnv(id=id,
                        ...
                        envoy_id=envoy_id,
                        envoy_window=envoy_window,
                        pawn_id=pawn_id,
                        pawn_window=pawn_window,
                        df=data,
                        features=features,
                        targets=targets,
                        ...
                        mode=mode,
                        step_len=step_len,
                        liaison_transformer_class=liaison_transformer_class,
                        envoy_transformer_class=envoy_transformer_class,
                        pawn_transformer_class=pawn_transformer_class,
                        **variant)
    return env
...
def train(variant):
    ...
    envoy_id = "dubhe_{0}_{1}c_{2}".format('sicro', '5', 'hs300')
    liaison_id = "dubhe_{0}_{1}".format('sicro', 'hs300')
    pawn_id = "dubhe_{0}_{1}_{2}h".format('sicro', 'hs300', 5)
    initial_amount = 10000000
    buy_cost_pct = 1e-3
    sell_cost_pct = 1e-3
    ### 原始特征数
    env_train, _ = create_env(id='hs300_dubhe',
                              initial_amount=initial_amount,
                              envoy_window=3,
                              envoy_id=envoy_id,
                              liaison_window=2,
                              ...
                              pawn_window=1,
                              pawn_id=pawn_id,
                              liaison_transformer_class=Liaison0004,
                              envoy_transformer_class=Envoy0002,
                              pawn_transformer_class=Pawn0003,
...
def predict(variant):
    ...
    env_val_gym = create_env(id='hs300_dubhe',
                            initial_amount=initial_amount,
                            envoy_window=3,
                            envoy_id=envoy_id,
                            liaison_window=2,
    ...
    model_path = os.path.join(g_train_path, agent_model_name, "model18000")
