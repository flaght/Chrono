...
# --- 辅助函数 ---
def calculate_detailed_statistics(series, name="", confidence_level=CONFIDENCE_LEVEL):
    """计算详细统计特性，包括CV和均值置信区间"""
    if series.empty:
        return {
            'name': name, 'count': 0, 'missing_pct': 100.0 if series.isnull().all() and not series.empty else (series.isnull().mean() * 100 if not series.empty else 0),
            'mean': np.nan, 'std_dev': np.nan, 'cv': np.nan, 'median': np.nan,
            'min': np.nan, 'max': np.nan, 'skewness': np.nan, 'kurtosis (excess)': np.nan,
            'mean_ci_lower': np.nan, 'mean_ci_upper': np.nan,
            'mean_ci_abs_width': np.nan, 'mean_ci_relative_width': np.nan,
            'skew_interpretation': "N/A (empty series)", 'kurtosis_interpretation': "N/A (empty series)",
    ...
    stats = {
        ...
        'mean_ci_lower': np.nan, 'mean_ci_upper': np.nan,
        'mean_ci_abs_width': np.nan, 'mean_ci_relative_width': np.nan,
        'skew_interpretation': "N/A", 'kurtosis_interpretation': "N/A",
        'mean_median_diff_norm_by_std': np.nan
    }
    ...
    current_kurt_val = kurtosis(series_numeric_dropped_na) # scipy.stats.kurtosis is excess kurtosis

    stats.update({
        'mean': mean_val, 'std_dev': std_val, 'median': median_val,
        'min': series_numeric_dropped_na.min(), 'max': series_numeric_dropped_na.max(),
        'skewness': current_skew_val,
        'kurtosis (excess)': current_kurt_val
    })

    if abs(mean_val) < CV_VERY_LOW_MEAN_THRESHOLD: # Avoid division by zero or near-zero
        stats['cv'] = np.inf if std_val > 1e-9 else 0 # if std is also zero, cv is 0
    else:
        stats['cv'] = std_val / mean_val if mean_val != 0 else np.inf 

    if std_val > 1e-9: # Avoid division by zero if std is effectively zero
        stats['mean_median_diff_norm_by_std'] = (mean_val - median_val) / std_val
    else: # std is zero
        stats['mean_median_diff_norm_by_std'] = 0 if mean_val == median_val else (np.inf if mean_val > median_val else -np.inf)
    ...
    if stats['count'] > 1 and std_val > 0: # Need at least 2 points and some variance for CI
        ...
        if std_err_mean > 0 : # Ensure scale is positive for t.interval
            ci = t_dist.interval(confidence_level, stats['count'] - 1, loc=mean_val, scale=std_err_mean)
            stats['mean_ci_lower'] = ci[0]
            stats['mean_ci_upper'] = ci[1]
            stats['mean_ci_abs_width'] = ci[1] - ci[0]
            if abs(mean_val) > 1e-9: # Avoid division by zero for relative width
                stats['mean_ci_relative_width'] = stats['mean_ci_abs_width'] / abs(mean_val)
            else: # Mean is ~0, relative width can be huge or undefined
                stats['mean_ci_relative_width'] = np.inf if stats['mean_ci_abs_width'] > 1e-9 else 0
    ...
    elif SKEW_MODERATE_THRESHOLD <= abs_s < SKEW_HIGH_THRESHOLD: stats['skew_interpretation'] = "中等偏斜"
    else: stats['skew_interpretation'] = "高度偏斜"
    if not np.isnan(s) and abs_s >= SKEW_MODERATE_THRESHOLD: # Add direction for non-symmetrical
    ...
    if np.isnan(ek): stats['kurtosis_interpretation'] = "N/A (kurtosis is NaN)"
    elif abs(ek) < KURTOSIS_MODERATE_POSITIVE_THRESHOLD and ek >= KURTOSIS_MODERATE_NEGATIVE_THRESHOLD : stats['kurtosis_interpretation'] = "接近正态峰 (Mesokurtic-like)"
    elif ek >= KURTOSIS_HIGH_POSITIVE_THRESHOLD: stats['kurtosis_interpretation'] = "高度尖峰/厚尾 (Highly Leptokurtic)"
    elif ek >= KURTOSIS_MODERATE_POSITIVE_THRESHOLD: stats['kurtosis_interpretation'] = "中等尖峰/厚尾 (Moderately Leptokurtic)"
        ...
        if ek < -2.0 : # Example threshold for highly platykurtic, adjust as needed
            ...
            stats['kurtosis_interpretation'] = "中等平顶/薄尾 (Moderately Platykurtic)"
...
def apply_transformation(series, transform_type, original_skew, original_kurt,
    ...
    if enable_reflection_for_left_skew and is_left_skewed_significantly and needs_reflection_candidate:
        ...
        if transform_type == 'boxcox': # X' must be > 0
            ...
            reflection_constant_C = max_val + abs(min_val) + epsilon if min_val < 0 else max_val + epsilon
    ...
    try:
        ...
        if transform_type == 'log1p':
            if (current_data_to_transform < -1 + epsilon).any(): # Check x > -1 strictly
                return None, "数据(或反射后)含<=-1的值,无法log1p", lambda_param, applied_reflection
            transformed_values = np.log1p(current_data_to_transform)
            transformed_series.loc[current_data_to_transform.index] = transformed_values
            ...
            if not (current_data_to_transform > 0).all(): # Must be all strictly positive
            ...
            transformed_values, lambda_param = boxcox(current_data_to_transform)
            transformed_series.loc[current_data_to_transform.index] = transformed_values
            ...
            if applied_reflection: # If reflection was done in THIS call for YJ (which it shouldn't be for YJ)
                transformed_series = series.copy() # Reset to original series (pre-reflection for YJ)
                applied_reflection = False # YJ does its own thing
            
            non_nan_data_for_yj = transformed_series.dropna() # Use original series (or post-winsor)
            ...
            transformed_series.loc[non_nan_data_for_yj.index] = transformed_values
        else:
...
def screen_and_correct_factors(df, factor_cols):
    ...
    for factor_name in factor_cols:
        ...
        factor_report = {"actions": [], "final_status": "未知", 
                         "initial_stats_summary": None, "final_stats_summary": None,
                         "warnings": [], "transformation_applied": "无"}
        
        current_series_orig = df_corrected[factor_name].copy() 
        current_series = pd.to_numeric(current_series_orig, errors='coerce')

        print("计算初始统计...")
        initial_stats = calculate_detailed_statistics(current_series, f"{factor_name} (原始)")
        factor_report["initial_stats_summary"] = { k: initial_stats[k] for k in ['mean', 'median', 'std_dev', 'cv', 'skewness', 'kurtosis (excess)', 'missing_pct', 'count', 'mean_ci_relative_width', 'skew_interpretation', 'kurtosis_interpretation', 'mean_median_diff_norm_by_std']}
        # Print a concise summary
        print(f"初始统计: Count={initial_stats['count']}, Mean={initial_stats['mean']:.2f}, Median={initial_stats['median']:.2f}, "
        ...
        if initial_stats['count'] == 0:
            ...
            factor_report["actions"].append(message); factor_report["final_status"] = f"抛弃 ({message})"
            if factor_name in df_corrected.columns: df_corrected.drop(columns=[factor_name], inplace=True)
            describe_factor_status(factor_name, "抛弃", message); report[factor_name] = factor_report
            continue
        ...
        if nan_pct > NAN_THRESHOLD_DISCARD:
            ...
            if NAN_IMPUTE_METHOD == 'median': fill_value = initial_stats['median']
            elif NAN_IMPUTE_METHOD == 'mean': fill_value = initial_stats['mean']
            
            if not np.isnan(fill_value):
                current_series.fillna(fill_value, inplace=True)
                action_msg = f"使用{NAN_IMPUTE_METHOD} ({fill_value:.4f}) 填充缺失值"
                factor_report["actions"].append(action_msg); print(action_msg)
                ...
                if factor_name in df_corrected.columns: df_corrected.drop(columns=[factor_name], inplace=True)
                describe_factor_status(factor_name, "抛弃", "缺失值填充失败"); report[factor_name] = factor_report
                continue


        # Recalculate stats after potential imputation for subsequent checks
        stats_after_imputation = calculate_detailed_statistics(current_series, f"{factor_name} (填充后)")
        ...
        if np.isnan(current_std) or current_std < STD_DEV_THRESHOLD_DISCARD:
            ...
            continue
        # Add warnings
        if not np.isnan(current_cv) and abs(current_mean) >= CV_VERY_LOW_MEAN_THRESHOLD and current_cv > CV_HIGH_THRESHOLD:
            warn_msg = f"高变异系数(CV): {current_cv:.2f}. 相对波动性大."
            factor_report["warnings"].append(warn_msg); print(f"警告: {warn_msg}")
        if not np.isnan(current_ci_rel_width) and current_ci_rel_width > MEAN_CI_RELATIVE_WIDTH_THRESHOLD:
            warn_msg = f"均值置信区间相对宽度大: {current_ci_rel_width:.2%}. 均值估计可能不够精确."
            factor_report["warnings"].append(warn_msg); print(f"警告: {warn_msg}")
            
        # --- 开始修正流程 ---
        series_for_correction = current_series.copy() # This series is now NaN-filled
        transformation_log = []
        
        # Step A: Winsorization
        # current_skew/kurt are from *after imputation*
        if ENABLE_WINSORIZATION and (abs(current_skew) > SKEW_TARGET_MAX_ABS or abs(current_kurt) > KURTOSIS_TARGET_MAX_ABS):
            print(f"尝试Winsorization... (当前偏度:{current_skew:.2f}, 峰度:{current_kurt:.2f})")
            non_nan_indices = series_for_correction.notna() # Should be all true if imputation worked
            if non_nan_indices.any(): # Still good practice to check
                winsorized_values = winsorize(series_for_correction[non_nan_indices].values, limits=WINSORIZE_LIMITS)
                series_for_correction.loc[non_nan_indices] = winsorized_values
                
                skew_after_winsor = skew(series_for_correction.dropna()) # dropna for safety
                kurt_after_winsor = kurtosis(series_for_correction.dropna())
                action_msg = (f"Winsorization ({WINSORIZE_LIMITS}) 应用. 偏度 {current_skew:.2f}->{skew_after_winsor:.2f}, 峰度 {current_kurt:.2f}->{kurt_after_winsor:.2f}")
                transformation_log.append(action_msg); print(action_msg)
        ...
        best_transform_name_overall = factor_report["transformation_applied"] # Start with "Winsorization" or "无"
        ...
        if ENABLE_LOG_TRANSFORM and abs(current_skew) > LOG_TRANSFORM_MIN_SKEW_FOR_ATTEMPT: # Check abs for general attempt
            potential_transforms.append('log1p')
        if ENABLE_SQRT_TRANSFORM and abs(current_skew) > SQRT_TRANSFORM_MIN_SKEW_FOR_ATTEMPT:
             potential_transforms.append('sqrt')
        if ENABLE_BOXCOX_TRANSFORM and abs(current_skew) > BOXCOX_MIN_ABS_SKEW_FOR_ATTEMPT:
            potential_transforms.append('boxcox')
        if ENABLE_YEOJOHNSON_TRANSFORM and abs(current_skew) > YEOJOHNSON_MIN_ABS_SKEW_FOR_ATTEMPT:
        ...
        if potential_transforms:
            print(f"当前偏度 {current_skew:.2f} (Winsor后或填充后). 尝试形状转换: {potential_transforms}")
        ...
        for transform_type in potential_transforms:
            ...
            if transformed_s is not None:
                ...
                if abs(temp_skew_after_transform) < abs(best_transform_skew):
                    ...
                    current_best_transform_name_part = transform_type + ("(R)" if was_reflected else "")
                    # Combine with previous step if any
                    if factor_report["transformation_applied"] != "无" and factor_report["transformation_applied"] != "Winsorization":
                    ...
                    elif factor_report["transformation_applied"] == "Winsorization":
        ...
        if best_transformed_series is not None:
            ...
            final_shape_transform_name_part = best_transform_name_overall.split(' + ')[-1]
            transformation_log.append(f"应用 {final_shape_transform_name_part}: {best_transform_message}")
            factor_report["transformation_applied"] = best_transform_name_overall
            ...
            print(f"最终选择的形状转换: {final_shape_transform_name_part} (偏度: {best_transform_skew:.2f})")
        
        factor_report["actions"].extend(transformation_log)
        
        # Final stats after all transformations
        print("计算最终统计...")
        final_stats_obj = calculate_detailed_statistics(series_for_correction, f"{factor_name} (最终形态)")
        factor_report["final_stats_summary"] = { k: final_stats_obj[k] for k in ['mean', 'median', 'std_dev', 'cv', 'skewness', 'kurtosis (excess)', 'missing_pct', 'count', 'mean_ci_relative_width', 'skew_interpretation', 'kurtosis_interpretation', 'mean_median_diff_norm_by_std']}
        final_skew = final_stats_obj['skewness']
        final_kurt = final_stats_obj['kurtosis (excess)']
        print(f"最终统计: 偏度={final_skew:.2f} ({final_stats_obj['skew_interpretation']}), 峰度={final_kurt:.2f} ({final_stats_obj['kurtosis_interpretation']})")
        ...
        if passes_skew_test and passes_kurt_test:
            status_msg = "保留"
            if factor_report["transformation_applied"] != "无": status_msg += " (已修正)"
            else: status_msg += " (原始/填充后状态可接受)"
            ...
            reason_parts = []
            if not passes_skew_test: reason_parts.append(f"最终偏度 ({final_skew:.2f}, {final_stats_obj['skew_interpretation']}) 超出目标 |S|<={SKEW_TARGET_MAX_ABS}")
            if not passes_kurt_test: reason_parts.append(f"最终超额峰度 ({final_kurt:.2f}, {final_stats_obj['kurtosis_interpretation']}) 超出目标 |EK|<={KURTOSIS_TARGET_MAX_ABS}")
            ...
            if abs(final_skew) > SKEW_HIGH_THRESHOLD or abs(final_kurt) > KURTOSIS_HIGH_POSITIVE_THRESHOLD:
                biz_note = ("因子分布在修正后仍显著偏离正态。请结合业务逻辑判断此分布特性是否合理或为真实信号。")
                factor_report["warnings"].append(f"业务提示: {biz_note}"); print(f"业务提示: {biz_note}")
...
# --- 辅助函数：滚动统计分析 (可选) ---
def analyze_factor_stability_rolling(data_series, factor_name, window=50, mean_median_std_threshold=0.3):
    """对单个因子进行滚动统计分析，评估其稳定性。"""
    if data_series.count() < window * 2: # Need enough data for rolling window
        print(f"因子 {factor_name} 数据点不足 ({data_series.count()}) 以进行窗口为 {window} 的滚动分析。")
        return
    ...
    rolling_mean_median_diff_norm = np.full_like(rolling_mean, np.nan) # Initialize with NaNs
    ...
    mean_eq_median_at_zero_std = zero_std_calc_mask & (rolling_mean == rolling_median)
    mean_gt_median_at_zero_std = zero_std_calc_mask & (rolling_mean > rolling_median)
    mean_lt_median_at_zero_std = zero_std_calc_mask & (rolling_mean < rolling_median)
    ...
    rolling_mean_median_diff_norm_series = pd.Series(rolling_mean_median_diff_norm, index=data_series.index)
    plt.figure(figsize=(12, 4)) # Adjusted figure size
    rolling_mean_median_diff_norm_series.plot(label='(Rolling Mean - Rolling Median) / Rolling Std', color='green')
    plt.axhline(0, color='black', linestyle='--', linewidth=0.8)
    plt.axhline(mean_median_std_threshold, color='red', linestyle=':', linewidth=0.8, label=f'Threshold ({mean_median_std_threshold})')
    plt.axhline(-mean_median_std_threshold, color='red', linestyle=':', linewidth=0.8) # No label to avoid duplicate
    ...
    plt.tight_layout() # Ensure everything fits
    plt.show()
    print(f"观察上图：该值持续大于 {mean_median_std_threshold} 或小于 -{mean_median_std_threshold} 的区域，"
...
if __name__ == "__main__":
    ...
    print("\n开始筛选和修正因子...")
    df_corrected, report = screen_and_correct_factors(total_data1, factor_cols[0:20])
    pdb.set_trace()
    print("\n筛选和修正后的DataFrame因子描述 (部分列):")
