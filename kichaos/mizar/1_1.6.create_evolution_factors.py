...
def create_data(variant, merge=True):
    variant['g_start_pos'] = variant['g_start_pos'] - 2
    ...
    pdb.set_trace()
    return pd.concat(res, axis=0).sort_values(by=['trade_time', 'code'])
...
def evolution(variant):
    ...
    configure = {
        'n_jobs': 8,
        'population_size': tournament_size * 20,
        'tournament_size': tournament_size,
        'init_depth': 6,
...
def build(variant):
    rootid = '100001'
    basic_data = create_data(variant=variant, merge=True)
    basic_data = basic_data.sort_values(
        by=['trade_time', 'code']).set_index('trade_time')

    filename = os.path.join(base_path, 'icir', variant['method'],
    ...
    res = create_parellel(process_list=process_list,
                          ...
                          basic_factors=basic_data)
    res = list(itertools.chain.from_iterable(res))
    factors_data = pd.concat(res, axis=1)
    factors_data = factors_data.reset_index().merge(
        basic_data[['code', 'nxt1_ret', 'price']].reset_index(),
        on=['trade_time', 'code'])
    factors_data.to_feather("temp/24.feather")
...
def rolling_train(data, features, window_size):
    ...
    #normal_train = pd.concat([normal_train, returns_data], axis=1)
    normal_train = pd.concat([normal_train, returns_data, price_data], axis=1)
...
def normal1():
    factors_data = pd.read_feather("temp/24.feather")
    train1 = factors_data.set_index('trade_time').loc[
        '2024-12-30 09:01:00':'2025-04-01 23:00:00'].reset_index()
    val1 = factors_data.set_index('trade_time').loc[
        '2025-04-02 09:01:00':'2025-04-16 23:00:00'].reset_index()
    test1 = factors_data.set_index('trade_time').loc[
        '2025-04-17 09:01:00':'2025-04-30 14:58:00'].reset_index()
    features = [
    ...
    ### 修正
    train2, train_report = process_features_for_window(df=train1,
    ...
    pdb.set_trace()
    normal_train.reset_index(
        drop=True).to_feather("temp/normal_train_24.feather")
    normal_val.reset_index(drop=True).to_feather("temp/normal_val_24.feather")
    normal_test.reset_index(
        drop=True).to_feather("temp/normal_test_24.feather")
    print('-->')
