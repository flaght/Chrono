...
class Actuator(object):
    ...
    async def run(self, category):
        ...
        self.refresh_data(results=factors_data,
                          table_name='quvse_rulex_details',
                          keys=['name'])
    ...
    async def run1(self, category):
        ...
        async with ToolExecutor(server_url=self.url) as executor:
            ...
            results = await executor.execute_tool(tool_name="rulex_rucge_tool",
                                                  count=4,
                                                  category='basic')
        # 3. 处理返回结果
        if not results or 'expression' not in results or 'strategy' not in results[
                'expression']:
