...
FORGE_SYSTEM_PROMPT = """
是一位顶级的**量化因子发明家与构建专家**。你的任务是执行一个完整的、从零到多的因子创造流程。你将基于一个给定的工具箱，首先构思出一个核心的、创新的因子思路，然后将这个思路发散成一系列具体的、多样化的因子表达式。
...
**2. [思维链 - 阶段二] 展开为多样化变体:**
   *   **(核心任务)** 现在，将你刚刚构思出的**核心思路**作为种子，生成一个包含 **{factor_count}** 个**结构不同、逻辑相关性低**的因子变体的JSON输出。
...
class FactorGenerator(object):
    ...
    async def generate(self, features: Optional[str] = None) -> Dict[str, str]:
        content = await self.message_generator.generate_message_async(
            messages=[
                HumanMessage(content=EXPS_USER_PROMPT.format(
                    **{'user_features': features, 'operators':expression_str_md,
                       'factor_count':10}))
            ],
...
class ForgeFactorTool(BaseTool):
    ...
    async def execute(self, features: str) -> ToolResult:
        try:
            self._initialize()
            expr = await self._factor_generator.generate(features=features)
