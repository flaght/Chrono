...
from lib.rl013.train import train_model
from lib.rl013.predict import predict_test_set
from lib.rl013.signal import Config
from lib.rl013.custom_feature import CrossSectionalExtractor
...
def load_data_train_val(method, task_id, features, ret_name):
    ...
    train_data = pd.read_feather(os.path.join(target_dir, "train_data.feather"))
    ...
    train_data = train_data.sort_values(["trade_time", "code"]).reset_index(drop=True)
    val_data = val_data.sort_values(["trade_time", "code"]).reset_index(drop=True)
    return train_data, val_data

def load_data_test(method, task_id, features, ret_name):
    target_dir = os.path.join(base_path, method, "rl", str(task_id))
    test_data = pd.read_feather(os.path.join(target_dir, "test_data.feather"))
    pdb.set_trace()
    ...
    if use_custom_policy:
        ...
        if "policy_kwargs" in model_params:
            ...
            sac_config["policy_kwargs"] = {
                "features_extractor_class": extractor_mapping[
...
def train(method, task_id, env_id, trade_id, model_id, train_id, feature_id):
    file_dirs = os.path.join(base_path, method, 
                             "temp", "trl", task_id)
    pdb.set_trace()
    ...
    output_dir = os.path.join(base_path,method,
    ...
    return_columns = (
        train_data.filter(regex="^nxt1").columns.to_list()
        + train_data.filter(regex="^abret_").columns.to_list()
    )
    features = [f for f in train_data.columns if f not in ["trade_time", "code"] + return_columns]
    train_data = train_data[["trade_time", "code", "nxt1_ret"] + features]
    val_data = val_data[["trade_time", "code", "nxt1_ret"] + features]
...
def predict(method, task_id, env_id, trade_id, model_id, train_id, feature_id):
    ...
    best_model_zip = os.path.join(output_dir, "models", "best_model", "best_model.zip")
    best_model = os.path.join(output_dir, "models", "best_model", "best_model")
    model_path = best_model_zip if os.path.exists(best_model_zip) else best_model
