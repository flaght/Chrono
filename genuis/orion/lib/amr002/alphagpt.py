...
class AlphaGPT(nn.Module):
    ...
    def __init__(self, features_list, d_model=64, nhead=4,
                 num_layers=2, dim_feedforward=128):
        """
        Parameters
        ----------
        features_list : list[str]
            外部传入的特征列名列表, 决定词汇表的前半部分。
        """
        super().__init__()
    ...
    # ---------- layers ----------
        ...
        self.pos_emb = nn.Parameter(
            torch.zeros(1, ModelConfig.MAX_FORMULA_LEN + 1, d_model))
        ...
        self.head_actor = nn.Linear(d_model, self.vocab_size)
