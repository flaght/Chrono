...
from lib.rl003.envs import TradingEnv
...
def create_env(df: pd.DataFrame, features: List[str], config: Dict[str, Any],
...
def train_model(train_df: pd.DataFrame,
    ...
    config_to_save = {
        ...
        'sac_config': {
            k:
            str(v)
            if not isinstance(v, (int, float, bool, str, type(None))) else v
    ...
    logger.info(f"开始训练期现正套模型...")
    logger.info(f"  交易对数量: {train_env.n_pairs}")
