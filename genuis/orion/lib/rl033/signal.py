...
class Config:
    # 权重约束
    min_weight: float           # 最小权重 (正套 必须为0 不能为负)
    max_weight: float           # 单个交易对最大权重上限
...
def calculate_transaction_cost(
    ...
    sell_cost = sell_amount * (config.cost_rate)
...
def scores_to_market_neutral_weights(
    ...
    """
    Build market-neutral long/short weights from raw scores:
    - top_k highest scores: equal-weight long (+)
    - top_k lowest scores: equal-weight short (-)
    - net exposure = 0, gross exposure = 2
    """
    raw_scores = np.asarray(scores, dtype=np.float32).flatten()
    n_assets = raw_scores.size
    if n_assets == 0:
        return raw_scores

    # Ensure long and short buckets do not overlap
    max_k = max(1, n_assets // 2)
    k = int(top_k) if int(top_k) > 0 else max(1, min(20, max_k))
    k = min(k, max_k)
