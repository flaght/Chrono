...
from lib.rl033.envs import TradingEnv
from lib.rl033.signal import Config
from lib.rl033.custom_policy import CrossSectionalSACPolicy
...
def train_model(
    ...
    val_env = Monitor(raw_val_env,
                      filename=os.path.join(log_dir, 'val_monitor'))
    ...
    if use_custom_policy:
        ...
        working_sac_config['policy_kwargs'][
            'n_assets'] = raw_train_env.subset_size
        working_sac_config['policy_kwargs']['n_stock_features'] = len(features)

        logger.info(
            f"使用 CrossSectionalSACPolicy (n_assets={raw_train_env.subset_size}, n_features={len(features)})"
        )
    else:
        policy_class = 'MlpPolicy'
        logger.info(f"使用 MlpPolicy")

    model = SAC(policy=policy_class,
                env=train_env,
    ...
    if 'policy_kwargs' in safe_sac_config:
        ...
        if 'features_extractor_class' in pk and hasattr(
                pk['features_extractor_class'], '__name__'):
            pk['features_extractor_class'] = pk[
                'features_extractor_class'].__name__
    safe_sac_config = {
        k:
        str(v) if
        not isinstance(v,
                       (int, float, bool, str, type(None), dict, list)) else v
    ...
    # ── 训练 ──
    logger.info(f"开始训练 截面选股模型 (r023)...")
    ...
    logger.info(
        f"  policy: {'CrossSectionalSACPolicy' if use_custom_policy else 'MlpPolicy'}"
    )
