...
class Metrics(object):
    ...
    def quick(cls,
        ...
        ereturns = booster.yields(returns.values.copy(), dummy_vals, skip, category)
        ...
        long_weight, _, both_weight, _ = booster.direction(right, left, ereturns)

        # IC / ICIR（only both）
        _, ic_mean, ic_std = booster.correlation(both_weight, ereturns, 'both')
        ...
        result = {'ic': round(ic_mean, 6),
                  'icir': round(icir, 6),
                  'turnover': round(turnover, 6)}

        if show_log:
            print(f"[quick_ic] IC={ic_mean:.4f}  ICIR={icir:.4f}  turnover={turnover:.4f}")
