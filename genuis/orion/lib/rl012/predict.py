...
class TradingSignalGenerator:
    ...
    def _score_custom_policy(
        self, stock_features: np.ndarray, portfolio_features: np.ndarray
    ) -> np.ndarray:
        with th.no_grad():
            stocks_t = th.as_tensor(stock_features, device=self.model.device, dtype=th.float32)
            pf_t = th.as_tensor(portfolio_features, device=self.model.device, dtype=th.float32)
            scores = self.actor.score_assets(stocks_t, pf_t).detach().cpu().numpy()
    ...
    def _score_mlp_policy(
        ...
        for batch_start in range(0, n_stocks, self.subset_size):
            ...
            if batch_features.shape[0] < self.subset_size:
                pad = np.zeros(
                    (self.subset_size - batch_features.shape[0], self.n_features),
                    dtype=np.float32,
                )
                batch_features = np.vstack([batch_features, pad])

            # 构造 obs: [features.flatten(), portfolio_features]
            obs = np.concatenate([batch_features.flatten(), portfolio_features]).astype(np.float32)
            obs = obs.reshape(1, -1)
    ...
    def _score_cross_section(
        ...
        if self.use_custom_policy:
            return self._score_custom_policy(stock_features, portfolio_features)
        else:
    ...
    def predict_signals(
        ...
        for step_idx, t in enumerate(unique_times):
            ...
            code_indices = np.array([code_to_index[c] for c in codes], dtype=np.int64)
            stock_features = cs_data[self.features].values.astype(np.float32)

            # Portfolio 状态特征
            hhi_prev = float(np.sum(prev_weights ** 2))
            holding_ratio_prev = float(np.sum(prev_weights > 1e-6) / max(len(prev_weights), 1))
            portfolio_features = np.array([last_turnover, hhi_prev, holding_ratio_prev], dtype=np.float32)

            # 打分
            scores = self._score_cross_section(stock_features, portfolio_features)
            ...
            cost = calculate_transaction_cost(prev_weights, new_weights, self.signal_config)
            returns = cs_data["nxt1_ret"].values.astype(np.float32)
            portfolio_return = calculate_portfolio_return(subset_weights, returns)
