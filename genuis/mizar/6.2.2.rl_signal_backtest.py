...
def parallel_backtest(signal_data, name, parts, period, contract_multiplier,
    ...
    signal_data = signal_data.rename(columns={'net_er_out': 'value'})
...
def metrics_signal1(method, instruments, task_id, period, model_id):
    base_path1 = os.path.join(base_path, method, instruments, 'temp', 'model',
                              str(task_id), str(period), 'rl')
    dirs1 = os.path.join(base_path1, "signal", "rl", str(model_id))
...
def create_signal1(method, instruments, task_id, period, model_id):
    ...
    base_path1 = os.path.join(base_path, method, instruments, 'temp', 'model',
                              str(task_id), str(period), 'rl')
    dirs_path = os.path.join(base_path1, "composite", "model", 'rl',
                             str(model_id), 'data')
