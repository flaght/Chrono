...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, mask=None):
        ...
        Q = Q.view(bs, -1, self.n_heads, self.head_dim).permute(
            0, 2, 1, 3)  # (bs, n_heads, src_len, head_dim)
        K = K.view(bs, -1, self.n_heads, self.head_dim).permute(
            0, 2, 1, 3)  # (bs, n_heads, src_len, head_dim)
        V = V.view(bs, -1, self.n_heads, self.head_dim).permute(
            0, 2, 1, 3)  # (bs, n_heads, src_len, head_dim)

        energy = torch.matmul(Q, K.permute(
            0, 1, 3, 2)) / self.scale  # (bs, n_heads, query_len, key_len)
        energy = self.dropout(energy)
        if mask is not None:
            energy = energy.masked_fill(mask == 0, -1e10)
        attention = torch.softmax(energy,
                                  dim=-1)  # (bs, n_heads, seq_len, seq_len)
        x = torch.matmul(attention, V)  # (bs, n_heads, seq_len, head_dim)
        x = x.permute(0, 2, 1,
                      3).contiguous()  # (bs, seq_len, n_heads, head_dim)
...
class MultiheadFeedForward(nn.Module):
    ...
    def __init__(self, d_model, n_heads, ff_dim, dropout=0.1):
        ...
        self.mhfw = nn.ModuleList([
            FeedForward(d_model=self.head_dim, ff_dim=ff_dim, dropout=dropout)
    ...
    def forward(self, x):  # [bs, seq_len, d_model]
        ...
        outputs = torch.cat(outputs, dim=-2).reshape(
