...
def create_env(df: pd.DataFrame,
    ...
    env = TradingEnv(
        ...
        max_allowed_position=config['max_allowed_position'],
        use_cooldown=config['use_cooldown'],
        cooldown_steps=config['cooldown_steps'],
        masking_threshold_multiplier=config['masking_threshold_multiplier'],
        ...
        debug_daily_logs=debug_daily_logs
    )
    return env
...
def train_model(
    ...
    # 创建训练环境（带观测噪声，防过拟合）
    train_env = create_env(train_df, features, env_config, signal_config, obs_noise_std=env_config['obs_noise_std'], 
                           debug_daily_logs=debug_daily_logs)
    train_env = ResetFixWrapper(train_env)
    train_env = Monitor(train_env, filename=os.path.join(log_dir, 'train_monitor.csv'))

    # 创建校验环境（不加噪声，纯净评估）
    val_env = create_env(val_df, features, env_config, signal_config, obs_noise_std=0.0,
                         debug_daily_logs=debug_daily_logs)
    ...
    config_info = {
        ...
        'signal_config': {
            'temperature': signal_config.temperature,
            'cash_score': signal_config.cash_score,
            ...
            'score_mapping': signal_config.score_mapping,
        },
        'features': features,
        'total_timesteps': total_timesteps,
