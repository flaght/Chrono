...
def to_tick(row: pd.Series, night_session_start, exchange) -> TickData:
...
def minute_bars(csv_path: str,
    ...
    builder.flush()
    dt = pd.DataFrame(builder.result_bars)
    dt = dt.sort_values(by='datetime') if not dt.empty else dt
    return dt
