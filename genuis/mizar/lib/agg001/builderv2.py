...
def _prepare_tick_datetimes(
    ...
    trading_day_series = df["TradingDay"].astype(int).astype(str)
    trading_dates = pd.to_datetime(trading_day_series, format="%Y%m%d")
    update_times = pd.to_datetime(df["UpdateTime"], format="%H:%M:%S")
    update_clock = update_times.dt.time
    ...
    natural_dates = trading_dates.copy()

    # Some raw files keep evening ticks at TradingDay itself, others already
    # shift evening ticks to the previous trading day. Normalize both cases.
    natural_dates.loc[night_mask & trading_dates.eq(anchor_ts)] = previous_anchor_ts
    natural_dates.loc[night_mask & trading_dates.eq(previous_anchor_ts)] = previous_anchor_ts

    # After midnight, some files keep TradingDay at the anchor date already,
    # while others still carry the previous trading date. Normalize to anchor.
    natural_dates.loc[overnight_mask & trading_dates.eq(previous_anchor_ts)] = anchor_ts
    natural_dates.loc[overnight_mask & trading_dates.eq(anchor_ts)] = anchor_ts
