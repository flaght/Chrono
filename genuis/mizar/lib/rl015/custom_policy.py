...
class TCNFeaturesExtractor(BaseFeaturesExtractor):
    """将 (分钟窗口, 特征) 编码为 SAC 可消费的一维状态向量。"""
    ...
    def __init__(
        ...
        if len(observation_space.shape) != 2:
            raise ValueError(
                "TCN observation 必须是 (lookback, n_features)，"
                f"实际为 {observation_space.shape}"
            )
        super().__init__(observation_space, features_dim)
        n_features = int(observation_space.shape[1])
        blocks = []
    ...
    def forward(self, observations: th.Tensor) -> th.Tensor:
        # SB3: (batch, lookback, features) -> Conv1d: (batch, features, lookback)
...
class ResNetActor(BasePolicy):
    ...
    3. 输出连续动作值；环境经 Softmax 转为权重，再计算连续预测信号 p_up-p_down。
    """
    def __init__(
        self,
