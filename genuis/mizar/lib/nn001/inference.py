...
def predict_store(model: th.nn.Module, store: SequenceStore,
    ...
    """对完整数据按品种批量推理，保留无效标签行以维持 rolling 位置。"""
    if int(batch_size) <= 0:
    ...
    with th.no_grad():
        for code in store.asset_codes:
            ...
            frames.append(pd.DataFrame({
                "trade_time": store.trade_times[positions],
                "code": code,
    ...
    if not frames:
        raise ValueError("没有可推理的数据")
    return pd.concat(frames, ignore_index=True)


def model_config_from_checkpoint(checkpoint: Dict) -> Dict:
    required = {"n_features", "channels", "kernel_size", "num_blocks",
                "features_dim", "head_hidden", "dropout",
