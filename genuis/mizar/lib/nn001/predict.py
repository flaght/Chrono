...
def predict_test_set(model_path: str, config_path: str,
    ...
    model = TCNRegressor(**architecture).to(runtime_device)
    ...
    batch_size = int(inference_batch_size or
                     train_config.get("inference_batch_size", 8192))
    print(f"[TCN_MSE_TEST_INFERENCE] rows={len(test_df)} "
    ...
    metrics["ic_sequence"].to_csv(
        os.path.splitext(output_path)[0] + "_rolling_ic.csv", index=False)
    print(f"[TCN_MSE_TEST_END] score={summary['selection_score']:.6f} "
