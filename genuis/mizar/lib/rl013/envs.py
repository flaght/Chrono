...
def target_transform(
    ...
    s = pd.Series(raw_target, dtype="float64")
    s = s.replace([np.inf, -np.inf], np.nan)
    mu = s.rolling(window=roll_win).mean().shift(shift)
    sg = s.rolling(window=roll_win).std(ddof=0).shift(shift)
    
    out = (s - mu) / (sg + eps)
    out = out.clip(-clip, clip) / clip
    
    return out.fillna(0.0).to_numpy(dtype=np.float32)
...
class TradingEnv(gym.Env):
    ...
    def __init__(self, df: pd.DataFrame, features: List[str], config: Dict[str, Any]):
        ...
        self.future_ret_target = target_transform(
            self.future_ret_h,
            shift=self.env_config['shift']
        )
    ...
    def step(self, action: np.ndarray):
        ...
        target_ret = float(self.future_ret_target[self.current_step]) if can_open else 0.0
