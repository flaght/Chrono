...
核心设计:
  ...
  - 收益定义 (log return):
      ...
      正值 = 正套盈利, 无需取负
  - 成本 = 现货手续费 + 期货手续费 (基于权重变化)

数据格式:
  面板数据 DataFrame 包含:
    - trade_time: 时间
    - pair_id: 交易对 (如 'BTC', 'ETH')
...
from lib.rl003.signal import Config, action_to_weights,calculate_turnover,calculate_transaction_cost,calculate_arbitrage_return
...
class TradingEnv:
    ...
    def __init__(self,
        ...
        """
        Args:
            df: 面板数据, 必须包含 [trade_time, pair_id, log_return] + features
            features: 因子特征列名
        ...
        # 验证数据
        required_cols = ['trade_time', 'log_return']
        ...
        # funding_rate 列可选
        self.has_funding = 'funding_rate' in self.df.columns
        ...
        # 交易对
        if 'pair_id' in self.df.columns:
            self.pair_ids = sorted(self.df['pair_id'].unique())
            self.n_pairs = len(self.pair_ids) if n_pairs == 0 else n_pairs
        else:
            self.pair_ids = ['pair_0']
            self.n_pairs = 1
        ...
        self.n_state_features = self.n_pairs * (self.n_features + 1) + self.n_portfolio_features
    ...
    def _build_panel_index(self):
        ...
        if 'pair_id' in self.df.columns:
            for t_idx, t in enumerate(self.unique_times):
                ...
                time_data = time_data.sort_values('pair_id').reset_index(drop=True)
    ...
    def observation(self, time_index: int) -> np.ndarray:
        ...
        # 交易对因子特征
        pair_features = cs_data[self.features].values.astype(np.float32)  # (N, n_features)
        
        # 填充或截断
        if len(pair_features) < self.n_pairs:
            pad = np.zeros((self.n_pairs - len(pair_features), self.n_features), dtype=np.float32)
        ...
        pair_obs = np.hstack([pair_features, weights_col])  # (N, n_features + 1)
        pair_obs_flat = pair_obs.flatten()
        
        # 组合状态特征
        hhi = float(np.sum(self.current_weights ** 2))
        holding_ratio = float(np.sum(self.current_weights > 0.001)) / max(self.n_pairs, 1)
        ...
        portfolio_features = np.array(
            [turnover, hhi, holding_ratio, weighted_basis], 
            dtype=np.float32
        )
    ...
    def step(self, action: np.ndarray) -> Tuple[np.ndarray, float, bool, Dict[str, Any]]:
        ...
        if len(cs_data) > 0 and 'log_return' in cs_data.columns:
            log_returns = cs_data['log_return'].values.astype(np.float32)
            log_returns = self._pad_or_truncate(log_returns, self.n_pairs)
        else:
            log_returns = np.zeros(self.n_pairs, dtype=np.float32)
        
        # ========== 4. 计算正套收益 ==========
        # log_return 已经是正确符号: 正值 = 正套盈利
        arb_return = calculate_arbitrage_return(new_weights, log_returns)
        
        # ========== 5. 资金费率收益 (可选) ==========
        funding_return = 0.0
        if self.has_funding and len(cs_data) > 0 and 'funding_rate' in cs_data.columns:
            funding_rates = cs_data['funding_rate'].values.astype(np.float32)
            funding_rates = self._pad_or_truncate(funding_rates, self.n_pairs)
            # 正套做空期货, 当 funding_rate > 0 时收取资金费
            funding_return = float(np.dot(new_weights, funding_rates))
        
        # ========== 6. 计算奖励 ==========
        reward = arb_return + funding_return - cost
        ...
        self.total_arb_return += arb_return + funding_return
