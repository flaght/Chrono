...
from lib.rl011.custom_feature import TCNFeatureExtractor
...
def _normalize_signal_config(signal_config: Optional[Any]) -> Dict[str, Any]:
    ...
    if isinstance(signal_config, dict):
        ...
        source = {
            key: getattr(signal_config, key)
            for key in ("discrete_mode", "discrete_threshold")
            if hasattr(signal_config, key)
        }
    out = {}
    if "discrete_mode" in source:
        out["discrete_mode"] = bool(source["discrete_mode"])
    if "discrete_threshold" in source:
        out["discrete_threshold"] = float(source["discrete_threshold"])
    return out
...
def create_env(
    ...
    config = {
        ...
        "signal_config": _normalize_signal_config(signal_config)
    }
