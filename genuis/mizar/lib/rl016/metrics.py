"""与既有因子评估一致的重采样 rolling IC 及预测诊断指标。"""
...
def evaluate_predictions(records: pd.DataFrame,
    ...
    严格采用既有评估口径：
      ...
      minute % resampling_minutes == 0
      future_ret_h.rolling(roll_window, min_periods).corr(predicted_z)

    完整校验每个品种是一段连续评估序列；快速校验每个固定短窗口是一段，
    不允许 rolling 窗口跨越两个不连续的固定窗口。
    """
    ...
    for code, asset in frame.groupby("code", sort=True):
        ...
        assets[str(code)] = {
            ...
            "mse_raw_return": (float(np.mean(error ** 2))
