"""加载 HybridTransformer-Loss 最佳模型并批量评估测试集。"""
...
def predict_test_set(model_path: str, config_path: str,
    ...
    if checkpoint.get("version") != "hybrid_transformer_loss_v1":
        raise ValueError(
            f"检查点版本不匹配: {checkpoint.get('version')!r}")
    if "loss_config" not in checkpoint:
        raise ValueError("检查点缺少 loss_config，不能确认训练目标")
    ...
    print(f"[HYBRID_TRANSFORMER_LOSS_TEST_INFERENCE] rows={len(test_df)} "
    ...
    summary = {key: value for key, value in metrics.items()
               if key != "ic_sequence"}
    summary["loss_config"] = dict(checkpoint["loss_config"])
    ...
    print(f"[HYBRID_TRANSFORMER_LOSS_TEST_END] "
