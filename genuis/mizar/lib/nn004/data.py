"""HybridTransformer-Loss 的分钟级多资产因果时序批次。"""
...
class SequenceStore:
    ...
    def __init__(self, frame: pd.DataFrame, features: List[str], lookback: int,
        ...
        self.asset_id_by_code = {
            code: index for index, code in enumerate(self.asset_codes)}
        self.asset_ids = np.asarray(
            [self.asset_id_by_code[str(code)] for code in self.codes],
            dtype=np.int64)
        self.code_positions = {
    ...
    def asset_id_batch(self, positions: np.ndarray) -> np.ndarray:
        """返回每条样本的整数品种标识，供逐品种损失使用。"""
        positions = np.asarray(positions, dtype=np.int64).reshape(-1)
        return self.asset_ids[positions]
...
def balanced_batches(store: SequenceStore, batch_size: int, epoch: int,
    ...
    if len(codes) < 2 or int(batch_size) < 2 * len(codes):
        raise ValueError("至少需要两个品种，且每个品种每批至少需要2条样本")
    ...
    while produced < int(samples_per_epoch):
        remaining_total = int(samples_per_epoch) - produced
        # 相关性至少需要每个品种2条；极小尾批直接舍弃，避免无效相关系数。
        if remaining_total < 2 * len(codes):
            break
        width = min(int(batch_size), remaining_total)
        if width < 2 * len(codes):
            break
