"""多资产 Transformer-MSE 训练和滚动 IC 选模。"""
...
def _architecture(n_features, lookback, prediction_bound_std, config):
    return {
        "n_features": int(n_features), "lookback": int(lookback),
...
def _save_checkpoint(path, model, optimizer, architecture, features,
    ...
    th.save({
        "version": "transformer_mse_v1",
        "objective": "standardized_return_mse",
        "model_state_dict": model.state_dict(),
        "optimizer_state_dict": optimizer.state_dict(),
        "model_config": architecture, "features": list(features),
        "lookback": int(lookback), "ret_scale_by_code": dict(scales),
        "epoch": int(epoch), "selection_score": float(score),
...
def train_model(train_df: pd.DataFrame, val_df: pd.DataFrame,
                ...
                output_dir: str) -> Tuple[TransformerRegressor, Dict[str, Any]]:
    if not output_dir:
    ...
    model = TransformerRegressor(**architecture).to(device)
    ...
    model_dir, log_dir = (os.path.join(output_dir, "models"),
    ...
    run_config = {
        ...
        "ret_scale_by_code": train_store.ret_scale_by_code,
        "env_config": dict(env_config), "model_config": dict(model_config),
        "architecture": architecture, "train_config": dict(train_config),
        "train_size": int(len(train_store.df)),
        "val_size": int(len(val_store.df)), "device": str(device),
    ...
    for epoch in range(1, epochs + 1):
        ...
        for positions in balanced_batches(
            ...
            if now - progress_at >= progress_seconds:
                print(f"[TRANSFORMER_MSE_PROGRESS] epoch={epoch}/{epochs} "
        ...
        print(f"[TRANSFORMER_MSE_EPOCH] epoch={epoch}/{epochs} "
        ...
        result = _evaluate(model, val_store, inference_batch_size, device,
                           ...
                           f"TRANSFORMER_MSE_VAL_E{epoch}")
        score = float(result["selection_score"])
        eligible = np.isfinite(score) and bool(result["assets"])
        for metrics in result["assets"].values():
            eligible = (eligible and metrics["valid_rolling_ic"] >= min_valid_ic
                        and metrics["prediction_std"] > 1e-12)
        ...
        if improved:
            ...
            print(f"[TRANSFORMER_MSE_BEST] epoch={epoch} score={score:.6f} "
        ...
        if eligible:
            ...
            if eval_count >= min_evals and no_improve_count >= patience:
                stopped_early = True
                print(f"[TRANSFORMER_MSE_EARLY_STOP] epoch={epoch} "
    ...
    best_model = TransformerRegressor(**checkpoint["model_config"]).to(device)
    best_model.load_state_dict(checkpoint["model_state_dict"])
    final_result = _evaluate(best_model, val_store, inference_batch_size,
                             device, resampling, roll_window, min_periods,
                             "TRANSFORMER_MSE_FINAL")
    ...
    info = {"best_model_path": best_path, "final_model_path": final_path,
    ...
    print(f"[TRANSFORMER_MSE_END] best_epoch={best_epoch} "
