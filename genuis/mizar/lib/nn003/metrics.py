...
def evaluate_predictions(records: pd.DataFrame, resampling_minutes: int = 5,
    ...
    for code, asset in frame.groupby("code", sort=True):
        ...
        sampled_asset = (pd.concat(pieces, ignore_index=True)
        ...
        rolling = (sampled_asset.rolling_pearson_ic
                   if not sampled_asset.empty else pd.Series(dtype=float))
        ...
        assets[str(code)] = {
            ...
            "rolling_pearson_ic_std": (float(rolling.std(ddof=1))
                                       if rolling.notna().sum() > 1 else np.nan),
            "rolling_pearson_positive_rate": (
                float((rolling.dropna() > 0).mean())
                if rolling.notna().any() else np.nan),
            ...
            "mse_skill_vs_zero": (float(1 - model_mse / zero_mse)
    ...
    return {
        ...
        "selection_score": (float(min(finite))
                            if len(finite) == len(assets) and assets else np.nan),
