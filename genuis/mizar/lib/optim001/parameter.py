...
class ParameterOptimizer:
    ...
    def suggest_parameters(
        ...
        trial: optuna.Trial,
        expression: str,
        param_choices: list = [5, 10, 15, 30, 60, 90, 120,
                               240]) -> Dict[str, Any]:
        """
        为表达式中的参数提供建议值
        ...
        '''
        for param in parsed['parameters']:
            suggestions[f'param_{param}'] = trial.suggest_categorical(
                f'param_{param}', param_choices)
        return suggestions
    ...
    def replace_parameters(self, expression: str,
        ...
        sorted_params = sorted(param_mapping.items(),
                               key=lambda x: x[0],
                               reverse=True)
