"""Transformer-MSE 调用端：RB 为主、HC 为辅的多资产监督基线。"""
...
from lib.uvx import *
from lib.nn002.predict import predict_test_set
from lib.nn002.train import train_model
...
def _validate_core(period, trade_params, env_params):
    if int(period) != 5 or trade_params["ret_name"] != "nxt1_ret_5h":
        raise ValueError(
            "Transformer-MSE 要求 period=5、ret_name=nxt1_ret_5h")
    if not bool(env_params.get("use_transformer", True)):
        raise ValueError("Transformer-MSE 要求 use_transformer=true")


def _result_dir(method, instruments, task_id, period, name):
    return os.path.join(base_path, method, instruments, "temp", "model",
                        str(task_id), str(period), "rl", "transformer_mse",
                        "result", str(name))
...
def train(method, instruments, task_id, period, env_id, trade_id, model_id,
    ...
    logger.info(f"Transformer-MSE训练集: {len(train_data)} 行，RB={len(rb_train)} HC={len(hc_train)}")
    logger.info(f"Transformer-MSE校验集: {len(val_data)} 行，RB={len(rb_val)} HC={len(hc_val)}")
...
def predict(method, instruments, task_id, period, env_id, trade_id, model_id,
    ...
    if not os.path.isfile(model_path):
        raise FileNotFoundError(
            f"Transformer-MSE 最佳模型不存在: {model_path}")
    if not os.path.isfile(config_path):
        raise FileNotFoundError(
            f"Transformer-MSE 训练配置不存在: {config_path}")
    output_path = os.path.join(output_dir, "metrics", "test_results.csv")
    print(f"[TRANSFORMER_MSE_TEST_START] RB={len(rb)} HC={len(hc)}")
