"""TCN-MSE 调用端：RB 为主、HC 为辅的多资产监督学习基线。"""
...
from lib.uvx import *
from lib.nn001.predict import predict_test_set
from lib.nn001.train import train_model
...
def load_data1(method, instruments, task_id, period, features, regime,
               ret_name):
    """读取上游已经切好的 train/val，不重新切割数据。"""
    train = _load_split(
        method, instruments, task_id, period, "train", features, regime,
        ret_name, expected_code="RB" if instruments == "rbb" else "HC")
    expected_code = str(train["code"].iloc[0])
    val = _load_split(
        method, instruments, task_id, period, "val", features, regime,
        ret_name, expected_code=expected_code)
...
def _validate_core(period, trade_params, env_params):
    if int(period) != 5 or trade_params["ret_name"] != "nxt1_ret_5h":
        raise ValueError("TCN-MSE 当前要求 period=5、ret_name=nxt1_ret_5h")
    if not bool(env_params.get("use_tcn", True)):
        raise ValueError("TCN-MSE 要求 use_tcn=true")


def _result_dir(method, instruments, task_id, period, name):
    return os.path.join(base_path, method, instruments, "temp", "model",
                        str(task_id), str(period), "rl", "tcn_mse", "result",
                        str(name))
...
def train(method, instruments, task_id, period, env_id, trade_id, model_id,
    ...
    output_dir = _result_dir(
        method, instruments, task_id, period, name)
    ...
    logger.info(f"TCN-MSE训练集: {len(train_data)} 行，RB={len(rb_train)} HC={len(hc_train)}")
    logger.info(f"TCN-MSE校验集: {len(val_data)} 行，RB={len(rb_val)} HC={len(hc_val)}")
    ...
    model, training_info = train_model(
        ...
        train_config=copy.deepcopy(train_params),
...
def predict(method, instruments, task_id, period, env_id, trade_id, model_id,
    ...
    rb = load_test_data(
        method, instruments, task_id, period, selected_features, min_regime,
        trade_params["ret_name"], expected_code="RB")
    hc = load_test_data(
        method, right_instruments, right_task, period, selected_features,
        min_regime, trade_params["ret_name"], expected_code="HC")
    model_path = os.path.join(output_dir, "models", "best_model.pt")
    config_path = os.path.join(output_dir, "config.json")
    if not os.path.isfile(model_path):
        raise FileNotFoundError(f"TCN-MSE 最佳模型不存在: {model_path}")
    if not os.path.isfile(config_path):
        raise FileNotFoundError(f"TCN-MSE 训练配置不存在: {config_path}")
    test_data = _merge_assets(rb, hc)
    output_path = os.path.join(output_dir, "metrics", "test_results.csv")
    print(f"[TCN_MSE_TEST_START] RB={len(rb)} HC={len(hc)}")
    return predict_test_set(
        model_path=model_path, config_path=config_path,
        test_df=test_data, output_path=output_path,
        inference_batch_size=int(
            train_params.get("inference_batch_size", 8192)),
