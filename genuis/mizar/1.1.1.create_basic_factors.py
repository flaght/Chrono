...
def recent(method, instruments, source):
    ...
    features = [col for col in total_data.columns if col not in ['trade_time','code'] + nxt1_columns]
    ...
    recent_data.reset_index(drop=True).to_feather(
        os.path.join(os.path.join(base_path, method, instruments, 'basic'), 'recent_data.feather'))
    
    recent_returns.reset_index(drop=True).to_feather(
        os.path.join(os.path.join(base_path, method, instruments, 'returns'), 'recent_returns.feather'))
