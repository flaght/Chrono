...
_PARALLEL_TOTAL_DATA1 = None
_PARALLEL_PERIOD = None

def _parallel_evaluate1_shared(column):
    return parallel_evaluate1(column=column)

def parallel_evaluate1(column, total_data=None, total_data1=None, period=None):
    if isinstance(column, dict):
        column = SimpleNamespace(**column)
    print(column.formual)
    total_data = _PARALLEL_TOTAL_DATA if total_data is None else total_data
    total_data1 = _PARALLEL_TOTAL_DATA1 if total_data1 is None else total_data1
    period = _PARALLEL_PERIOD if period is None else period
...
@add_process_env_sig
def run_evalute(target_column, total_data, period):
    state_dt = run_process(
        target_column=target_column,
        callback=parallel_evaluate1,
...
def valid_programs(method,
    ...
    programs = programs.loc[:4]
    #program_rows = [program for program in programs.itertuples()]
    #program_rows = list(program_rows)
    program_rows = programs[["name", "formual", "final_fitness"]].to_dict(
        orient="records")
    total_data1 = total_data.set_index(['trade_time'])
    
    
    global _PARALLEL_TOTAL_DATA, _PARALLEL_TOTAL_DATA1, _PARALLEL_PERIOD
    _PARALLEL_TOTAL_DATA = total_data
    _PARALLEL_TOTAL_DATA1 = total_data1
    _PARALLEL_PERIOD = period
    
    if len(program_rows) <= 1:
        res = [_parallel_evaluate1_shared(program_rows[0])] if program_rows else []
    else:
        try:
        ...
        with ctx.Pool(processes=2) as pool:
            res = pool.map(_parallel_evaluate1_shared, program_rows)
    
    
    #k_split = 2
    # programs = [program for program in programs.itertuples()]
    # programs_list = split_k(k_split, programs)
    # res = create_parellel(process_list=programs_list,
    #                     #   total_data1=total_data1,
    #                       total_data=total_data,
    #                       period=period,
    ...
    res = Parallel(n_jobs=2, verbose=1)(
        delayed(parallel_evaluate1)(column=program,
                                    total_data=total_data,
                                    total_data1=total_data1,
                                    period=period) for program in program_rows)
