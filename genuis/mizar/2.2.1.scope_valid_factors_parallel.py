...
def parallel_evaluate1(column, period):
...
def valid_programs(method,
    ...
    args_for_starmap = zip(program_rows, itertools.repeat(period))
    if len(program_rows) <= 1:
        res = [parallel_evaluate1(program_rows[0], period=period)] if program_rows else []
    else:
        try:
