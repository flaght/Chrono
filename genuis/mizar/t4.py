...
def build_tick_datetimes(df: pd.DataFrame) -> pd.Series:
    trading_dates = pd.to_datetime(df["TradingDay"].astype(str), format="%Y%m%d")
    update_times = pd.to_datetime(df["UpdateTime"], format="%H:%M:%S")
    is_night = update_times.dt.time >= NIGHT_SESSION_START
    natural_dates = trading_dates.where(~is_night, trading_dates - pd.Timedelta(days=1))
...
def aggregate_minute_bars(csv_path: str) -> pd.DataFrame:
    ...
    df["datetime"] = build_tick_datetimes(df)
    df["minute"] = df["datetime"].dt.floor("min")

    df = df.sort_values(
        ["datetime", "UpdateMillisec"],
        kind="stable",
    ).reset_index(drop=True)

    grouped = df.groupby("minute", sort=True, as_index=False)
    bars = grouped.agg(
        symbol=("InstrumentID", "last"),
        open=("LastPrice", "first"),
        high=("LastPrice", "max"),
        low=("LastPrice", "min"),
        close=("LastPrice", "last"),
        end_volume=("Volume", "last"),
        end_turnover=("Turnover", "last"),
        open_interest=("OpenInterest", "last"),
    )

    bars["prev_end_volume"] = bars["end_volume"].shift(1)
    bars["prev_end_turnover"] = bars["end_turnover"].shift(1)
    bars["volume"] = (bars["end_volume"] - bars["prev_end_volume"]).clip(lower=0)
    bars["value"] = (bars["end_turnover"] - bars["prev_end_turnover"]).clip(lower=0)

    if DROP_FIRST_PARTIAL_BAR and not bars.empty:
        bars = bars.iloc[1:].copy()
    elif not bars.empty:
        bars["volume"] = bars["volume"].fillna(0.0)
        bars["value"] = bars["value"].fillna(0.0)

    bars["vt_symbol"] = bars["symbol"] + ".SHFE"
    bars["exchange"] = "SHFE"
    bars["date"] = bars["minute"].dt.strftime("%Y-%m-%d")
    bars["time"] = bars["minute"].dt.strftime("%H:%M:%S")
    bars["datetime"] = bars["minute"].dt.strftime("%Y-%m-%d %H:%M:%S")
    bars["vwap"] = bars["close"]

    volume_mask = bars["volume"] > 0
    bars.loc[volume_mask, "vwap"] = (
        bars.loc[volume_mask, "value"] / bars.loc[volume_mask, "volume"] / RB_CONTRACT_MULTIPLIER
    )
