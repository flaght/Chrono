...
def to_tick(row: pd.Series) -> TickData:
    ...
    # CTP TradingDay is the settlement/trading date. For night-session ticks
    # after 20:00, the natural date is the previous calendar day.
    if update_time >= NIGHT_SESSION_START:
        natural_date = trading_date - dt.timedelta(days=1)
    else:
