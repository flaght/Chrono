...
def _pd_rolling(pd_object, pd_object_cm, how, *args, **kwargs):
    ...
    if g_pandas_has_rolling:
        ...
        for key, value in kwargs.items():
            # pct 参数属于 rank 方法
            if how == 'rank' and key in ['pct', 'method', 'na_option', 'ascending']:
                method_kwargs[key] = value
            else:
                rolling_kwargs[key] = value

        rolling_obj = pd_object.rolling(*args, **rolling_kwargs)
        if hasattr(rolling_obj, how):
            if pd_object_cm is None:
                method = getattr(rolling_obj, how)
...
#pd_rolling_quantile = partial(_pd_rolling, how='quantile', pd_object_cm=None)
...
pd_ewm_sum = partial(_pd_ewm, how='sum', pd_object_cm=None)
#pd_ewm_skew = partial(_pd_ewm, how='skew', pd_object_cm=None)
#pd_ewm_max = partial(_pd_ewm, how='max', pd_object_cm=None)
#pd_ewm_min = partial(_pd_ewm, how='min', pd_object_cm=None)
...
pd_rolling_corr = partial(_pd_rolling, how='corr')
pd_rolling_cov = partial(_pd_rolling, how='cov')
pd_rolling_rank = partial(_pd_rolling, how='rank', pd_object_cm=None)
