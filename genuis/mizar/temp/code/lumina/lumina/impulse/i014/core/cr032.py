...
def cr032(close, high, low, volume, window, weriod, ewm=False):
    ...
    # 三阶混合极差（近似实现：三变量中心化后乘积的rolling极差）
    ...
    #mix_range = mix_prod.rolling(weriod).max() - mix_prod.rolling(weriod).min()
    mix_range = roller_max(mix_prod, window=weriod,
                               min_periods=1) - roller_min(
                                   mix_prod, window=weriod, min_periods=1)
