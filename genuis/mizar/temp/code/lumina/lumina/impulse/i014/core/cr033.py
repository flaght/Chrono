...
def cr033(close, high, low, volume, window, weriod, ewm=False):
    ...
    # 三阶混合分布熵（近似实现：三变量中心化后乘积的rolling归一化概率分布熵）
    ...
    # 归一化概率
    #prob = (mix_prod - mix_prod.rolling(weriod).min()) / (
    #    mix_prod.rolling(weriod).max() - mix_prod.rolling(weriod).min() + 1e-8)

    prob = (mix_prod - roller_min(mix_prod, weriod, 1, 'rolling')
            ) / (roller_max(mix_prod, weriod, 1, 'rolling') -
                 roller_min(mix_prod, weriod, 1, 'rolling') + 1e-8)

    prob = prob.clip(1e-8, 1)
    #entropy = -(prob * np.log(prob)).rolling(weriod).sum()
