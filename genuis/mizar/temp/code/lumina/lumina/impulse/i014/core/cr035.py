...
def cr035(close, high, low, volume, window, weriod, ewm=False):
    ...
    # 三阶混合变异系数（近似实现：三变量中心化后乘积的rolling均值/rolling标准差）
    ...
    mix_mean = roller_mean(mix_prod, weriod, 1, method)
    mix_std = roller_std(mix_prod, weriod, 1, method)
    #mix_mean = mix_prod.rolling(weriod).mean()
    #mix_std = mix_prod.rolling(weriod).std()
    factor = roller_mean(mix_mean / (mix_std + 1e-8), window, 1, method)
    return factor
