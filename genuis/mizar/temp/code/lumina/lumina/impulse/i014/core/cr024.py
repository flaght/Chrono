...
def cr024(high, low, close, window, weriod, ewm=False):
    ...
    #kurt = price_range.rolling(weriod).kurt()
    kurt = roller_kurt(price_range, weriod, 1, 'rolling')
    #vol = close.rolling(weriod).std()
    vol = roller_std(close, weriod, 1, method)
    #cov = kurt.rolling(weriod).cov(vol)
    cov = roller_cov(kurt, vol, weriod, 1, method)
