...
def cr057(close, high, low, openint, window, weriod, ewm=False):
    ...
    sign_change = mix_prod.rolling(weriod).apply(lambda x: np.sum(np.diff(np.sign(x)) != 0) / (len(x) - 1 + 1e-8), raw=True)
    factor = roller_mean(sign_change, window, 1, method)
    return factor 