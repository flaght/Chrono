...
class FactorDisplaceBollSell(FactorSellID):
        ...
        if len(ma_line) > int(self.std_xd):
            if self.ewm == 1:
                band = pd_ewm_std(self.xd_kl.close,
                                  span=int(self.std_xd),
                                  min_periods=1,
                                  adjust=False)
            else:
                band = pd_rolling_std(self.xd_kl.close,
                                      window=int(self.std_xd),
                                      min_periods=1,
                                      center=False)
            ...
            for order in orders:
                if order.expect_direction == 1 and bar.low < disp_bot.iloc[-1]:
                    self.sell_next(order)
                elif order.expect_direction == -1 and bar.high > disp_top.iloc[
                        -1]:
                    self.sell_next(order)
