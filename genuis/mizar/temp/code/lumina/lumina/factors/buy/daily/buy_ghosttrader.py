...
class FactorGhostTraderBuy(FactorBuyXD):
...
class FactorGhostTraderBuyL(FactorGhostTraderBuy, BuyCallMixin):
    ...
    def fit_day(self, today):
        ...
        # 计算RSI
        rsi = _calc_rsi_from_pd(self.xd_kl.close, 
                                self.rsi_period)
        ...
        if fast_line[-1] > slow_line[-1] and rsi[-1] < self.over_bought \
            and today.high > self.xd_kl.high[-2]:
            return self.buy_tomorrow()

class FactorGhostTraderBuyS(FactorGhostTraderBuy, BuyCallMixin):
    ...
    def fit_day(self, today):
        ...
        if fast_line[-1] < slow_line[-1] and rsi[-1] > self.over_sold \
            and today.low < self.xd_kl.low[-2]:
            return self.buy_tomorrow()