...
from lumina.factors.buy.fixes import BuyCallMixin, FactorBuyXD
from ultron.ump.technical.line import Line
from ultron.ump.technical.golden import create_golden


class FactorUpDownTrend(FactorBuyXD, BuyCallMixin):
    ...
    def _init_self(self, **kwargs):
        ...
        if 'xd' not in kwargs:
            # 如果外部没有设置xd值，默认给一个30
            kwargs['xd'] = 20
        super(FactorUpDownTrend, self)._init_self(**kwargs)
...
class FactorDownUpTrend(FactorBuyXD, BuyCallMixin):
    ...
    def _init_self(self, **kwargs):
        ...
        super(FactorDownUpTrend, self)._init_self(**kwargs)
...
class FactorUpDownGolden(FactorUpDownTrend, BuyCallMixin):
    ...
    def fit_day(self, today):
        ...
        if tl_long.is_up_trend(up_deg_threshold=self.up_deg_threshold, show=False):
            ...
            golden = create_golden(self.xd_kl, show=False)
            if today.pre_close < golden.below382 < today.close and Line(
