...
from ultron.ump.ump.manager import Manager as UmpManager
from ultron.ump.agent.manager import Manager as AgentManager
...
class FactorBuyBase(six.with_metaclass(ABCMeta, ParamBase)):
    ...
    def __init__(self, capital, kl_pd, combine_kl_pd, benchmark, **kwargs):
        ...
        self.ump_manger = UmpManager(factor=self)
        # 构造agent对外的接口对象AgentManager
        self.agent_manger = AgentManager(factor=self)
    ...
    def make_buy_order(self, day_ind=-1):
        ...
        if order.order_deal:
            ...
            # 决策交易是否被agent拦截还是可以放行
            block = self.make_agent_block_decision(ml_feature_dict)
            if block:
                return None
            # 决策交易是否被ump拦截还是可以放行
    ...
    def make_agent_block_decision(self, ml_feature_dict):
        """
        输入需要决策的当前买入交易特征通过agent模块的对外manager对交易进行决策，
        判断是否拦截买入交易，还是放行买入交易。子类可复写此方法，即子类策略因子实现
        自己的任意ump组合拦截方式，根据策略的拦截比例需要等等参数确定ump具体策略，
        且对于多种策略并行执行策略本身定制适合自己的拦截策略，提高灵活度
        :param ml_feature_dict: 需要决策的当前买入时刻交易特征dict
        :return: bool, 对ml_feature_dict所描述的交易特征是否进行拦截
        """
        return self.agent_manger.agent_block(ml_feature_dict)
