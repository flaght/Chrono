...
# 默认参数范围设置
# =============================
default_open_interest_window_range = [x for x in range(5, 30, 2)
                                      ]  # 均值窗口 5,10,...,60
default_max_volume_range = [1]#[1, 2, 3]  # 最大持仓手数范围


def open_interest_strategy(signal: pd.DataFrame,
    ...
    """
    openint = total_data['openint']
    signal = signal.reindex(total_data.index).fillna(0)
    codes = signal.columns
    mean = openint.rolling(int(window)).mean()
    # 多头：成交量高于均值且信号为1
    long_cond = (openint > mean) & (signal == 1)
    # 空头：成交量低于均值且信号为-1
    short_cond = (openint < mean) & (signal == -1)
...
def create_muster(window_sets=None, max_volume_sets=None):
    ...
    window_sets = window_sets if isinstance(
        window_sets, list) else default_open_interest_window_range
    ...
    for window in window_sets:
        for max_volume in max_volume_sets:
            muster.append(
                Function(function=open_interest_strategy,
                         name='open_interest_strategy',
                         params={
                             'window': int(window),
