...
def calc_maximum_sum(kl_pd, xd, ewm=True):
    ...
    if ewm:
        maximum = pd_ewm_sum(pd.Series(np.where(ret > ext_big, ret, np.nan)),
                             span=xd,
                             min_periods=1)
    else:
        maximum = pd_rolling_sum(pd.Series(np.where(ret > ext_big, ret,
                                                    np.nan)),
                                 window=xd,
                                 min_periods=1)

    line = Line(maximum, 'maximum_freq') if not maximum.empty else Line(
        0, 'maximum_freq')
    return line
