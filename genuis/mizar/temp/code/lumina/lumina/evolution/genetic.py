...
from ultron.factor.genetic.geneticist.fitness import metrics_fitness
from ultron.factor.genetic.geneticist.operators import operators_sets, Function, FunctionType
from lumina.evolution.program import Program
from lumina.evolution.warehouse import WareHouse, callback_relevance, sequential_gain
from lumina.evolution.adaptive import Adaptive
from lumina.evolution.thresholdor import Thresholdor
import warnings
...
def merge_factors(best_programs, is_concat=True):
    res = []
    for best_program in best_programs:
        factor_data = best_program._factor_data
        factor_data = factor_data.reset_index().set_index(
            ['trade_time', 'code'])['transformed'].sort_index()
        factor_data.name = best_program._name
...
def parallel_evolve(n_programs, parents, total_data, seeds, greater_is_better,
    ...
    def _tournament(tour_parents):
        contenders = random_state.randint(0, len(tour_parents),
                                          tournament_size)
        final_fitness = [tour_parents[p]._final_fitness for p in contenders]
        if greater_is_better:
            parent_index = contenders[np.argmax(final_fitness)]
        else:
            parent_index = contenders[np.argmin(final_fitness)]
...
class Gentic(object):
    ...
    def __init__(
        ...
        self._function_set = list(set([op.name for op in self._operators_set]))
    ...
    def save_model(self,
        ...
        result_list = [{
            ...
            'fitness': program._final_fitness
        ...
        for gen in range(prior_generations, self._generations):
            ...
            if gen == 0:
                ...
                parents = [(setattr(p, '_operator_probs',
                            ...
                            setattr(p, '_factor_probs',
                                    self._factor_probs), p)[-1]
                           for p in parents]
            ...
            kd_logger.info("0. 本轮共挖掘 program {}".format(len(population)))
            ...
            kd_logger.info("1. 无效program {}-->{}，保留率:{}".format(
                valid_count, len(population),
            ...
            #population = self.filter_programs(
            #    gen, population, self._standard_score * 0.0)  # 大于0即可
            ...
            ## 相关性惩罚 调整fitness分数
            ## 必须要在筛选之前
            ## 一堆高相关的、_raw_fitness虚高的因子可能在筛选阶段就被选入了best_programs，
            ## 而一些低相关、_raw_fitness稍低但惩罚后分数可能更高的因子，在filter_programs阶段就被过早地淘汰了。
            ## 惩罚没有参与到精英筛选中
   
            if self._ware_house.permanent_factors is not None:
            ...
            ## 更新基准库
            if 'warehouse' in self._custom_params:
                if self._ware_house.permanent_factors is None:
                    candidate_factors = merge_factors(self._best_programs,
                                                      True)
                    ## 新建核心库, 做相似过滤
                    returns_series = total_data.reset_index().set_index(
                        ['trade_time', 'code'])['nxt1_ret']
                    factors_data = sequential_gain(
                    ...
                    self._ware_house.set_initial_benchmark(
                        core_factors=factors_data)
                else:
                    factors_data = merge_factors(self._best_programs, False)
                    for factor_data in factors_data:
                        self._ware_house.add_new_factor(factor_data)
            ...
            ## 动态更新基因概率
            self._operator_probs, self._factor_probs = self.probs(
            ...
            kd_logger.info("最后有效种群:{0}".format(len(self._best_programs)))
    ...
    def probs(self, best_programs, operators_set, factors_sets):
