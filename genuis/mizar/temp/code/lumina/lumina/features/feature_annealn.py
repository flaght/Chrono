...
from lumina.techinical import calc_annealn


class FeatureAnnealn(FeatureBase, BuyFeatureMixin, SellFeatureMixin):

    def __init__(self):
        self.annealn_keys = frozenset([(7, 1), (14, 1), (7, 0)])  # xd, ewm
        self.windows = [i for i in range(g_max_window + 1)]

    def get_feature_keys(self, buy_feature):
        return [
            '{}price_annealn{}_{}_w{}'.format(
                self.feature_prefix(buy_feature=buy_feature), dk[0], dk[1], w)
            for dk in self.annealn_keys for w in self.windows
        ]

    def _calc_feature(self, kl_pd, combine_kl_pd, day_ind, buy_feature, window,
                      annealn_dict):
        for dk in self.annealn_keys:
            day_th = dk[0] * 2 + 1
            ...
            annealn = calc_annealn(annealn_df,
                                   xd=dk[0],
            ...
            annealn_dict['{}price_annealn{}_{}_w{}'.format(
