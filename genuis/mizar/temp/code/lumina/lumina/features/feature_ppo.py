# -*- encoding:utf-8 -*-
import pdb
import numpy as np
from lumina.env import g_max_window, g_point_num
from lumina.features.fixes import BuyFeatureMixin, SellFeatureMixin, FeatureBase
from lumina.techinical import calc_ppo


class FeaturePPO(FeatureBase, BuyFeatureMixin, SellFeatureMixin):

    def __init__(self):
        self.ppo_keys = frozenset([(7, 14, 1, 1), (5, 10, 1, 0),
                                   (5, 10, 1, 1)])  # fast slow, scalar, ewm
        self.windows = [i for i in range(g_max_window + 1)]

    def get_feature_keys(self, buy_feature):
        return [
            '{}price_ppo{}_{}_{}_{}_w{}'.format(
                self.feature_prefix(buy_feature=buy_feature), dk[0], dk[1],
                dk[2], dk[3], w) for dk in self.ppo_keys for w in self.windows
        ]

    def _calc_feature(self, kl_pd, combine_kl_pd, day_ind, buy_feature, window,
                      ppo_dict):
        for dk in self.ppo_keys:
            day_th = dk[1] * 2 + 1
            if day_ind - day_th - window >= 0:
                ppo_df = kl_pd[day_ind - day_th + 1 - window:day_ind + 1 -
                               window]
            else:
                combine_kl_pd = combine_kl_pd.loc[:kl_pd.index[day_ind]]
                ppo_df = combine_kl_pd[-day_th -
                                       window:] if combine_kl_pd.shape[0] > (
                                           day_th + window) else combine_kl_pd
            ppo = calc_ppo(ppo_df,
                           fast=dk[0],
                           slow=dk[1],
                           scalar=dk[2],
                           ewm=True if dk[3] == 1 else False)
            ppo_score = ppo.close
            ppo_score = 0 if np.isnan(ppo_score) else round(
                ppo_score, g_point_num)
            ppo_dict['{}price_ppo{}_{}_{}_{}_w{}'.format(
                self.feature_prefix(buy_feature=buy_feature), dk[0], dk[1],
                dk[2], dk[3], window)] = ppo_score

    def calc_feature(self, kl_pd, combine_kl_pd, day_ind, buy_feature):
        ppo_dict = {}
        for w in self.windows:
            self._calc_feature(kl_pd=kl_pd,
                               combine_kl_pd=combine_kl_pd,
                               day_ind=day_ind,
                               buy_feature=buy_feature,
                               window=w,
                               ppo_dict=ppo_dict)
        return ppo_dict
