...
from lumina.techinical import calc_liquidity2


class FeatureLiquidity2(FeatureBase, BuyFeatureMixin, SellFeatureMixin):

    def __init__(self):
        self.liquidity2_keys = frozenset([(3, 1)])  # xd, ewm
        self.windows = [i for i in range(g_max_window + 1)]

    def get_feature_keys(self, buy_feature):
        return [
            '{}volume_liquidity2{}_{}_w{}'.format(
                self.feature_prefix(buy_feature=buy_feature), dk[0], dk[1], w)
            for dk in self.liquidity2_keys for w in self.windows
        ]

    def _calc_feature(self, kl_pd, combine_kl_pd, day_ind, buy_feature, window,
                      annealn_dict):
        for dk in self.liquidity2_keys:
            day_th = dk[0] * 2 + 1
            if day_ind - day_th - window >= 0:
                ...
                annealn_df = combine_kl_pd[-day_th - window:] if combine_kl_pd.shape[
                    0] > (day_th + window) else combine_kl_pd
            annealn = calc_liquidity2(annealn_df,
                                      xd=dk[0],
            ...
            annealn_dict['{}volume_liquidity2{}_{}_w{}'.format(
