...
from lumina.techinical import calc_er


class FeatureER(FeatureBase, BuyFeatureMixin, SellFeatureMixin):

    def __init__(self):
        self.er_keys = frozenset([(7, 2, 1), (14, 2, 1),
                                  (7, 2, 0)])  # xd,drift,ewm
        self.windows = [i for i in range(g_max_window + 1)]

    def get_feature_keys(self, buy_feature):
        return [
            '{}price_er{}_{}_{}_w{}'.format(
                self.feature_prefix(buy_feature=buy_feature), dk[0], dk[1],
                dk[2], w) for dk in self.er_keys for w in self.windows
        ]

    def _calc_feature(self, kl_pd, combine_kl_pd, day_ind, buy_feature, window,
                      er_dict):
        for dk in self.er_keys:
            day_th = dk[0] * 2 + 1
            if day_ind - day_th - window >= 0:
                er_df = kl_pd[day_ind - day_th + 1 - window:day_ind + 1 -
                              window]
            else:
                combine_kl_pd = combine_kl_pd.loc[:kl_pd.index[day_ind]]
                er_df = combine_kl_pd[-day_th -
            ...
            er_score = er.close
            er_score = 0 if np.isnan(er_score) else round(
                er_score, g_point_num)
            er_dict['{}price_er{}_{}_{}_w{}'.format(
                self.feature_prefix(buy_feature=buy_feature), dk[0], dk[1],
                dk[2], window)] = er_score
