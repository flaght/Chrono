...
from lumina.techinical import calc_rank


class FeatureRank(FeatureBase, BuyFeatureMixin, SellFeatureMixin):

    def __init__(self):
        self.rank_keys = frozenset([60, 90, 120])
    ...
    def get_feature_keys(self, buy_feature):
        return [
            ...
            for dk in self.rank_keys for w in self.windows
        ]

    def _calc_feature(self, kl_pd, combine_kl_pd, day_ind, buy_feature, window,
                      price_rank_dict):
        for dk in self.rank_keys:
            day_th = dk
            ...
            price_rank = calc_rank(price_rank_df)
