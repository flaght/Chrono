...
def run1(method, instruments, task_id, period):
    ...
    total_data = pd.concat([train_data,val_data],axis=0).sort_values(by=['trade_time','code'])
    selected = []
    selected_expr = []
    corr_threshold = 0.7
    pdb.set_trace()
    for _, row in metrics_data.iterrows():
        ...
        for kept_expr in selected_expr:
            ...
            corr = tmp["cur"].rolling(window=15, min_periods=5).corr(
                tmp["kept"]).replace([np.inf, -np.inf], np.nan).dropna().mean()
...
def selected(method, instruments, task_id, period):
    ...
    pdb.set_trace()
    for thr in thresholds:
        selected_metrics[thr].to_csv(os.path.join(dt_path, "{0}.csv".format(str(thr * 10))))
...
def create_evaluate(df, factor_names, pnl_ret_col,  
    ...
    ax1.plot(profit_data['corr_4'].index, profit_data['corr_4'].cumsum().values, label="Corr_4", color="orange", linewidth=1.8)
    ax1.plot(profit_data['corr_5'].index, profit_data['corr_5'].cumsum().values, label="Corr_5", color="royalblue", linewidth=1.8)
    ax1.plot(profit_data['corr_6'].index, profit_data['corr_6'].cumsum().values, label="Corr_6", color="purple", linewidth=1.8)
    ax1.plot(profit_data['corr_7'].index, profit_data['corr_7'].cumsum().values, label="Corr_7", color="seagreen", linewidth=1.8)
    ax1.plot(profit_data['corr_8'].index, profit_data['corr_8'].cumsum().values, label="Corr_8", color="goldenrod", linewidth=1.8)
    ax1.set_title("Net")
    ax1.set_ylabel("NAV")
    ax1.legend(loc="best")
    
    ax2 = axes[1]
    ax2.plot(ic_data['corr_4'].index, ic_data['corr_4'].cumsum().values, label="Corr_4", color="orange", linewidth=1.8)
    ax2.plot(ic_data['corr_5'].index, ic_data['corr_5'].cumsum().values, label="Corr_5", color="royalblue", linewidth=1.8)
    ax2.plot(ic_data['corr_6'].index, ic_data['corr_6'].cumsum().values, label="Corr_6", color="purple", linewidth=1.8)
    ax2.plot(ic_data['corr_7'].index, ic_data['corr_7'].cumsum().values, label="Corr_7", color="seagreen", linewidth=1.8)
    ax2.plot(ic_data['corr_8'].index, ic_data['corr_7'].cumsum().values, label="Corr_8", color="goldenrod", linewidth=1.8)
...
def composite(method, instruments, task_id, period):
    ...
    for csv_file in file_path.rglob('*.csv'):
        ...
        train_data["corr_{0}".format(name)] = train_data[corr_data['expression'].to_list()].mean(axis=1)
        val_data["corr_{0}".format(name)] = val_data[corr_data['expression'].to_list()].mean(axis=1)
        test_data["corr_{0}".format(name)] = test_data[corr_data['expression'].to_list()].mean(axis=1)
    
    image_path = os.path.join(base_path, method, instruments, 'temp','model', 
                           str(task_id), str(period), 'rl', 'blend', 'metrics')
    print(image_path)
    os.makedirs(image_path, exist_ok=True)
    
    create_evaluate(df=train_data, factor_names=['corr_4', 'corr_5','corr_6', 'corr_8', 'corr_7'], 
    ...
    create_evaluate(df=val_data, factor_names=['corr_4', 'corr_5','corr_6', 'corr_8', 'corr_7'], 
                    pnl_ret_col='nxt1_ret_5h',  
                    cost_rate=COST_MAPPING[INSTRUMENTS_CODES[instruments]], 
                    holding_period=period, 
                    pnl_method='points_norm',
    ...
    create_evaluate(df=test_data, factor_names=['corr_4', 'corr_5', 'corr_6', 'corr_8', 'corr_7'], 
                    pnl_ret_col='nxt1_ret_5h',  
                    cost_rate=COST_MAPPING[INSTRUMENTS_CODES[instruments]], 
                    holding_period=period, 
                    pnl_method='points_norm',
