...
def calculate(df_lazy: pl.LazyFrame, period: int) -> pl.LazyFrame:
    ...
    return x.with_columns(pl.when(pl.col("_d")>pl.col("_m")+pl.col("_s")).then(pl.col("_r")).otherwise(0.0).rolling_std(period).over("code").alias(NAME))


def compute(df_lazy: pl.LazyFrame, period: int = DEFAULT_PERIOD) -> pl.LazyFrame:
    """从 df_lazy: pl.LazyFrame 计算 tc004_008；输入含 trade_time、code、open、high、low、close、volume、value。"""
