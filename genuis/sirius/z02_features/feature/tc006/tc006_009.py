...
因子代号: tc006_009
原历史名: tc006_009
因子定义: 带符号方向性趋势强度 ADX(14) * SIGN(PLUS_DI(14) - MINUS_DI(14))
"""
import polars as pl
...
def calculate(df_lazy: pl.LazyFrame) -> pl.LazyFrame:
    """
    计算带符号方向性趋势强度 TREND_INTENSITY。

    参数:
        df_lazy: pl.LazyFrame，必须包含 trade_time、code、high、low、close
    返回:
        pl.LazyFrame: 包含 trade_time, code, tc006_009 列
    """
    factor_lazy = (
        df_lazy
        ...
        .with_columns(
            ...
            (pl.col('_plus_di') - pl.col('_minus_di')).sign().alias('_di_sign'),
        )
        .with_columns(
            pl.col('_dx').ewm_mean(alpha=1.0/14.0, adjust=False).over('code').alias('_adx')
        )
        .with_columns(
            (pl.col('_adx') * pl.col('_di_sign')).alias("tc006_009")
        )
        .select(['trade_time', 'code', "tc006_009"])
    )
    return factor_lazy


def compute(df_lazy: pl.LazyFrame) -> pl.LazyFrame:
    """从 df_lazy: pl.LazyFrame 计算 tc006_009；输入含 trade_time、code、high、low、close。"""
