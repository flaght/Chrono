...
因子代号: tc006_010
原历史名: tc006_010
因子定义: 20周期顺势指标 CCI(high, low, close, 20)
"""
import polars as pl
...
def calculate(df_lazy: pl.LazyFrame) -> pl.LazyFrame:
    ...
    返回:
        pl.LazyFrame: 包含 trade_time, code, tc006_010 列
    """
    factor_lazy = (
        df_lazy
        ...
        .with_columns(
            safe_div(pl.col('_tp') - pl.col('_sma_tp'), 0.015 * pl.col('_md')).alias("tc006_010")
        )
        .select(['trade_time', 'code', "tc006_010"])
    )
    return factor_lazy


def compute(df_lazy: pl.LazyFrame) -> pl.LazyFrame:
    """从 df_lazy: pl.LazyFrame 计算 tc006_010；输入含 trade_time、code、high、low、close。"""
