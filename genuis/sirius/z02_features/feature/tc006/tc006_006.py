...
因子代号: tc006_006
原历史名: tc006_006
因子定义: 14周期平均趋向指标 ADX(high, low, close, 14)
"""
import polars as pl
...
def calculate(df_lazy: pl.LazyFrame) -> pl.LazyFrame:
    ...
    返回:
        pl.LazyFrame: 包含 trade_time, code, tc006_006 列
    """
    factor_lazy = (
        df_lazy
        ...
        .with_columns(
            pl.col('_dx').ewm_mean(alpha=1.0/14.0, adjust=False).over('code').alias("tc006_006")
        )
        .select(['trade_time', 'code', "tc006_006"])
    )
    return factor_lazy


def compute(df_lazy: pl.LazyFrame) -> pl.LazyFrame:
    """从 df_lazy: pl.LazyFrame 计算 tc006_006；输入含 trade_time、code、high、low、close。"""
