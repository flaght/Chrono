...
因子代号: tc002_013
原历史名: tc002_013
因子定义: 量价协方差符号动量：价格收益率与成交量变化率的滚动协方差符号及强度
"""
import polars as pl
...
def calculate(df_lazy: pl.LazyFrame, period: int) -> pl.LazyFrame:
    ...
    x=x.with_columns(pl.when(pl.col("_r")>pl.col("_m")+pl.col("_s")).then(pl.col("volume")).alias("_v"))
    return x.with_columns(safe_div(pl.col("_v").rolling_std(period).over("code"),pl.col("_v").rolling_mean(period).over("code")).alias(NAME))


def compute(df_lazy: pl.LazyFrame, period: int = DEFAULT_PERIOD) -> pl.LazyFrame:
    """从 df_lazy: pl.LazyFrame 计算 tc002_013；输入含 trade_time、code、open、high、low、close、volume、value。"""
